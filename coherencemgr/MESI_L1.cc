...
bool MESIL1::handleGetS(MemEvent *event, bool inMSHR) {
  ...
  switch (state) {
  case I: /* Miss */
    status = processCacheMiss(
        event, line, inMSHR); // Attempt to allocate an MSHR entry and/or line

    if (status == MemEventStatus::OK) {
      line = cacheArray_->lookup(addr, false);
      // eventProfileAndNotify(event, I, NotifyAccessType::READ,
      // NotifyResultType::MISS, true, LatType::MISS);
  ...
  case M:
    ...
    if (event->isLoadLink())
      line->atomicStart();
    data.assign(
        line->getData()->begin() + (event->getAddr() - event->getBaseAddr()),
        line->getData()->begin() +
            (event->getAddr() - event->getBaseAddr() + event->getSize()));
...
bool MESIL1::handleGetX(MemEvent *event, bool inMSHR) {
  ...
  case S:
    status =
        processCacheMiss(event, line, inMSHR); // Just acquire an MSHR entry
    if (status == MemEventStatus::OK) {
      if (!mshr_->getProfiled(addr)) {
        notifyListenerOfAccess(event, NotifyAccessType::WRITE,
                               NotifyResultType::MISS);
  ...
  case M:
    ...
    if (!event->isStoreConditional() ||
        line->isAtomic()) { // Don't write on a non-atomic SC
      line->setData(event->getPayload(),
    ...
    sendTime =
        sendResponseUp(event, nullptr, inMSHR, line->getTimestamp(), atomic);
    line->setTimestamp(sendTime - 1);
    if (is_debug_addr(addr))
      eventDI.reason = "hit";
    cleanUpAfterRequest(event, inMSHR);
 ...
 * GetSX acquires a line in exclusive state and locks it until any future GetX
 * arrives
 */
...
bool MESIL1::handleFlushLine(MemEvent *event, bool inMSHR) {
  ...
  if (!inMSHR && mshr_->exists(addr)) {
    return (allocateMSHR(event, false) == MemEventStatus::Reject) ? false
                                                                  : true;
  } else if (inMSHR) {
    mshr_->removePendingRetry(addr);
  }
...
bool MESIL1::handleFetch(MemEvent *event, bool inMSHR) {
  ...
  snoopInvalidation(event, line); // Let core know that line is being accessed
                                  // elsewhere (for ARM/gem5)

  if (is_debug_event(event))
    eventDI.prefill(event->getID(), Command::Fetch, false, event->getBaseAddr(),
                    state);
  ...
  case IM:
  case I_B:
    /* In these cases, an eviction raced with this request; ignore and wait for
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 ...
                 "%s. Time = %" PRIu64 "ns\n",
                 cachename_.c_str(), StateString[state],
...
bool MESIL1::handleInv(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_event(event))
    eventDI.prefill(event->getID(), Command::Inv, false, event->getBaseAddr(),
                    state);
  ...
  /* Note - not possible to receive an inv when the line is locked (locked
   * implies state = E or M) */
  ...
  case IM:
  case I:
    // Raced with Put* or FlushLineInv; will be resolved by arrival of our
    // request
    if (is_debug_event(event))
      eventDI.action = "Ignore";
    break;
...
bool MESIL1::handleForceInv(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_event(event))
    eventDI.prefill(event->getID(), Command::ForceInv, false,
...
bool MESIL1::handleFetchInv(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_event(event))
    eventDI.prefill(event->getID(), Command::FetchInv, false,
                    event->getBaseAddr(), state);
...
bool MESIL1::handleFetchInvX(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_event(event))
    eventDI.prefill(event->getID(), Command::FetchInvX, false,
                    event->getBaseAddr(), state);
...
bool MESIL1::handleGetSResp(MemEvent *event, bool inMSHR) {
  ...
  if (localPrefetch) {
    ...
    vector<uint8_t> data(line->getData()->begin() + offset,
                         line->getData()->begin() + offset + req->getSize());
...
bool MESIL1::handleGetXResp(MemEvent *event, bool inMSHR) {
  ...
  case IS: {
    ...
    if (localPrefetch) {
      ...
      data.assign(line->getData()->begin() + offset,
                  line->getData()->begin() + offset + req->getSize());
      uint64_t sendTime =
          sendResponseUp(req, &data, true, line->getTimestamp());
      line->setTimestamp(sendTime - 1);
    }
    break;
  }
  ...
  case SM: {
    ...
    if (req->getCmd() == Command::GetX) {
      if (!req->isStoreConditional() ||
          line->isAtomic()) { // Normal or successful store-conditional
    ...
    data.assign(line->getData()->begin() + offset,
                line->getData()->begin() + offset + req->getSize());
    uint64_t sendTime =
        sendResponseUp(req, &data, true, line->getTimestamp(), false);
    line->setTimestamp(sendTime - 1);
    break;
  }
  default:
...
/* We're using NULLCMD to signal an internally generated event - in thsi case an
 * eviction */
...
bool MESIL1::handleNACK(MemEvent *event, bool inMSHR) {
  ...
  delete event;
  resendEvent(nackedEvent, false); // An L1 always resends NACKed events and
                                   // they are always sent down
 ...
 * Handle a cache miss
 */
MemEventStatus MESIL1::processCacheMiss(MemEvent *event, L1CacheLine *line,
                                        bool inMSHR) {
  MemEventStatus status = MemEventStatus::OK;
  ...
  if (mshr_->exists(addr)) {
    if (mshr_->getFrontType(addr) == MSHREntryType::Event) {
      ...
      mshr_->addPendingRetry(addr);
             ...
             // address is waiting to evict this one
...
void MESIL1::cleanUpAfterResponse(MemEvent *event, bool inMSHR) {
  ...
  mshr_->removeFront(addr); // delete req after this since debug might print the
                            // event it's removing
  delete event;
  if (req)
    delete req;
 ...
 *  data:  The data to be returned. **L1's return only the desired word so this
 * should be ONLY the requested bytes inMSHR: Whether 'event' is in the MSHR
 * (and required an MSHR access to look it up) time: The earliest time at which
 * the requested cacheline can be accessed success: for requests that can fail
 * (e.g., LLSC), whether the request was successful or not (default false)
 *
 *  Return: time that the requested cacheline can again be accessed
 */
uint64_t MESIL1::sendResponseUp(MemEvent *event, vector<uint8_t> *data,
                                bool inMSHR, uint64_t time, bool success) {
  Command cmd = event->getCmd();
...
void MESIL1::forwardFlush(MemEvent *event, L1CacheLine *line, bool evict) {
  ...
  if (evict) {
    ...
    latency =
        accessLatency_; // Time to check coherence & access data (in parallel)
  } else {
...
void MESIL1::snoopInvalidation(MemEvent *event, L1CacheLine *line) {
  if (snoopL1Invs_ && line) {
    MemEvent *snoop = new MemEvent(cachename_, event->getAddr(),
                                   event->getBaseAddr(), Command::Inv);
    uint64_t baseTime =
        timestamp_ > line->getTimestamp() ? timestamp_ : line->getTimestamp();
...
void MESIL1::recordPrefetchResult(L1CacheLine *line,
...
void MESIL1::eventProfileAndNotify(MemEvent *event, State state,
                                   NotifyAccessType type,
                                   NotifyResultType result, bool inMSHR) {
  if (!inMSHR || !mshr_->getProfiled(event->getBaseAddr())) {
    stat_eventState[(int)event->getCmd()][state]->addData(
        1); // Profile event receive
...
MemEventInitCoherence *MESIL1::getInitCoherenceEvent() {
  return new MemEventInitCoherence(cachename_, Endpoint::Cache, true, false,
...
Addr MESIL1::getBank(Addr addr) { return cacheArray_->getBank(addr); }

void MESIL1::printLine(Addr addr) {}
void MESIL1::printData(Addr addr) {}
void MESIL1::printData(vector<uint8_t> *data, bool set) {}

void MESIL1::printStatus(Output &out) { cacheArray_->printCacheArray(out); }
