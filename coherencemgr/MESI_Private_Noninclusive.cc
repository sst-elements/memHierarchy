 ...
 *
 * This protocol does not support prefetchers because the cache can't determine
 *whether an address would be a hit in an upper level (closer to cpu) cache.
 *
 *---------------------------------------------------------------------------------------------------------------------*/
...
bool MESIPrivNoninclusive::handleGetS(MemEvent *event, bool inMSHR) {
  ...
  case I:
    ...
    if (status == MemEventStatus::OK) {
      if (!mshr_->getProfiled(addr)) {
        ...
        notifyListenerOfAccess(event, NotifyAccessType::READ,
                               NotifyResultType::MISS);
  ...
  case S:
    ...
    line->setShared(true);
    sendTime =
        sendResponseUp(event, line->getData(), inMSHR, line->getTimestamp());
  ...
  case M:
    ...
    if (protocol_) { // Transfer ownership of dirty block
      line->setOwned(true);
      sendTime = sendExclusiveResponse(event, line->getData(), inMSHR,
                                       line->getTimestamp(), state == M);
    } else { // Will writeback dirty block if we evict
      line->setShared(true);
      sendTime = sendResponseUp(event, line->getData(), inMSHR,
                                line->getTimestamp(), Command::GetSResp);
    }
    recordLatencyType(event->getID(), LatType::HIT);
    line->setTimestamp(sendTime);
    cleanUpAfterRequest(event, inMSHR);
  ...
  case S:
    ...
    if (status == MemEventStatus::OK) {
      if (!mshr_->getProfiled(addr)) {
        ...
        stat_miss[(event->getCmd() == Command::GetX ? 1 : 2)][inMSHR]->addData(
            1);
        notifyListenerOfAccess(event, NotifyAccessType::WRITE,
                               NotifyResultType::MISS);
        mshr_->setProfiled(addr);
      }
      sendTime = forwardMessage(event, event->getSize(), 0, nullptr);
...
bool MESIPrivNoninclusive::handleFlushLine(MemEvent *event, bool inMSHR) {
  ...
  case I:
    if (status == MemEventStatus::OK) {
      forwardFlush(event, event->getEvict(), &(event->getPayload()),
                   event->getDirty(), 0);
      ...
      event->setEvict(false);
      if ((static_cast<MemEvent *>(mshr_->getFrontEvent(addr)))->getCmd() ==
          Command::FetchInvX) {
  ...
  case S:
    if (status == MemEventStatus::OK) {
      sendTime =
          forwardFlush(event, false, nullptr, false, line->getTimestamp());
      line->setTimestamp(sendTime);
      line->setState(S_B);
      mshr_->setInProgress(addr);
      if (!mshr_->getProfiled(addr)) {
  ...
  case M:
    if (status == MemEventStatus::OK) {
      ...
      forwardFlush(event, true, line->getData(),
                   (state == M || event->getDirty()), line->getTimestamp());
      line->setState(S_B);
      mshr_->setInProgress(addr);
      if (!mshr_->getProfiled(addr)) {
...
bool MESIPrivNoninclusive::handleFlushLineInv(MemEvent *event, bool inMSHR) {
  ...
  case I:
    ...
    if (status == MemEventStatus::OK) {
      forwardFlush(event, event->getEvict(), &(event->getPayload()),
                   event->getDirty(),
                   0); // No need to evict since we didn't race
      mshr_->setInProgress(addr);
      if (!mshr_->getProfiled(addr)) {
      ...
      if (mshr_->getFrontEvent(addr)) { // There's an event with raced with
        ...
        if (is_debug_event(event))
          eventDI.action = "Retry";
                 ...
                 mshr_->pendingWritebackIsDowngrade(addr) &&
                 mshr_->getEntryEvent(
                     addr, 1)) { // There's an event we *will* race with
        // In this case this cache issued a downgrade (PutX), an invalidation
        // was received & stalled pending the AckPut and now we've got a
        // flushlineinv Resolve race with the invalidation
      ...
      if (race->getCmd() == Command::FetchInvX) {
        event->setDirty(false);
                 ...
                 Command::Fetch) { // FetchInv, ForceInv, or Inv
        event->setDirty(false);
        event->setEvict(false);
      }
    }
    /* Note: If there's a WB-X followed by a FetchInv/Inv/etc. in the MSHR, the
     * FetchInv will get replayed when the WB-X resolves and will not forward
     * since we copied the evict data into the MSHR.
     */
    break;
  case S:
    if (status == MemEventStatus::OK) {
  ...
  case M:
    if (status == MemEventStatus::OK) {
      ...
      forwardFlush(event, true, line->getData(), line->getState() == M,
                   line->getTimestamp());
      line->setState(I_B);
      mshr_->setInProgress(addr);
      if (!mshr_->getProfiled(addr)) {
...
bool MESIPrivNoninclusive::handlePutS(MemEvent *event, bool inMSHR) {
  ...
  case I:
    if (!inMSHR &&
        mshr_->exists(addr)) { // Raced with something; must be an Inv/Fetch
                               // since there can only be one cache above us
      mshr_->setData(addr, event->getPayload(), false);
      responses.erase(addr);
      mshr_->decrementAcksNeeded(addr);
      if (mshr_->getFrontType(addr) == MSHREntryType::Event &&
          mshr_->getFrontEvent(addr)->getCmd() == Command::Fetch) {
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received PutS in unhandled state '%s'. Event: %s. "
                 "Time = %" PRIu64 "ns\n",
...
bool MESIPrivNoninclusive::handlePutE(MemEvent *event, bool inMSHR) {
  ...
  case I:
    if (mshr_->getAcksNeeded(addr) != 0) { // Race
      if (mshr_->getFrontType(addr) == MSHREntryType::Event &&
          mshr_->getFrontEvent(addr)->getCmd() == Command::FetchInvX) {
        mshr_->decrementAcksNeeded(addr);
        responses.erase(addr);
        mshr_->setData(addr, event->getPayload(), false);
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received PutE in unhandled state '%s'. Event: %s. "
                 "Time = %" PRIu64 "ns\n",
                 getName().c_str(), StateString[state],
                 event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool MESIPrivNoninclusive::handlePutM(MemEvent *event, bool inMSHR) {
  ...
  case I:
    if (mshr_->getAcksNeeded(addr) !=
        0) { // Race with eviction, FetchInvX, or FetchInv/Inv/ForceInv
      if (mshr_->getFrontType(addr) == MSHREntryType::Event &&
          mshr_->getFrontEvent(addr)->getCmd() == Command::FetchInvX) {
        mshr_->decrementAcksNeeded(addr);
        responses.erase(addr);
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received PutM in unhandled state '%s'. Event: %s. "
                 "Time = %" PRIu64 "ns\n",
                 getName().c_str(), StateString[state],
                 event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool MESIPrivNoninclusive::handlePutX(MemEvent *event, bool inMSHR) {
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received PutX in unhandled state '%s'. Event: %s. "
                 "Time = %" PRIu64 "ns\n",
                 getName().c_str(), StateString[state],
                 event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool MESIPrivNoninclusive::handleFetch(MemEvent *event, bool inMSHR) {
  ...
  case I:
    if (mshr_->hasData(addr)) {
      sendResponseDown(event, event->getSize(), &(mshr_->getData(addr)),
                       mshr_->getDataDirty(addr));
      cleanUpAfterRequest(event, inMSHR);
    } else if (!inMSHR && mshr_->exists(addr)) {
      if (mshr_->getFrontType(addr) == MSHREntryType::Writeback ||
          mshr_->getFrontEvent(addr)->getCmd() ==
              Command::FlushLineInv) { // Raced with eviction
        if (is_debug_event(event)) {
          eventDI.action = "Drop";
          eventDI.reason = "MSHR conflict";
        }
        delete event;
      } else if (mshr_->getFrontEvent(addr)->getCmd() ==
                 Command::PutS) { // Raced with replacement
        ...
        sendFwdRequest(event, Command::Fetch, upperCacheName_, event->getSize(),
                       0, inMSHR);
      }
    } else {
      if (!inMSHR)
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received Fetch in unhandled state '%s'. Event: "
                 "%s. Time = %" PRIu64 "ns\n",
                 getName().c_str(), StateString[state],
                 event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool MESIPrivNoninclusive::handleInv(MemEvent *event, bool inMSHR) {
  ...
  case I:
    ...
    if (mshr_->exists(addr) &&
        (mshr_->getFrontType(addr) == MSHREntryType::Writeback ||
         mshr_->getFrontEvent(addr)->getCmd() ==
             Command::FlushLineInv)) { // Already evicted block, drop the Inv
      if (is_debug_event(event)) {
        eventDI.action = "Drop";
        eventDI.reason = "MSHR conflict";
      }
      delete event;
               ...
               mshr_->getFrontEvent(addr)->getCmd() == Command::PutS) {
      // Handle Inv
      ...
      cleanUpAfterRequest(event, inMSHR);
               ...
               mshr_->hasData(addr)) { // Race with Put (replay)
      // Handle Inv
      sendResponseDown(event, event->getSize(), nullptr, false);
      if (mshr_->hasData(addr))
        mshr_->clearData(addr);
      ...
      if (status == MemEventStatus::OK) {
        sendFwdRequest(event, Command::Inv, upperCacheName_, event->getSize(),
                       0, inMSHR);
      }
    }
    break;
  ...
  case S_B: // If sharers -> SB_Inv & stall, if no sharers -> I & respond (done)
    ...
    handle = true;
    break;
  case SM: // If requestor is sharer, send inv -> SM_Inv, & stall. If no
           // sharers, -> I & respond(done)
    state1 = SM_Inv;
    state2 = I;
    handle = true;
    break;
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received Inv in unhandled state '%s'. Event: %s. "
                 "Time = %" PRIu64 "ns\n",
                 getName().c_str(), StateString[state],
                 event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
  ...
  if (handle) {
    if (line->getShared()) {
      ...
      if (status == MemEventStatus::OK) {
        uint64_t sendTime =
            sendFwdRequest(event, Command::Inv, upperCacheName_,
                           event->getSize(), line->getTimestamp(), inMSHR);
...
bool MESIPrivNoninclusive::handleForceInv(MemEvent *event, bool inMSHR) {
  ...
  switch (state) {
  case I:
    if (mshr_->pendingWritebackIsDowngrade(
            addr)) { // Waiting for AckPut to PutX, go ahead with the force inv
      ...
      status = allocateMSHR(event, true, 1); // Retry when we get the AckPut
               ...
               (mshr_->exists(addr) && mshr_->getFrontEvent(addr)->getCmd() ==
      ...
      delete event;
               ...
               mshr_->getFrontEvent(addr)->getCmd() ==
                   Command::PutX) { // Drop PutX, Ack it, forward request up
      MemEvent *put =
          static_cast<MemEvent *>(mshr_->swapFrontEvent(addr, event));
      sendWritebackAck(put);
      mshr_->setData(addr, put->getPayload(), put->getDirty());
      delete put;
      sendFwdRequest(event, Command::ForceInv, upperCacheName_,
                     event->getSize(), 0, inMSHR);
    } else if (mshr_->exists(addr) &&
               (CommandWriteback[(int)mshr_->getFrontEvent(addr)->getCmd()])) {
      if (mshr_->hasData(addr))
        mshr_->clearData(addr);
      sendWritebackAck(static_cast<MemEvent *>(mshr_->getFrontEvent(addr)));
  ...
  case SM_Inv: { // ForceInv if there's an un-inv'd sharer, else in mshr & stall
    ...
    if (status != MemEventStatus::Reject) {
      if (line->getShared()) {
        uint64_t sendTime =
            sendFwdRequest(event, Command::ForceInv, src, lineSize_,
                           line->getTimestamp(), inMSHR);
        line->setTimestamp(sendTime);
      }
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received ForceInv in unhandled state '%s'. Event: "
                 "%s. Time = %" PRIu64 "ns\n",
                 getName().c_str(), StateString[state],
                 event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool MESIPrivNoninclusive::handleFetchInv(MemEvent *event, bool inMSHR) {
  ...
  case I:
    if (inMSHR && mshr_->hasData(addr)) { // Replay
      ...
      mshr_->clearData(addr);
      cleanUpAfterRequest(event, inMSHR);
    } else if (mshr_->pendingWritebackIsDowngrade(
                   addr)) { // Resolve races with pending evictions from upper
                            // level caches
      /* In this case we're waiting for an AckPut for a PutX so we've gone from
       * M/E to S. Check if any queued events are evictions and if so, handle
       * them. Otherwise stallthe FetchInv until the writeback ack returns. Note
       * that the next level can NACK a PutX if it is noninclusive and in a
       * stable state, but it won't NACK the PutX if it's in a transition state.
      ...
      if (entry) {
        if (entry->getCmd() == Command::PutS) {
          // Return AckInv
          sendResponseDown(event, event->getSize(),
                           &(static_cast<MemEvent *>(entry)->getPayload()),
                           false);
          delete event;
          // Drop PutS
          if (mshr_->hasData(addr))
      ...
      // Stall the event until the writeback ack returns (don't actually have to
      // but it makes the tracking simpler) Since we profiled this event in I
      // above, that means we're going to have slightly imprecise statistics
      // (will eventually be correct) TODO fix this maybe?
      ...
      mshr_->setData(addr, put->getPayload(), put->getDirty());
      delete put;
      sendFwdRequest(event, Command::FetchInv, upperCacheName_,
                     event->getSize(), 0, inMSHR);
    } else if (mshr_->exists(addr) &&
               (CommandWriteback[(int)mshr_->getFrontEvent(addr)->getCmd()])) {
      MemEvent *put = static_cast<MemEvent *>(mshr_->getFrontEvent(addr));
      sendWritebackAck(put);
      sendResponseDown(event, put->getSize(), &(put->getPayload()),
                       put->getDirty());
      mshr_->removeFront(addr);
      delete put;
      cleanUpAfterRequest(event, inMSHR);
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received FetchInv in unhandled state '%s'. Event: "
                 "%s. Time = %" PRIu64 "ns\n",
                 getName().c_str(), StateString[state],
                 event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool MESIPrivNoninclusive::handleFetchInvX(MemEvent *event, bool inMSHR) {
  ...
  case I:
    if (inMSHR && mshr_->hasData(addr)) { // Replay
      ...
      if (mshr_->getFrontEvent(addr)->getCmd() == Command::FlushLine ||
          mshr_->getFrontEvent(addr)->getCmd() == Command::FlushLineInv) {
        if (is_debug_event(event)) {
          eventDI.action = "Drop";
          eventDI.reason = "MSHR conflict";
        }
        ...
        cleanUpAfterRequest(event, inMSHR);
        break;
      } else if (mshr_->getFrontEvent(addr)->getCmd() == Command::PutE ||
                 mshr_->getFrontEvent(addr)->getCmd() == Command::PutM) {
        MemEvent *put = static_cast<MemEvent *>(mshr_->getFrontEvent(addr));
        sendResponseDown(event, put->getSize(), &(put->getPayload()),
                         put->getDirty());
        put->setCmd(Command::PutS); // Make this a PutS so we only record the
                                    // block in shared later
        put->setDirty(false);
        delete event;
        break;
      }
    ...
    if (!inMSHR)
      status = allocateMSHR(event, true, 0);
    if (status == MemEventStatus::OK) {
      sendFwdRequest(event, Command::FetchInvX, upperCacheName_,
                     event->getSize(), 0, inMSHR);
    }
    break;
  case E:
  case M:
    if (line->getOwned()) {
      ...
      if (status == MemEventStatus::OK) {
        uint64_t sendTime =
            sendFwdRequest(event, Command::FetchInvX, upperCacheName_,
                           lineSize_, line->getTimestamp(), inMSHR);
        line->setTimestamp(sendTime);
        mshr_->setInProgress(addr);
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received FetchInvX in unhandled state '%s'. "
...
bool MESIPrivNoninclusive::handleGetSResp(MemEvent *event, bool inMSHR) {
  ...
  uint64_t sendTime = sendResponseUp(req, &(event->getPayload()), true,
                                     line ? line->getTimestamp() : 0);
...
bool MESIPrivNoninclusive::handleGetXResp(MemEvent *event, bool inMSHR) {
  ...
  switch (state) {
  case I: {
    sendExclusiveResponse(req, &(event->getPayload()), true, 0,
                          event->getDirty());
    cleanUpAfterResponse(event, inMSHR);
    break;
  }
  case SM: {
    ...
    uint64_t sendTime = sendExclusiveResponse(
        req, line->getData(), true, line->getTimestamp(), event->getDirty());
    line->setTimestamp(sendTime - 1);
    cleanUpAfterResponse(event, inMSHR);
    break;
  }
...
bool MESIPrivNoninclusive::handleFlushLineResp(MemEvent *event, bool inMSHR) {
  ...
  sendResponseUp(req, nullptr, true, timestamp_, Command::FlushLineResp,
                 event->success());
...
bool MESIPrivNoninclusive::handleFetchResp(MemEvent *event, bool inMSHR) {
  ...
  if (state == I) { // Fetch or FetchInv
    MemEvent *req = static_cast<MemEvent *>(mshr_->getFrontEvent(addr));
    sendResponseDown(req, event->getSize(), &(event->getPayload()),
                     event->getDirty());
    cleanUpAfterResponse(event, inMSHR);
  } else { // FetchInv only
    if (event->getDirty()) {
...
bool MESIPrivNoninclusive::handleAckInv(MemEvent *event, bool inMSHR) {
  ...
  case I: {
    ...
    if (mshr_->hasData(addr) && req->getCmd() == Command::FetchInv)
      sendResponseDown(req, req->getSize(), &(mshr_->getData(addr)),
                       mshr_->getDataDirty(addr));
    else
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received AckInv in unhandled state '%s'. Event: "
                 "%s. Time = %" PRIu64 "ns\n",
                 getName().c_str(), StateString[state],
                 event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool MESIPrivNoninclusive::handleAckPut(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_event(event)) {
    eventDI.prefill(event->getID(), Command::AckPut, false,
                    event->getBaseAddr(), I);
    eventDI.action = "Done";
  }
  cleanUpAfterResponse(event, inMSHR);
...
/* We're using NULLCMD to signal an internally generated event - in this case an
 * eviction */
bool MESIPrivNoninclusive::handleNULLCMD(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_addr(oldAddr))
    eventDI.prefill(event->getID(), Command::Evict, false, oldAddr,
                    (line ? line->getState() : I));

  bool evicted = handleEviction(newAddr, line, eventDI);
  if (is_debug_addr(oldAddr) || is_debug_addr(newAddr) ||
      is_debug_addr(line->getAddr())) {
    eventDI.newst = line->getState();
    eventDI.verboseline = line->getString();
  }
  if (evicted) {
    notifyListenerOfEvict(line->getAddr(), lineSize_,
    ...
    if (oldAddr != line->getAddr()) { // We're waiting for a new line now...
      if (is_debug_addr(oldAddr) || is_debug_addr(line->getAddr()) ||
          is_debug_addr(newAddr)) {
        stringstream reason;
...
bool MESIPrivNoninclusive::handleNACK(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_event(event) || is_debug_event(nackedEvent)) {
    eventDI.prefill(event->getID(), Command::NACK, false, event->getBaseAddr(),
                    (line ? line->getState() : I));
  }
  ...
  case Command::ForceInv:
    ...
    addr, responses.find(addr)->second); } else { debug->debug(_L3_, "\tResponse
    for 0x%" PRIx64 " is: None\n", addr);
        }
    }*/
    if ((responses.find(addr) != responses.end()) &&
        (responses.find(addr)->second == nackedEvent->getID())) {
      // if (is_debug_addr(addr))
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received NACK with unhandled command type. Event: "
                 "%s. NackedEvent: %s Time = %" PRIu64 "ns\n",
                 getName().c_str(), event->getVerboseString().c_str(),
                 nackedEvent ? nackedEvent->getVerboseString().c_str()
                             : "nullptr",
 ...
 */
MemEventStatus MESIPrivNoninclusive::allocateLine(MemEvent *event,
                                                  PrivateCacheLine *&line,
                                                  bool inMSHR) {
  ...
  if (evicted) {
    ...
    if (inMSHR || mshr_->insertEvent(event->getBaseAddr(), event, -1, false,
                                     true) != -1) {
      mshr_->insertEviction(
          line->getAddr(),
          event->getBaseAddr()); // Since we're private we never start an
                                 // eviction (e.g., send invs) so it's safe to
                                 // ignore an attempted eviction
      if (inMSHR)
...
bool MESIPrivNoninclusive::handleEviction(Addr addr, PrivateCacheLine *&line,
                                          dbgin &diStruct) {
  if (!line)
  ...
  case S:
    if (!mshr_->getPendingRetries(line->getAddr())) {
      if (!line->getShared() && !silentEvictClean_) {
        uint64_t sendTime =
            sendWriteback(line->getAddr(), lineSize_, Command::PutS,
                          line->getData(), false, line->getTimestamp());
        line->setTimestamp(sendTime - 1);
  ...
  case E:
    if (!mshr_->getPendingRetries(line->getAddr())) {
      if (line->getShared()) {
        uint64_t sendTime =
            sendWriteback(line->getAddr(), lineSize_, Command::PutX,
                          line->getData(), false, line->getTimestamp());
        line->setTimestamp(sendTime - 1);
        ...
        uint64_t sendTime =
            sendWriteback(line->getAddr(), lineSize_, Command::PutE,
                          line->getData(), false, line->getTimestamp());
        line->setTimestamp(sendTime - 1);
        mshr_->insertWriteback(line->getAddr(), false);
        if (is_debug_addr(line->getAddr()))
  ...
  case M:
    if (!mshr_->getPendingRetries(line->getAddr())) {
      if (line->getShared()) {
        uint64_t sendTime =
            sendWriteback(line->getAddr(), lineSize_, Command::PutX,
                          line->getData(), true, line->getTimestamp());
        line->setTimestamp(sendTime - 1);
        mshr_->insertWriteback(line->getAddr(), true);
        if (is_debug_addr(addr) || is_debug_addr(line->getAddr()))
          diStruct.action = "Writeback";
        ...
        uint64_t sendTime =
            sendWriteback(line->getAddr(), lineSize_, Command::PutM,
                          line->getData(), true, line->getTimestamp());
        line->setTimestamp(sendTime - 1);
        mshr_->insertWriteback(line->getAddr(), false);
        if (is_debug_addr(addr) || is_debug_addr(line->getAddr())) {
  ...
  case S_Inv:
    if (!mshr_->getPendingRetries(line->getAddr())) {
      if (line->getShared()) { // This cache is not responsible for the block,
                               // drop it
        line->setState(I);
        if (is_debug_addr(addr) || is_debug_addr(line->getAddr())) {
          diStruct.action = "Drop";
  ...
  case E_InvX:
    if (!mshr_->getPendingRetries(line->getAddr())) {
      if (line->getOwned()) { // This cache is not responsible for the block,
                              // drop it
        line->setState(I);
        if (is_debug_addr(addr) || is_debug_addr(line->getAddr())) {
          diStruct.action = "Drop";
...
void MESIPrivNoninclusive::cleanUpAfterRequest(MemEvent *event, bool inMSHR) {
  ...
  if (mshr_->exists(addr)) {
    if (mshr_->getFrontType(addr) == MSHREntryType::Event) {
      if (!mshr_->getInProgress(addr) && !mshr_->getStalledForEvict(addr) &&
          mshr_->getAcksNeeded(addr) == 0) {
        retryBuffer_.push_back(mshr_->getFrontEvent(addr));
        mshr_->addPendingRetry(addr);
      }
    } else { // Pointer -> either we're waiting for a writeback ACK or another
             // address is waiting for this one
      if (mshr_->getFrontType(addr) == MSHREntryType::Evict &&
          mshr_->getAcksNeeded(addr) == 0) {
        std::list<Addr> *evictPointers = mshr_->getEvictPointers(addr);
        for (std::list<Addr>::iterator it = evictPointers->begin();
             it != evictPointers->end(); it++) {
...
uint64_t MESIPrivNoninclusive::sendExclusiveResponse(MemEvent *event,
                                                     vector<uint8_t> *data,
                                                     bool inMSHR, uint64_t time,
                                                     bool dirty) {
  ...
  //    if (is_debug_event(responseEvent)) {
  //        debug->debug(_L3_, "Sending response at cycle %" PRIu64 ". Current
  //        time = %" PRIu64 ". Response = (%s)\n",
  //                deliveryTime, timestamp_,
  //                responseEvent->getBriefString().c_str());
  //    }

  return deliveryTime;
}

uint64_t MESIPrivNoninclusive::sendResponseUp(MemEvent *event,
                                              vector<uint8_t> *data,
                                              bool inMSHR, uint64_t time,
                                              Command cmd, bool success) {
  MemEvent *responseEvent = event->makeResponse();
...
void MESIPrivNoninclusive::sendResponseDown(MemEvent *event, uint32_t size,
                                            vector<uint8_t> *data, bool dirty) {
  MemEvent *responseEvent = event->makeResponse();
  ...
  Response resp = {responseEvent, deliverTime,
                   packetHeaderBytes + responseEvent->getPayloadSize()};
  ...
  if (is_debug_event(responseEvent)) {
    ...
    //        debug->debug(_L3_, "Sending response at cycle %" PRIu64 ". Current
    //        time = %" PRIu64 ". Response = (%s)\n",
    //                deliverTime, timestamp_,
    //                responseEvent->getBriefString().c_str());
  }
}

uint64_t MESIPrivNoninclusive::forwardFlush(MemEvent *event, bool evict,
                                            std::vector<uint8_t> *data,
  ...
  Response resp = {flush, sendTime,
                   packetHeaderBytes + flush->getPayloadSize()};
  addToOutgoingQueue(resp);

  if (is_debug_addr(event->getBaseAddr())) {
    eventDI.action = "Forward";
    // debug->debug(_L3_,"Forwarding Flush at cycle = %" PRIu64 ", Cmd = %s, Src
    // = %s, %s\n", sendTime, CommandString[(int)flush->getCmd()],
    // flush->getSrc().c_str(), evict ? "with data" : "without data");
  }
  return sendTime;
}
 ...
 *  Latency: cache access + tag to read data that is being written back and
 * update coherence state
 */

uint64_t MESIPrivNoninclusive::sendWriteback(Addr addr, uint32_t size,
                                             Command cmd,
                                             std::vector<uint8_t> *data,
                                             bool dirty, uint64_t startTime) {
  ...
  sendTime += latency;
  Response resp = {writeback, sendTime,
                   packetHeaderBytes + writeback->getPayloadSize()};
  addToOutgoingQueue(resp);

  return sendTime;
  // if (is_debug_addr(addr))
  // debug->debug(_L3_,"Sending Writeback at cycle = %" PRIu64 ", Cmd = %s.
  // With%s data.\n", sendTime, CommandString[(int)cmd], ((cmd == Command::PutM
  // || writebackCleanBlocks_) ? "" : "out"));
}

uint64_t MESIPrivNoninclusive::sendFwdRequest(MemEvent *event, Command cmd,
                                              std::string dst, uint32_t size,
                                              uint64_t startTime, bool inMSHR) {
  Addr addr = event->getBaseAddr();
  ...
  responses.insert(std::make_pair(
      addr, req->getID())); // Record events we're waiting for to avoid trying
                            // to figure out what happened if we get a NACK

  uint64_t baseTime = timestamp_ > startTime ? timestamp_ : startTime;
  uint64_t deliveryTime =
      (inMSHR) ? baseTime + mshrLatency_ : baseTime + tagLatency_;
  Response resp = {req, deliveryTime, packetHeaderBytes};
  addToOutgoingQueueUp(resp);
  if (is_debug_addr(addr)) {
    eventDI.action = "Forward";
    //        debug->debug(_L7_, "Sending %s: Addr = 0x%" PRIx64 ", Dst = %s @
    //        cycles = %" PRIu64 ".\n",
...
/*----------------------------------------------------------------------------------------------------------------------
 *  Override message send functions with versions that record statistics & call
 *parent class
 *---------------------------------------------------------------------------------------------------------------------*/
...
MemEventInitCoherence *MESIPrivNoninclusive::getInitCoherenceEvent() {
  // Source, Endpoint type, inclusive, sends WB Acks, line size, tracks block
  // presence
  return new MemEventInitCoherence(cachename_, Endpoint::Cache, false, true,
                                   true, lineSize_, true);
}
...
void MESIPrivNoninclusive::recordLatency(Command cmd, int type,
                                         uint64_t latency) {
  if (type == -1)
    return;
