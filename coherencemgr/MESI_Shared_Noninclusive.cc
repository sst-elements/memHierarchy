 ...
 * - Unlike the private noninclusive protocol, this one supports prefetchers due
 *to the inclusive tag directory
 *---------------------------------------------------------------------------------------------------------------------*/
...
bool MESISharNoninclusive::handleGetS(MemEvent *event, bool inMSHR) {
    ...
    if (status == MemEventStatus::OK) {
      ...
      if (localPrefetch) { // Also need a data line
        if (processDataMiss(event, tag, data, true) != MemEventStatus::OK) {
          tag->setState(IA); // Don't let anyone steal this line, mark that
                             // we're waiting for a data line allocation
          break;
        } // Status is now inaccurate but it won't be Reject which is the only
          // thing we're checking for
      }
  ...
  case S:
    ...
    if (data || mshr_->hasData(addr)) {
      ...
      if (mshr_->hasData(addr))
        sendTime = sendResponseUp(event, &(mshr_->getData(addr)), inMSHR,
                                  tag->getTimestamp());
      else
        sendTime =
            sendResponseUp(event, data->getData(), inMSHR, tag->getTimestamp());
      ...
      if (status == MemEventStatus::OK) {
        ...
        sendTime =
            sendFetch(Command::Fetch, event, *(tag->getSharers()->begin()),
                      inMSHR, tag->getTimestamp());
        tag->setState(S_D);
        tag->setTimestamp(sendTime - 1);
        if (is_debug_event(event))
  ...
  case M:
    ...
    if (tag->hasOwner()) {
      ...
      if (status == MemEventStatus::OK) {
        sendTime = sendFetch(Command::FetchInvX, event, tag->getOwner(), inMSHR,
                             tag->getTimestamp());
      ...
      if (mshr_->hasData(addr))
        sendTime = sendResponseUp(event, &(mshr_->getData(addr)), inMSHR,
                                  tag->getTimestamp(), respcmd);
      else
        sendTime = sendResponseUp(event, data->getData(), inMSHR,
                                  tag->getTimestamp(), respcmd);
      tag->setTimestamp(sendTime - 1);
      cleanUpAfterRequest(event, inMSHR);
    } else {
...
bool MESISharNoninclusive::handleGetX(MemEvent *event, bool inMSHR) {
  ...
  case I:
    ...
    break;
  case S:
    if (!lastLevel_) { // If last level, upgrade silently by falling through to
                       // next case
      ...
      if (status == MemEventStatus::OK) {
        if (!mshr_->getProfiled(addr)) {
          ...
          stat_miss[(event->getCmd() == Command::GetX ? 1 : 2)][inMSHR]
              ->addData(1);
          notifyListenerOfAccess(event, NotifyAccessType::WRITE,
                                 NotifyResultType::MISS);
          mshr_->setProfiled(addr);
  ...
  case M:
    if (!tag->hasOtherSharers(event->getSrc()) && !tag->hasOwner()) {
      ...
      if (!inMSHR || !mshr_->getProfiled(addr)) {
        ...
        stat_hit[(event->getCmd() == Command::GetX ? 1 : 2)][inMSHR]->addData(
            1);
      }
      tag->setOwner(event->getSrc());
      if (tag->isSharer(event->getSrc())) {
        tag->removeSharer(event->getSrc());
        sendTime = sendResponseUp(event, nullptr, inMSHR, tag->getTimestamp(),
                                  Command::GetXResp);
      } else if (mshr_->hasData(addr))
        sendTime = sendResponseUp(event, &(mshr_->getData(addr)), inMSHR,
                                  tag->getTimestamp(), Command::GetXResp);
      else
        sendTime = sendResponseUp(event, data->getData(), inMSHR,
                                  tag->getTimestamp(), Command::GetXResp);
      tag->setTimestamp(sendTime - 1);
    ...
    if (status == MemEventStatus::OK) {
      ...
      if (tag->hasOtherSharers(event->getSrc())) {
        invalidateExceptRequestor(event, tag, inMSHR,
                                  !data && !tag->isSharer(event->getSrc()));
      } else {
  ...
  case M:
    if (status == MemEventStatus::OK) {
      ...
      if (event->getEvict()) {
        ...
        uint64_t sendTime =
            sendFetch(Command::FetchInvX, event, tag->getOwner(), inMSHR,
                      tag->getTimestamp());
        tag->setTimestamp(sendTime - 1);
        state == E ? tag->setState(E_InvX) : tag->setState(M_InvX);
        break;
      ...
      if (data)
        forwardFlush(event, true, data->getData(), tag->getState() == M,
                     tag->getTimestamp());
      else
        forwardFlush(event, true, &(mshr_->getData(addr)), tag->getState() == M,
                     tag->getTimestamp());
...
bool MESISharNoninclusive::handleFlushLineInv(MemEvent *event, bool inMSHR) {
  ...
  case S:
    if (status == MemEventStatus::OK) {
      ...
      if (tag->hasSharers()) {
        invalidateSharers(event, tag, inMSHR, !data && !mshr_->hasData(addr),
                          Command::Inv);
        tag->setState(S_Inv);
        break;
      }
      ...
      if (data)
        ...
        forwardFlush(event, true, &(mshr_->getData(addr)), false,
                     tag->getTimestamp());
      mshr_->setInProgress(addr);
      tag->setState(I_B);
    }
    ...
    if (status == MemEventStatus::OK) {
      ...
      if (tag->hasOwner()) {
        ...
        if (data)
          ...
          forwardFlush(event, true, &(mshr_->getData(addr)),
                       tag->getState() == M, tag->getTimestamp());
        mshr_->setInProgress(addr);
        tag->setState(I_B);
      }
    }
...
bool MESISharNoninclusive::handlePutS(MemEvent *event, bool inMSHR) {
      ...
      status =
          inMSHR
              ...
              : allocateMSHR(event, false,
                             1); // Put just after the Flush, will handle next
      break;
    }
    tag->removeSharer(event->getSrc());
    sendWritebackAck(event);
  ...
  case SB_D:
    if (event->getSrc() ==
        *(tag->getSharers()->begin())) { // Sent fetch to this requestor
      ...
      if (tag->numSharers() > 1) {
        ...
        if (inMSHR)
          mshr_->removeFront(
              addr); // Need to reinsert after the conflicting request
        entry = mshr_->getEntryEvent(addr, 1);
        if (entry && (CommandClassArr[(int)entry->getCmd()] ==
                          CommandClass::ForwardRequest ||
                      mshr_->getEntry(addr, 1).getInProgress()))
          status = allocateMSHR(
              event, false, 2); // Retry the waiting event and waiting forward
                                // request, then handle this replacement
        else
          status = allocateMSHR(
              event, false,
              1); // Retry the waiting event, then handle this replacement
      }
      break;
    }
    tag->removeSharer(event->getSrc());
  ...
  case M_InvX:
    ...
    if (data) {
      ...
      if (entry && (CommandClassArr[(int)entry->getCmd()] ==
                    ...
                    mshr_->getEntry(addr, 1).getInProgress()))
        status = allocateMSHR(event, false,
                              2); // Retry the waiting event and waiting forward
                                  // request, then handle this replacement
      else
        status = allocateMSHR(
            event, false,
...
bool MESISharNoninclusive::handlePutM(MemEvent *event, bool inMSHR) {
  ...
  case M_Inv:
    ...
    // Handle the coherence state part and buffer the data in the MSHR, we won't
    // need a line because we're either losing the data or one of our children
    // wants it
    tag->removeOwner();
    mshr_->decrementAcksNeeded(addr);
    if (!data && !mshr_->hasData(addr))
...
bool MESISharNoninclusive::handleFetch(MemEvent *event, bool inMSHR) {
  ...
  MemEvent *put;
  switch (state) {
  case I:   // Happens if we replaced and are waiting for a writeback ack or we
            // are replaying the event after an eviction -> drop the fetch
  ...
  case SA:
    ...
    put =
        static_cast<MemEvent *>(mshr_->getFirstEventEntry(addr, Command::PutS));
...
bool MESISharNoninclusive::handleInv(MemEvent *event, bool inMSHR) {
  ...
  case SA: // Merge with PutS and drop
    // TODO make sure the pending eviction won't mess anything up when it tries
    // to replay
...
bool MESISharNoninclusive::handleForceInv(MemEvent *event, bool inMSHR) {
  ...
  case M_Inv: // Evict, FlushlIneInv, Internal GetX
    if (!inMSHR) {
      ...
      } else { // In a race with GetX/GetSX, let the other event complete first
               // since it always can and this will avoid repeatedly losing the
               // block before the Get* can complete
        status = allocateMSHR(event, true, 1);
      }
    }
    break;
...
bool MESISharNoninclusive::handleFetchInvX(MemEvent *event, bool inMSHR) {
  ...
  case M:
    ...
    if (tag->hasOwner()) { // Get data from owner
      ...
      if (data)
        sendResponseDown(
            event, data->getData(), state == M,
            true); // TODO Double check that a downgrade counts as an evict
      else {
        sendResponseDown(event, &(mshr_->getData(addr)), state == M, true);
      }
      cleanUpAfterRequest(event, inMSHR);
  ...
  case MA:
    ...
    sendResponseDown(
        event, &(req->getPayload()), state == M,
        true); // TODO Double check that a downgrade counts as an evict
    // Clean up so that when we replay the replacement we get the right
    // downgraded state
...
bool MESISharNoninclusive::handleGetSResp(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_event(event))
    eventDI.prefill(event->getID(), Command::GetSResp, localPrefetch, addr,
                    state);
  ...
  if (localPrefetch) {
    ...
    uint64_t sendTime = sendResponseUp(req, &(event->getPayload()), true,
                                       tag->getTimestamp(), Command::GetSResp);
    tag->setTimestamp(sendTime - 1);
  }
...
bool MESISharNoninclusive::handleGetXResp(MemEvent *event, bool inMSHR) {
  ...
  MemEvent *req =
      static_cast<MemEvent *>(mshr_->getFrontEvent(event->getBaseAddr()));

  bool localPrefetch = req->isPrefetch() && (req->getRqstr() == cachename_);
  req->setFlags(event->getMemFlags());

  if (is_debug_event(event))
    eventDI.prefill(event->getID(), Command::GetXResp, localPrefetch, addr,
                    state);
  ...
  case IS: {
    ...
    if (localPrefetch) {
      ...
      if (tag->getState() == S || !protocol_ || mshr_->getSize(addr) > 1) {
        tag->addSharer(req->getSrc());
        uint64_t sendTime =
            sendResponseUp(req, &(event->getPayload()), true,
                           tag->getTimestamp(), Command::GetSResp);
        tag->setTimestamp(sendTime - 1);
      } else {
  ...
  case SM: {
    ...
    if (tag->isSharer(req->getSrc())) {
      ...
      sendTime = sendResponseUp(req, nullptr, true, tag->getTimestamp(),
                                Command::GetXResp);
    } else if (event->getPayloadSize() != 0) {
      sendTime = sendResponseUp(req, &(event->getPayload()), true,
                                tag->getTimestamp(), Command::GetXResp);
    } else {
      sendTime = sendResponseUp(req, &(mshr_->getData(addr)), true,
                                tag->getTimestamp(), Command::GetXResp);
    }
    tag->setTimestamp(sendTime - 1);
...
bool MESISharNoninclusive::handleFetchResp(MemEvent *event, bool inMSHR) {
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 "%s, Error: Received FetchResp in unhandled state '%s'. "
                 "Event: %s. Time = %" PRIu64 "ns\n",
                 getName().c_str(), StateString[state],
                 event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool MESISharNoninclusive::handleNULLCMD(MemEvent *event, bool inMSHR) {
  ...
  if (dirEvict) {
    ...
    if (is_debug_event(event)) {
      eventDI.prefill(event->getID(), Command::NULLCMD, false, oldAddr,
                      evictDI.oldst);
    }

    evicted = handleDirEviction(newAddr, tag);
    if (evicted) {
      notifyListenerOfEvict(tag->getAddr(), lineSize_,
      ...
      if (oldAddr != tag->getAddr()) { // Now evicting a different address than
                                       // we were before
        if (is_debug_event(event)) {
        ...
        if (mshr_->removeEvictPointer(oldAddr, newAddr))
          retry(oldAddr);
        evictionType_.erase(std::make_pair(oldAddr, newAddr));
        std::pair<Addr, Addr> evictpair =
            std::make_pair(tag->getAddr(), newAddr);
    ...
    // Data array eviction
    // Races can mean that this eviction is no longer neccessary but we can't
    // tell if we haven't started the eviction or if we're finishing. So finish
    // it, but only replace if we currently need an eviction (the event waiting
    // for eviction now and the one that triggered this eviction might not be
    // the same)
  } else {
    ...
    if (is_debug_event(event)) {
      eventDI.prefill(event->getID(), Command::NULLCMD, false, data->getAddr(),
                      evictDI.oldst);
    }
    if (evicted) {
      if (tag && (tag->getState() == IA || tag->getState() == SA ||
                  tag->getState() == EA || tag->getState() == MA)) {
      ...
      if (oldAddr != data->getAddr()) { // Now evicting a different address than
                                        // we were before
        ...
        evictionType_.erase(std::make_pair(oldAddr, newAddr));
        std::pair<Addr, Addr> evictpair =
            std::make_pair(data->getAddr(), newAddr);
        if (evictionType_.find(evictpair) == evictionType_.end()) {
...
bool MESISharNoninclusive::handleNACK(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_event(event) || is_debug_event(nackedEvent)) {
    DirectoryLine *tag = dirArray_->lookup(addr, false);
    eventDI.prefill(event->getID(), Command::NACK, false, addr,
                    tag ? tag->getState() : I);
  }
  ...
  default:
    debug->fatal(CALL_INFO, -1,
                 ...
                 "%s. Time = %" PRIu64 "ns\n",
                 getName().c_str(), nackedEvent->getVerboseString().c_str(),
 ...
 * Handle a cache miss
 */
MemEventStatus MESISharNoninclusive::processDirectoryMiss(MemEvent *event,
                                                          DirectoryLine *tag,
                                                          bool inMSHR) {
  MemEventStatus status =
      inMSHR ? MemEventStatus::OK
             : allocateMSHR(event, false); // Miss means we need an MSHR entry
...
MemEventStatus MESISharNoninclusive::processDataMiss(MemEvent *event,
                                                     DirectoryLine *tag,
                                                     DataLine *data,
                                                     bool inMSHR) {
  ...
  if (evicted) {
    ...
    std::pair<Addr, Addr> evictpair =
        std::make_pair(data->getAddr(), event->getBaseAddr());
    if (evictionType_.find(evictpair) == evictionType_.end()) {
 ...
 */
DirectoryLine *MESISharNoninclusive::allocateDirLine(MemEvent *event,
                                                     DirectoryLine *tag) {
  ...
  if (evicted) {
    ...
    std::pair<Addr, Addr> evictpair =
        std::make_pair(tag->getAddr(), event->getBaseAddr());
    if (evictionType_.find(evictpair) == evictionType_.end()) {
...
uint64_t MESISharNoninclusive::sendResponseUp(MemEvent *event,
                                              vector<uint8_t> *data,
...
void MESISharNoninclusive::sendResponseDown(MemEvent *event,
                                            std::vector<uint8_t> *data,
                                            bool dirty, bool evict) {
  MemEvent *responseEvent = event->makeResponse();
...
uint64_t MESISharNoninclusive::forwardFlush(MemEvent *event, bool evict,
                                            std::vector<uint8_t> *data,
 ...
 * update coherence state
 */
void MESISharNoninclusive::sendWritebackFromCache(Command cmd,
                                                  DirectoryLine *tag,
                                                  DataLine *data, bool dirty) {
  MemEvent *writeback =
      new MemEvent(cachename_, tag->getAddr(), tag->getAddr(), cmd);
  ...
  uint64_t baseTime =
      (timestamp_ > tag->getTimestamp()) ? timestamp_ : tag->getTimestamp();
  uint64_t deliveryTime = baseTime + latency;
...
void MESISharNoninclusive::sendWritebackFromMSHR(Command cmd,
                                                 DirectoryLine *tag,
                                                 bool dirty) {
  MemEvent *writeback =
      new MemEvent(cachename_, tag->getAddr(), tag->getAddr(), cmd);
...
uint64_t MESISharNoninclusive::sendFetch(Command cmd, MemEvent *event,
                                         std::string dst, bool inMSHR,
                                         uint64_t ts) {
  Addr addr = event->getBaseAddr();
  ...
  if (responses.find(addr) != responses.end()) {
    responses.find(addr)->second.insert(std::make_pair(
        dst,
        fetch->getID())); // Record events we're waiting for to avoid trying to
                          // figure out what happened if we get a NACK
  } else {
...
bool MESISharNoninclusive::invalidateExceptRequestor(MemEvent *event,
                                                     DirectoryLine *tag,
                                                     bool inMSHR,
                                                     bool needData) {
  ...
  for (set<std::string>::iterator it = tag->getSharers()->begin();
       it != tag->getSharers()->end(); it++) {
    if (*it == rqstr)
      continue;
    ...
    if (getData) { // FetchInv
      getData = false;
      deliveryTime =
          invalidateSharer(*it, event, tag, inMSHR, Command::FetchInv);
    } else { // Inv
...
bool MESISharNoninclusive::invalidateAll(MemEvent *event, DirectoryLine *tag,
                                         bool inMSHR, Command cmd) {
  uint64_t deliveryTime = 0;
  if (invalidateOwner(event, tag, inMSHR,
                      (cmd == Command::NULLCMD ? Command::FetchInv : cmd))) {
    return true;
  } else {
    if (cmd == Command::NULLCMD)
      cmd = Command::Inv;
    for (std::set<std::string>::iterator it = tag->getSharers()->begin();
         it != tag->getSharers()->end(); it++) {
...
void MESISharNoninclusive::invalidateSharers(MemEvent *event,
                                             DirectoryLine *tag, bool inMSHR,
...
uint64_t MESISharNoninclusive::invalidateSharer(std::string shr,
                                                MemEvent *event,
                                                DirectoryLine *tag, bool inMSHR,
                                                Command cmd) {
  if (tag->isSharer(shr)) {
    ...
    if (responses.find(addr) != responses.end()) {
      responses.find(addr)->second.insert(std::make_pair(
          shr,
          inv->getID())); // Record events we're waiting for to avoid trying to
                          // figure out what happened if we get a NACK
    } else {
      std::map<std::string, MemEvent::id_type> respid;
...
bool MESISharNoninclusive::invalidateOwner(MemEvent *metaEvent,
                                           DirectoryLine *tag, bool inMSHR,
                                           Command cmd) {
  Addr addr = tag->getAddr();
  if (tag->getOwner() == "")
  ...
  // Record events we're waiting for to avoid trying to figure out what happened
  // if we get a NACK
  if (responses.find(addr) != responses.end()) {
    responses.find(addr)->second.insert(
        std::make_pair(inv->getDst(), inv->getID()));
  } else {
    std::map<std::string, MemEvent::id_type> respid;
...
bool MESISharNoninclusive::applyPendingReplacement(Addr addr) {
  for (int i = mshr_->getSize(addr) - 1; i > 0; i--) {
    ...
    if (evb && CommandWriteback[(int)(evb->getCmd())]) {
      ...
      if (responses.find(addr) != responses.end()) {
        responses.find(addr)->second.insert(
            std::make_pair(evb->getSrc(), evb->getID()));
      } else {
        std::map<std::string, MemEvent::id_type> respid;
...
void MESISharNoninclusive::removeSharerViaInv(MemEvent *event,
                                              DirectoryLine *tag,
                                              DataLine *data, bool remove) {
  Addr addr = event->getBaseAddr();
  tag->removeSharer(event->getSrc());
  if (!data && !mshr_->hasData(addr))
    mshr_->setData(addr, event->getPayload());
...
void MESISharNoninclusive::removeOwnerViaInv(MemEvent *event,
                                             DirectoryLine *tag, DataLine *data,
                                             bool remove) {
  Addr addr = event->getBaseAddr();
  tag->removeOwner();
  if (data)
...
void MESISharNoninclusive::recordPrefetchResult(DirectoryLine *tag,
...
void MESISharNoninclusive::printLine(Addr addr) {
  ...
  debug->debug(_L8_, "  Line 0x%" PRIx64 ": %s Data Present: (%s)\n", addr,
               state.c_str(), data && data->getTag() ? "Y" : "N");
  if (data && data->getTag() && data->getTag() != tag) {
    if (data->getTag() != tag) {
      debug->fatal(CALL_INFO, -1,
                   "Error: Data has a tag but it does not match the tag found "
                   "in the directory. Addr 0x%" PRIx64 ". Data = (%" PRIu32
                   ",0x%" PRIx64 "), Tag = (%" PRIu32 ",0x%" PRIx64 ")\n",
                   addr, data->getIndex(), data->getAddr(), tag->getIndex(),
                   tag->getAddr());
    }
  }
}
...
void MESISharNoninclusive::recordLatency(Command cmd, int type,
                                         uint64_t latency) {
  if (type == -1)
    return;
