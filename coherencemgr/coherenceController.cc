...
CoherenceController::CoherenceController(ComponentId_t id, Params &params,
  ...
  /* Debug stream */
  debug = new Output(
      "", params.find<int>("debug_level", 1), 0,
  ...
  if (!found) {
    output->fatal(CALL_INFO, -1,
                  "%s, Param not specified: access_latency_cycles - this is "
                  "the access time in cycles for the cache; if tag_latency is "
                  "also specified, this is the data array access time\n",
                  getName().c_str());
  }

  tagLatency_ =
      params.find<uint64_t>("tag_access_latency_cycles", accessLatency_);
  mshrLatency_ = params.find<uint64_t>(
      "mshr_latency_cycles",
  ...
  UnitAlgebra downLinkBW =
      UnitAlgebra(params.find<std::string>("request_link_width", "0B"));
  UnitAlgebra upLinkBW =
      UnitAlgebra(params.find<std::string>("response_link_width", "0B"));

  if (!packetSize.hasUnits("B"))
    output->fatal(CALL_INFO, -1,
                  "%s, Invalid param: min_packet_size - must have units of "
                  "bytes (B), SI units OK. Ex: '8B'. You specified '%s'\n",
                  getName().c_str(), packetSize.toString().c_str());
  if (!downLinkBW.hasUnits("B"))
    output->fatal(CALL_INFO, -1,
                  "%s, Invalid param: request_link_width - must have units of "
                  "bytes (B), SI units OK. Ex: '64B'. You specified '%s'\n",
                  getName().c_str(), downLinkBW.toString().c_str());
  if (!upLinkBW.hasUnits("B"))
    output->fatal(CALL_INFO, -1,
                  "%s, Invalid param: response_link_width - must have units of "
                  "bytes (B), SI units OK. Ex: '64B'. You specified '%s'\n",
  ...
  // The following cache parameters are set by the cache controller in its
  // constructor Just in case, we give an initial value here
  dropPrefetchLevel_ = ((size_t)-1);
  maxOutstandingPrefetch_ = ((size_t)-2);
  cachename_ = getName().c_str();

  // Register statistics - only those that are common across all coherence
  // managers Give  all array entries a default statistic so we don't end up
  // with segfaults during execution
 ...
 * Initialization
 *******************************************************************************/
ReplacementPolicy *CoherenceController::createReplacementPolicy(
    uint64_t lines, uint64_t assoc, Params &params, bool L1, int slotnum) {
  SubComponentSlotInfo *rslots = getSubComponentSlotInfo("replacement");
  if (rslots && rslots->isPopulated(slotnum))
    return rslots->create<ReplacementPolicy>(0, ComponentInfo::SHARE_NONE,
                                             lines, assoc);

  // Default to the replacement policy that was used before all the recent memH
  // changes
  Params emptyparams;
  ...
  if (policy == "lru") {
    if (L1)
      return loadAnonymousSubComponent<ReplacementPolicy>(
          "memHierarchy.replacement.lru", "replacement", slotnum,
          ComponentInfo::SHARE_NONE, emptyparams, lines, assoc);
    else
      return loadAnonymousSubComponent<ReplacementPolicy>(
          "memHierarchy.replacement.lru-opt", "replacement", slotnum,
          ComponentInfo::SHARE_NONE, emptyparams, lines, assoc);
  }
  if (policy == "lfu") {
    if (L1)
      return loadAnonymousSubComponent<ReplacementPolicy>(
          "memHierarchy.replacement.lfu", "replacement", slotnum,
          ComponentInfo::SHARE_NONE, emptyparams, lines, assoc);
    else
      return loadAnonymousSubComponent<ReplacementPolicy>(
          "memHierarchy.replacement.lfu-opt", "replacement", slotnum,
          ComponentInfo::SHARE_NONE, emptyparams, lines, assoc);
  }
  if (policy == "mru") {
    if (L1)
      return loadAnonymousSubComponent<ReplacementPolicy>(
          "memHierarchy.replacement.mru", "replacement", slotnum,
          ComponentInfo::SHARE_NONE, emptyparams, lines, assoc);
    else
      return loadAnonymousSubComponent<ReplacementPolicy>(
          "memHierarchy.replacement.mru-opt", "replacement", slotnum,
          ComponentInfo::SHARE_NONE, emptyparams, lines, assoc);
  }
  if (policy == "random")
    return loadAnonymousSubComponent<ReplacementPolicy>(
        "memHierarchy.replacement.random", "replacement", slotnum,
        ComponentInfo::SHARE_NONE, emptyparams, lines, assoc);
  if (policy == "nmru")
    return loadAnonymousSubComponent<ReplacementPolicy>(
        "memHierarchy.replacement.nmru", "replacement", slotnum,
        ComponentInfo::SHARE_NONE, emptyparams, lines, assoc);

  debug->fatal(
      CALL_INFO, -1,
      "%s, Invalid param: replacement_policy - supported policies are 'lru', "
      "'lfu', 'random', 'mru', and 'nmru'. You specified '%s'.\n",
...
HashFunction *CoherenceController::createHashFunction(Params &params) {
  ...
  if (hashFunc == 1)
    ht = loadAnonymousSubComponent<HashFunction>(
        "memHierarchy.hash.linear", "hash", 0, ComponentInfo::SHARE_NONE,
        hparams);
  if (hashFunc == 2)
    ht = loadAnonymousSubComponent<HashFunction>(
        "memHierarchy.hash.xor", "hash", 0, ComponentInfo::SHARE_NONE, hparams);
  else
    ht = loadAnonymousSubComponent<HashFunction>(
        "memHierarchy.hash.none", "hash", 0, ComponentInfo::SHARE_NONE,
        hparams);
...
bool CoherenceController::handleGetS(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: GetS events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
...
bool CoherenceController::handleGetX(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: GetX events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleGetSX(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: GetSX events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleFlushLine(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: FlushLine events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleFlushLineInv(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: FlushLineInv events are not handled by this "
               "coherence manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
}
...
bool CoherenceController::handlePutS(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: PutS events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handlePutX(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: PutX events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handlePutE(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: PutE events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handlePutM(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: PutM events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleGetSResp(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: GetSResp events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleGetXResp(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: GetXResp events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleFlushLineResp(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: FlushLineResp events are not handled by this "
               "coherence manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleAckPut(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: AckPut events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleFetch(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: Fetch events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleFetchInv(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: FetchInv events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleFetchInvX(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: FetchInvX events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleForceInv(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: ForceInv events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleInv(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: Inv events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleFetchResp(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: FetchResp events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleFetchXResp(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: FetchXResp events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleAckInv(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: AckInv events are not handled by this coherence "
               "manager. Event: %s\n. Time: %" PRIu64 "ns.",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleNULLCMD(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: NULLCMD events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
bool CoherenceController::handleNACK(MemEvent *event, bool inMSHR) {
  debug->fatal(CALL_INFO, -1,
               "%s, Error: NACK events are not handled by this coherence "
               "manager. Event: %s. Time: %" PRIu64 "ns.\n",
               getName().c_str(), event->getVerboseString().c_str(),
               getCurrentSimTimeNano());
  return false;
...
/* Send commands when their timestampe expires. Return whether queue is empty or
 * not. */
bool CoherenceController::sendOutgoingEvents() {
  ...
  while (!outgoingEventQueue_.empty() &&
         outgoingEventQueue_.front().deliveryTime <= timestamp_) {
    ...
    outgoingEvent->setDst(
        linkDown_->findTargetDestination(outgoingEvent->getRoutingAddress()));
    ...
    if (is_debug_event(outgoingEvent)) {
      debug->debug(
          ...
          cachename_.c_str(), outgoingEvent->getBriefString().c_str());
    }
  ...
  while (!outgoingEventQueueUp_.empty() &&
    ...
    if (startTimes_.find(outgoingEvent->getResponseToID()) !=
        startTimes_.end()) {
      LatencyStat stat =
          startTimes_.find(outgoingEvent->getResponseToID())->second;
  ...
  // Return whether it's ok for the cache to turn off the clock - we need it on
  // to be able to send waiting events
...
uint64_t CoherenceController::forwardTowardsMem(MemEventBase *event) {
  ...
  Response fwdReq = {event, timestamp_ + 1,
                     packetHeaderBytes + event->getPayloadSize()};
  addToOutgoingQueue(fwdReq);
  return timestamp_ + 1;
}

/* Forward an event towards processor. Return expected send time. */
uint64_t CoherenceController::forwardTowardsCPU(MemEventBase *event,
                                                std::string dst) {
  event->setSrc(cachename_);
  event->setDst(dst);
...
void CoherenceController::processInitCoherenceEvent(
    MemEventInitCoherence *event, bool source) {
  // If something besides memory is below us, we are not the last level doing
  // coherence
  if (event->getType() != Endpoint::Memory)
    lastLevel_ = false;

  // If the component below us tracks whether a block is present above, then we
  // can't silently evict (per current protocols)
  if (event->getTracksPresence() || lastLevel_)
    silentEvictClean_ = false;

  // The component below us is noninclusive, therefore we need to write back
  // data when evicting clean blocks
  ...
  debug->debug(_L3_,
               "%s processInitCoherenceEvent. Result is: LL ? %s, silentEvict "
               "? %s, WBClean ? %s, sendWBAck ? %s, recvWBAck ? %s\n",
               cachename_.c_str(), lastLevel_ ? "Y" : "N",
               silentEvictClean_ ? "Y" : "N", writebackCleanBlocks_ ? "Y" : "N",
               sendWritebackAck_ ? "Y" : "N", recvWritebackAck_ ? "Y" : "N");
}
...
void CoherenceController::clearRetryBuffer() { retryBuffer_.clear(); }

/* Listener callbacks */
void CoherenceController::notifyListenerOfAccess(MemEvent *event,
  ...
  CacheListenerNotification notify(
      event->getAddr(), event->getBaseAddr(), event->getVirtualAddress(),
...
void CoherenceController::notifyListenerOfEvict(Addr addr, uint32_t size,
                                                Addr ip) {
...
uint64_t CoherenceController::forwardMessage(MemEvent *event,
  ...
  if (data == nullptr)
    forwardEvent->setPayload(0, nullptr);

  forwardEvent->setSrc(cachename_);
  forwardEvent->setDst(
      linkDown_->findTargetDestination(event->getRoutingAddress()));
  forwardEvent->setSize(requestSize);

  if (data != nullptr)
    forwardEvent->setPayload(*data);

  /* Determine latency in cycles */
  uint64_t deliveryTime;
  if (baseTime < timestamp_)
    baseTime = timestamp_;
  if (event->queryFlag(MemEvent::F_NONCACHEABLE)) {
    ...
    deliveryTime = baseTime + tagLatency_;
...
void CoherenceController::sendNACK(MemEvent *event) {
  ...
  uint64_t deliveryTime =
      timestamp_ +
      tagLatency_; // Probably had to lookup and see that we couldn't handle
                   // this request and/or MSHR was full
...
/* Send response up (towards CPU). L1s need to implement their own to split out
 * the requested block */
uint64_t CoherenceController::sendResponseUp(MemEvent *event,
                                             vector<uint8_t> *data, bool replay,
                                             uint64_t baseTime, bool atomic) {
  return sendResponseUp(event, CommandResponse[(int)event->getCmd()], data,
                        false, replay, baseTime, atomic);
}

/* Send response up (towards CPU). L1s need to implement their own to split out
 * the requested block */
uint64_t CoherenceController::sendResponseUp(MemEvent *event, Command cmd,
                                             vector<uint8_t> *data, bool replay,
                                             uint64_t baseTime, bool atomic) {
  return sendResponseUp(event, cmd, data, false, replay, baseTime, atomic);
}

/* Send response towards the CPU. L1s need to implement their own to split out
 * the requested block */
uint64_t CoherenceController::sendResponseUp(MemEvent *event, Command cmd,
...
MemEventStatus CoherenceController::allocateMSHR(MemEvent *event, bool fwdReq,
                                                 int pos, bool stallEvict) {
  ...
  int end_pos =
      mshr_->insertEvent(event->getBaseAddr(), event, pos, fwdReq, stallEvict);
  if (end_pos == -1) {
    if (is_debug_event(event)) {
      eventDI.action = "Reject";
...
// Format:
// TIME     CYCLE       COMPNAME    ID  ADDR    CMD OLDSTATE    NEWSTATE ACTION
// (REASON)    VERBOSE_LINE_STATE
//
// Prints at debug level 5
// VERBOSE_LINE_STATE is only printed at debug level 6
void CoherenceController::printDebugInfo() { printDebugInfo(&eventDI); }
...
void CoherenceController::printDebugInfo(dbgin *diStruct) {
  ...
  debug->debug(_L5_,
               ...
               " %-15s %-6s %-6s %-10s %-15s",
               Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
               cachename_.c_str(), cmd.c_str(), diStruct->addr,
               id.str().c_str(), StateString[diStruct->oldst],
               StateString[diStruct->newst], diStruct->action.c_str(),
               reas.str().c_str());
...
void CoherenceController::printDebugAlloc(bool alloc, Addr addr,
                                          std::string note) {
  if (dlevel < 5)
    return;
  ...
  debug->debug(_L5_,
               "C: %-20" PRIu64 " %-20" PRIu64 " %-20s %-13s 0x%-16" PRIx64 "",
               Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
               cachename_.c_str(), action.c_str(), addr);
...
void CoherenceController::printStatus(Output &out) {
  ...
  for (list<Response>::iterator it = outgoingEventQueue_.begin();
    ...
    out.output("      Time: %" PRIu64 ", Event: %s\n", (*it).deliveryTime,
               (*it).event->getVerboseString().c_str());
  }
...
void CoherenceController::addToOutgoingQueue(Response &resp) {
  ...
  for (rit = outgoingEventQueue_.rbegin(); rit != outgoingEventQueue_.rend();
       rit++) {
    if (resp.deliveryTime >= (*rit).deliveryTime)
      break;
    if (resp.event->getRoutingAddress() == (*rit).event->getRoutingAddress())
      break;
  }
...
void CoherenceController::addToOutgoingQueueUp(Response &resp) {
  list<Response>::reverse_iterator rit;
  for (rit = outgoingEventQueueUp_.rbegin();
       rit != outgoingEventQueueUp_.rend(); rit++) {
    if (resp.deliveryTime >= (*rit).deliveryTime)
      break;
    if (resp.event->getRoutingAddress() == (*rit).event->getRoutingAddress())
      break;
