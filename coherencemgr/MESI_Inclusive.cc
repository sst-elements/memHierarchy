...
bool MESIInclusive::handleGetS(MemEvent *event, bool inMSHR) {
  ...
  case I:
    ...
    if (status ==
        MemEventStatus::OK) { // Both MSHR insert and cache line allocation
                              // succeeded and there's no MSHR conflict
  ...
  case M:
    ...
    if (localPrefetch) {
      if (!inMSHR || !mshr_->getProfiled(addr)) {
        ...
        stat_hit[0][inMSHR]->addData(1);
        notifyListenerOfAccess(event, NotifyAccessType::PREFETCH,
                               NotifyResultType::HIT);
        statPrefetchRedundant->addData(1);
        recordPrefetchLatency(event->getID(), LatType::HIT);
      }
...
bool MESIInclusive::handleGetX(MemEvent *event, bool inMSHR) {
  ...
  case I:
    ...
    if (status == MemEventStatus::OK) {
      ...
      line->setState(IM);
      line->setTimestamp(sendTime);
      mshr_->setInProgress(addr); // Keeps us from retrying too early due to
                                  // certain race conditions between events
      if (is_debug_event(event))
        eventDI.reason = "miss";
    }
    break;
  case S:
    if (!lastLevel_) { // Otherwise can silently upgrade to M by falling through
                       // to next case
...
bool MESIInclusive::handleFlushLine(MemEvent *event, bool inMSHR) {
  ...
  case M_Inv:
    if (mshr_->getFrontType(addr) == MSHREntryType::Event) {
      ...
      if (headEvent->getCmd() == Command::FetchInvX &&
          !line->hasOwner()) { // Resolve race between downgrade request & this
                               // flush
...
bool MESIInclusive::handleForceInv(MemEvent *event, bool inMSHR) {
  ...
  case M_InvX: // in mshr & stall
    if (mshr_->getFrontType(addr) == MSHREntryType::Event &&
        (mshr_->getFrontEvent(addr)->getCmd() == Command::GetX ||
         mshr_->getFrontEvent(addr)->getCmd() == Command::GetS ||
         mshr_->getFrontEvent(addr)->getCmd() == Command::GetSX)) {
  ...
  case SM_Inv: { // ForceInv if there's an un-inv'd sharer, else in mshr & stall
    ...
    if (status != MemEventStatus::Reject) {
      ...
      if (line->isSharer(src)) {
        line->setTimestamp(
            invalidateSharer(src, event, line, inMSHR, Command::ForceInv));
      }
      status = MemEventStatus::Stall;
    }
    break;
...
bool MESIInclusive::handleGetSResp(MemEvent *event, bool inMSHR) {
  ...
  if (localPrefetch) {
    ...
    uint64_t sendTime =
        sendResponseUp(req, line->getData(), true, line->getTimestamp());
    line->setTimestamp(sendTime - 1);
  }
...
bool MESIInclusive::handleNULLCMD(MemEvent *event, bool inMSHR) {
  ...
  if (is_debug_addr(newAddr)) {
    eventDI.prefill(event->getID(), Command::NULLCMD, false, line->getAddr(),
                    evictDI.oldst);
    eventDI.newst = line->getState();
    eventDI.verboseline = line->getString();
  }
...
MemEventStatus MESIInclusive::processCacheMiss(MemEvent *event,
  ...
  if (inMSHR &&
      mshr_->getFrontEvent(event->getBaseAddr()) !=
          event) { // Sometimes happens if eviction and event both retry
    if (is_debug_event(event))
      eventDI.action = "Stall";
...
SharedCacheLine *MESIInclusive::allocateLine(MemEvent *event,
                                             SharedCacheLine *line) {
...
void MESIInclusive::retry(Addr addr) {
  if (mshr_->exists(addr)) {
    if (mshr_->getFrontType(addr) == MSHREntryType::Event) {
      // if (is_debug_addr(addr))
      //    debug->debug(_L5_, "    Retry: Waiting Event exists in MSHR and is
      //    ready to retry\n");
      retryBuffer_.push_back(mshr_->getFrontEvent(addr));
      mshr_->addPendingRetry(addr);
    } else if (!(mshr_->pendingWriteback(addr))) {
      // if (is_debug_addr(addr))
      //    debug->debug(_L5_, "    Retry: Waiting Evict in MSHR, retrying
      //    eviction\n");
      std::list<Addr> *evictPointers = mshr_->getEvictPointers(addr);
      for (std::list<Addr>::iterator it = evictPointers->begin();
           it != evictPointers->end(); it++) {
...
State MESIInclusive::doEviction(MemEvent *event, SharedCacheLine *line,
                                State state) {
  State nState = state;
  recordPrefetchResult(line, statPrefetchEvict);
  ...
  event->setEvict(
      false); // Avoid doing an eviction twice if the event gets replayed
...
SimTime_t MESIInclusive::sendResponseUp(MemEvent *event, vector<uint8_t> *data,
                                        bool inMSHR, uint64_t time, Command cmd,
                                        bool success) {
...
void MESIInclusive::sendResponseDown(MemEvent *event, SharedCacheLine *line,
                                     bool data, bool evict) {
  MemEvent *responseEvent = event->makeResponse();
...
void MESIInclusive::forwardFlush(MemEvent *event, SharedCacheLine *line,
                                 bool evict) {
  ...
  if (line && line->getTimestamp() > baseTime)
    baseTime = line->getTimestamp();
  ...
  if (is_debug_addr(event->getBaseAddr())) {
    ...
    //    debug->debug(_L3_,"Forwarding Flush at cycle = %" PRIu64 ", Cmd = %s,
    //    Src = %s, %s\n", deliveryTime, CommandString[(int)flush->getCmd()],
 ...
 */
void MESIInclusive::sendWriteback(Command cmd, SharedCacheLine *line,
                                  bool dirty) {
  MemEvent *writeback =
      new MemEvent(cachename_, line->getAddr(), line->getAddr(), cmd);
  ...
  uint64_t baseTime =
      (timestamp_ > line->getTimestamp()) ? timestamp_ : line->getTimestamp();
  uint64_t deliveryTime = baseTime + latency;
  ...
  // With%s data.\n", deliveryTime, CommandString[(int)cmd], ((cmd ==
  // Command::PutM || writebackCleanBlocks_) ? "" : "out"));
}
...
void MESIInclusive::downgradeOwner(MemEvent *event, SharedCacheLine *line,
                                   bool inMSHR) {
  Addr addr = event->getBaseAddr();
  ...
  if (is_debug_addr(line->getAddr())) {
    ...
    // debug->debug(_L7_, "Sending %s: Addr = 0x%" PRIx64 ", Dst = %s @ cycles =
    // %" PRIu64 ".\n",
    //        CommandString[(int)cmd], line->getAddr(),
    //        line->getOwner().c_str(), deliveryTime);
  }
}

bool MESIInclusive::invalidateExceptRequestor(MemEvent *event,
                                              SharedCacheLine *line,
                                              bool inMSHR) {
  uint64_t deliveryTime = 0;
  std::string rqstr = event->getSrc();

  for (set<std::string>::iterator it = line->getSharers()->begin();
       it != line->getSharers()->end(); it++) {
    if (*it == rqstr)
      continue;
...
bool MESIInclusive::invalidateAll(MemEvent *event, SharedCacheLine *line,
                                  bool inMSHR, Command cmd) {
  uint64_t deliveryTime = 0;
  if (invalidateOwner(event, line, inMSHR,
                      (cmd == Command::NULLCMD ? Command::FetchInv : cmd))) {
    return true;
  } else {
    if (cmd == Command::NULLCMD)
      cmd = Command::Inv;
    for (std::set<std::string>::iterator it = line->getSharers()->begin();
         it != line->getSharers()->end(); it++) {
...
uint64_t MESIInclusive::invalidateSharer(std::string shr, MemEvent *event,
...
bool MESIInclusive::invalidateOwner(MemEvent *event, SharedCacheLine *line,
                                    bool inMSHR, Command cmd) {
  Addr addr = line->getAddr();
  if (line->getOwner() == "")
    return false;
...
void MESIInclusive::recordPrefetchResult(SharedCacheLine *line,
...
MemEventInitCoherence *MESIInclusive::getInitCoherenceEvent() {
  return new MemEventInitCoherence(
      cachename_, Endpoint::Cache, true /* inclusive */,
      false /* sends WBAck */, false /* expects WBAck */, lineSize_,
      true /* tracks block presence */);
}
