...
bool Incoherent::handleFlushLineInv(MemEvent *event, bool inMSHR) {
  ...
  case M:
    if (status == MemEventStatus::OK) {
      ...
      forwardFlush(event, true, line->getData(), state == M,
                   line->getTimestamp());
      line->setState(I_B);
      mshr_->setInProgress(addr);
    }
...
bool Incoherent::handleNACK(MemEvent *event, bool inMSHR) {
  ...
  resendEvent(
      nackedEvent,
      false); // Re-send since we don't have races that make events redundant
...
MemEventStatus Incoherent::processCacheMiss(MemEvent *event,
...
MemEventStatus Incoherent::allocateLine(MemEvent *event,
                                        PrivateCacheLine *&line, bool inMSHR) {
...
bool Incoherent::handleEviction(Addr addr, PrivateCacheLine *&line,
                                dbgin &diStruct) {
  if (!line)
...
SimTime_t Incoherent::sendResponseUp(MemEvent *event, vector<uint8_t> *data,
                                     bool inMSHR, SimTime_t time, Command cmd,
                                     bool success) {
...
void Incoherent::sendWriteback(Command cmd, PrivateCacheLine *line,
                               bool dirty) {
  MemEvent *writeback =
  ...
  time += latency;
  Response resp = {writeback, time,
                   packetHeaderBytes + writeback->getPayloadSize()};
  addToOutgoingQueue(resp);
  line->setTimestamp(time - 1);
}

void Incoherent::forwardFlush(MemEvent *event, bool evict,
...
MemEventInitCoherence *Incoherent::getInitCoherenceEvent() {
  return new MemEventInitCoherence(cachename_, Endpoint::Cache, false, false,
                                   false, lineSize_, true);
}

void Incoherent::recordPrefetchResult(PrivateCacheLine *line,
