...
const MemEvent::id_type MESIDirectory::DirEntry::NO_LAST_REQUEST =
    std::make_pair((uint64_t)-1, -1);
...
MESIDirectory::MESIDirectory(ComponentId_t id, Params &params)
  ...
  if (mshrSize < 1)
    dbg.fatal(CALL_INFO, -1,
              ...
              "else -1 to indicate a very large MSHR\n",
              getName().c_str());
  ...
  stat_GetSXReqReceived =
      registerStatistic<uint64_t>("requests_received_GetSX");
  ...
  stat_NACKRespReceived =
      registerStatistic<uint64_t>("responses_received_NACK");
  stat_FetchRespReceived =
      registerStatistic<uint64_t>("responses_received_FetchResp");
  stat_FetchXRespReceived =
      registerStatistic<uint64_t>("responses_received_FetchXResp");
  stat_PutMRespReceived =
      registerStatistic<uint64_t>("responses_received_PutM");
  stat_PutERespReceived =
      registerStatistic<uint64_t>("responses_received_PutE");
  stat_PutSRespReceived =
      registerStatistic<uint64_t>("responses_received_PutS");
  ...
  stat_dirEntryReads =
      registerStatistic<uint64_t>("memory_requests_directory_entry_read");
  stat_dirEntryWrites =
      registerStatistic<uint64_t>("memory_requests_directory_entry_write");
 ...
 */
inline void MESIDirectory::profileRequestRecv(MemEvent *event,
                                              DirEntry *entry) {
...
void MESIDirectory::handleGetS(MemEvent *event, bool replay) {
  ...
  /* Put request in MSHR (all GetS requests need to be buffered while they wait
   * for data) and stall if there are waiting requests ahead of us */
  ...
  if (!entry->isCached()) {
    ...
    if (is_debug_addr(entry->getBaseAddr()))
      dbg.debug(_L6_,
                "Entry %" PRIx64 " not in cache.  Requesting from memory.\n",
                entry->getBaseAddr());
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "Directory %s received GetS but state is %s. Event: %s\n",
...
void MESIDirectory::handleGetX(MemEvent *ev, bool replay) {
  ...
  case S:
    if (entry->getSharerCount() == 1 &&
      ...
      if (is_debug_event(ev)) {
        dbg.debug(_L4_,
                  "Sending response for 0x%" PRIx64
                  " to %s, send time: %" PRIu64 "\n",
                  entry->getBaseAddr(), respEv->getDst().c_str(),
                  timestamp + accessLatency);
      }
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "Directory %s received %s but state is %s. Event: %s\n",
              getName().c_str(), CommandString[(int)ev->getCmd()],
              StateString[state], ev->getVerboseString().c_str());
  }
}
...
void MESIDirectory::handlePutS(MemEvent *ev) {
  ...
  case SD_Inv:
    ...
    if (entry->getWaitingAcks() == 0) {
      if (state == S_Inv)
        ...
        (entry->getSharerCount() > 0) ? entry->setState(S_D)
                                      : entry->setState(IS);
      replayWaitingEvents(addr);
    }
    break;
  default:
    dbg.fatal(CALL_INFO, -1,
              ...
              "Time = %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              ev->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
}
...
void MESIDirectory::handlePutE(MemEvent *ev) {
  ...
  if (!((uint32_t)entry->getOwner() == node_name_to_id(ev->getSrc()))) {
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: received PutE from a node who does not own the "
              "block. Event = %s. Time = %" PRIu64 "ns\n",
  ...
  case M:
    ...
    break;
  case M_Inv: /* If PutE comes with data then we can handle this immediately but
                 otherwise */
    entry->setState(IM);
    issueMemoryRequest(
        static_cast<MemEvent *>(mshr->lookupFront(ev->getBaseAddr())), entry);
...
void MESIDirectory::handlePutM(MemEvent *ev) {
  ...
  if (!((uint32_t)entry->getOwner() == node_name_to_id(ev->getSrc()))) {
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: received PutM from a node who does not own the "
              "block. Event: %s. Time = %" PRIu64 "ns\n",
              getName().c_str(), ev->getVerboseString().c_str(),
              getCurrentSimTimeNano());
  }
  ...
  if (!(entry->isCached())) {
    if (!(mshr->elementIsHit(ev->getBaseAddr(), ev))) {
      ...
      if (is_debug_event(ev)) {
        dbg.debug(_L8_,
                  "Inserting request in mshr. Cmd = %s, BaseAddr = 0x%" PRIx64
                  ", Addr = 0x%" PRIx64 ", MSHR size: %d\n",
                  CommandString[(int)ev->getCmd()], ev->getBaseAddr(),
                  ev->getAddr(), mshr->getSize());
      }
...
void MESIDirectory::handleFlushLine(MemEvent *ev) {
  ...
  case M_Inv: // Owner in progress, waiting on FetchInvResp
    // Assume that the sender will treat our outstanding FetchInv as an Inv and
  ...
  case M_InvX: // Sharer in progress, waiting on FetchInvXResp
    ...
    if (entry->getOwner() == srcID) {
      ...
      replayWaitingEvents(entry->getBaseAddr());
      updateCache(entry);
    } else if (!inMSHR && !mshr->insert(ev->getBaseAddr(), ev))
      mshrNACKRequest(ev);
    break;
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Directory received FlushLine but state is %s. Event: "
              "%s. Time = %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              ev->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
void MESIDirectory::handleFlushLineInv(MemEvent *ev) {
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Directory received FlushLineInv but state is %s. "
...
void MESIDirectory::handleFetchResp(MemEvent *ev, bool keepEvent) {
  ...
  if (!((uint32_t)entry->getOwner() == node_name_to_id(ev->getSrc()))) {
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: received FetchResp from a node who does not own the "
              "block. Event: %s. Time = %" PRIu64 "ns, %" PRIu64 " cycles\n",
              getName().c_str(), ev->getVerboseString().c_str(),
              getCurrentSimTimeNano(), timestamp);
  }
  ...
  case M_Inv:
    if (reqEv->getCmd() != Command::FetchInv &&
        reqEv->getCmd() !=
            Command::ForceInv) { // GetX request, not back invalidation
      writebackData(ev, Command::PutM);
  ...
  case M_InvX: // GetS request
    if (reqEv->getCmd() == Command::FetchInv ||
        reqEv->getCmd() == Command::ForceInv) {
      // TODO should probably just handle the Get* that caused a FetchInvX first
      // and then do the FetchInv/ForceInv Actually should only do
      // FetchInv/ForceInv first if it required a request to memory (IS or IM)
      // -> everything else should have the FetchInv/ForceInv go second
    }
    writebackData(ev, Command::PutM);
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Directory received %s but state is %s. Event: %s. "
              "Time = %" PRIu64 "ns, %" PRIu64 " cycles\n",
              getName().c_str(), CommandString[(int)ev->getCmd()],
              StateString[state], ev->getVerboseString().c_str(),
              getCurrentSimTimeNano(), timestamp);
...
void MESIDirectory::handleAckInv(MemEvent *ev) {
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Directory received AckInv but state is %s. Event = "
              "%s. Time = %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              ev->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
void MESIDirectory::handleBackInv(MemEvent *ev) {
  ...
  if (!(mshr->elementIsHit(ev->getBaseAddr(), ev))) {
    ...
    if (conflict) {
      if (mshr->exists(ev->getBaseAddr())) { // front entry is an event
        /* If state is IS, IM, or S_D, we are waiting on memory so resolve this
         * Inv first to avoid deadlock */
        if (entry->getState() == IS || entry->getState() == IM ||
            entry->getState() == S_D) {
          inserted =
              mshr->insertInv(ev->getBaseAddr(), ev, false); // insert front
          conflict = false;                                  // don't stall
          noreplay = true; // don't replay second event if this one completes
        } else {
          inserted =
              mshr->insertInv(ev->getBaseAddr(), ev, true); // insert second
        }
      } else {
        inserted =
            mshr->insertInv(ev->getBaseAddr(), ev, true); // insert second
    ...
    if (conflict)
      return; // stall until conflicting request finishes
    else
      profileRequestRecv(ev, entry);
  }
  ...
  case S:
    ...
    issueInvalidates(ev, entry,
                     Command::Inv); // Fwd inv for forceInv and fetchInv
    break;
  case M:
    entry->setState(M_Inv);
...
void MESIDirectory::handleNACK(MemEvent *ev) {
  ...
  if (is_debug_event(ev)) {
    dbg.debug(_L5_,
              "Orig resp ID = (%" PRIu64 ",%d), Nack resp ID = (%" PRIu64
              ",%d), last req ID = (%" PRIu64 ",%d)\n",
              origEvent->getResponseToID().first,
              origEvent->getResponseToID().second, ev->getResponseToID().first,
              ev->getResponseToID().second, entry->lastRequest.first,
              entry->lastRequest.second);
  }

  /* Retry request if it has not already been handled */
  if ((ev->getResponseToID() == entry->lastRequest) ||
      origEvent->getCmd() == Command::Inv) {
    /* Re-send request */
...
void MESIDirectory::handleMemoryResponse(SST::Event *event) {
  ...
  if (ev->queryFlag(MemEvent::F_NONCACHEABLE)) {
    ...
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Received unexpected noncacheable response from "
              "memory. %s. Time = %" PRIu64 "ns\n",
              getName().c_str(), ev->getVerboseString().c_str(),
              getCurrentSimTimeNano());
  }
  ...
  if (ev->getCmd() == Command::ForceInv || ev->getCmd() == Command::FetchInv) {
    ...
    replayWaitingEvents(ev->getBaseAddr());
  } else if (ev->getBaseAddr() == 0 &&
             dirEntryMiss.find(ev->getResponseToID()) !=
    ...
    memReqs.erase(ev->getResponseToID());
    if (ev->getCmd() == Command::FlushLineResp)
      handleFlushLineResponse(ev);
    else
      handleDataResponse(ev);

  } else {
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Received unexpected response from memory - matching "
              "request not found. %s. Time = %" PRIu64 "ns\n",
              getName().c_str(), ev->getVerboseString().c_str(),
              getCurrentSimTimeNano());
  }
...
void MESIDirectory::handleDataResponse(MemEvent *ev) {
  ...
  if (is_debug_event(ev))
    dbg.debug(_L10_, "\t%s\tMSHR remove event <%s, %" PRIx64 ">\n",
              getName().c_str(), CommandString[(int)reqEv->getCmd()],
              reqEv->getBaseAddr());
  ...
  default:
    dbg.fatal(CALL_INFO, 1,
              "Directory %s received Get Response for addr 0x%" PRIx64
...
void MESIDirectory::handleFlushLineResponse(MemEvent *ev) {
  ...
  reqEv->setMemFlags(
      ev->getMemFlags()); // Copy anything back up that needs to be
...
void MESIDirectory::handleDirEntryMemoryResponse(MemEvent *ev) {
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "Directory Controller %s: DirEntry response received for addr "
              "0x%" PRIx64 " but state is %s. Event: %s\n",
...
void MESIDirectory::issueInvalidates(MemEvent *ev, DirEntry *entry,
                                     Command cmd) {
  ...
  if (is_debug_addr(entry->getBaseAddr()))
    dbg.debug(_L4_,
              "Sending Invalidates to fulfill request for exclusive, BsAddr = "
              "%" PRIx64 ".\n",
              entry->getBaseAddr());
}
...
void MESIDirectory::issueFetch(MemEvent *ev, DirEntry *entry, Command cmd) {
  MemEvent *fetch = new MemEvent(getName(), ev->getAddr(), ev->getBaseAddr(),
                                 cmd, cacheLineSize);
...
void MESIDirectory::issueMemoryRequest(MemEvent *ev, DirEntry *entry) {
  ...
  if (is_debug_addr(entry->getBaseAddr())) {
    dbg.debug(_L5_,
              "\tRequesting data from memory.  Cmd = %s, BaseAddr = x%" PRIx64
              ", Size = %u, noncacheable = %s\n",
              CommandString[(int)reqEv->getCmd()], reqEv->getBaseAddr(),
              reqEv->getSize(),
              reqEv->queryFlag(MemEvent::F_NONCACHEABLE) ? "true" : "false");
  }
}
...
void MESIDirectory::getDirEntryFromMemory(DirEntry *entry) {
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "Direcctory Controller %s: cache miss for addr 0x%" PRIx64
              " but state is %s\n",
              getName().c_str(), entry->getBaseAddr(), StateString[st]);
  }

  Addr entryAddr = 0; /* Dummy addr reused for dir cache misses */
  MemEvent *me = new MemEvent(getName(), entryAddr, entryAddr, Command::GetS,
                              cacheLineSize);
  ...
  if (is_debug_addr(entry->getBaseAddr()))
    dbg.debug(_L10_,
              "Requesting Entry from memory for 0x%" PRIx64 "(%" PRIu64
              ", %d)\n",
...
MESIDirectory::DirEntry *MESIDirectory::getDirEntry(Addr baseAddr) {
  ...
  if (directory.end() == i) {
    ...
    entry->setCached(true); // TODO fix this so new entries go to memory if
                            // we're caching, little bit o cheatin here
  } else {
    entry = i->second;
  }
  return entry;
...
void MESIDirectory::sendInvalidate(int target, MemEvent *reqEv, DirEntry *entry,
                                   Command cmd) {
  MemEvent *me = new MemEvent(getName(), entry->getBaseAddr(),
                              entry->getBaseAddr(), cmd, cacheLineSize);
...
void MESIDirectory::forwardFlushRequest(MemEvent *event) {
  MemEvent *reqEv =
      new MemEvent(getName(), event->getAddr(), event->getBaseAddr(),
                   Command::FlushLine, cacheLineSize);
  ...
  if (is_debug_event(event)) {
    dbg.debug(_L5_,
              "Forwarding FlushLine to memory. BaseAddr = x%" PRIx64
              ", Size = %u, noncacheable = %s\n",
              reqEv->getBaseAddr(), reqEv->getSize(),
              reqEv->queryFlag(MemEvent::F_NONCACHEABLE) ? "true" : "false");
  }
}
...
uint32_t MESIDirectory::node_name_to_id(const std::string &name) {
  ...
  if (node_lookup.end() == i) {
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Attempt to lookup node ID but name not found: %s. "
              "Time = %" PRIu64 "ns\n",
...
void MESIDirectory::updateCache(DirEntry *entry) {
  if (0 == entryCacheMaxSize) {
    ...
    if (entry->getState() == I) {
      if (is_debug_addr(entry->getBaseAddr()))
        dbg.debug(_L10_,
                  "Entry for 0x%" PRIx64 " has no references - purging\n",
                  entry->getBaseAddr());
      ...
      while (entryCacheSize > entryCacheMaxSize) {
        ...
        if (mshr->isHit(oldEntry->getBaseAddr()))
          break;

        if (is_debug_addr(entry->getBaseAddr()))
          dbg.debug(_L10_,
                    "entryCache too large.  Evicting entry for 0x%" PRIx64 "\n",
...
void MESIDirectory::sendEntryToMemory(DirEntry *entry) {
  ...
  MemEvent *me =
      new MemEvent(getName(), entryAddr, entryAddr, Command::PutE,
                   cacheLineSize); // MemController discards PutE's without
                                   // writeback so this is safe
  me->setSize(entrySize);
  profileRequestSent(me);
...
MemEvent::id_type MESIDirectory::writebackData(MemEvent *data_event,
                                               Command wbCmd) {
  MemEvent *ev = new MemEvent(getName(), data_event->getBaseAddr(),
                              data_event->getBaseAddr(), wbCmd, cacheLineSize);

  if (data_event->getPayload().size() != cacheLineSize) {
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Writing back data request but payload does not match "
              "cache line size of %uB. Event: %s. Time = %" PRIu64 "ns\n",
              getName().c_str(), cacheLineSize, ev->getVerboseString().c_str(),
              getCurrentSimTimeNano());
  }
  ...
  /* We will get a response if this is a flush request or if we are talking to
   * an endpoint that sends WB Acks */
...
void MESIDirectory::postRequestProcessing(MemEvent *ev, DirEntry *entry,
                                          bool stable) {
  Command cmd = ev->getCmd();
  if (cmd == Command::GetS || cmd == Command::GetX || cmd == Command::GetSX) {
    stat_getRequestLatency->addData(getCurrentSimTimeNano() -
                                    ev->getDeliveryTime());
  } else {
    stat_replacementRequestLatency->addData(getCurrentSimTimeNano() -
                                            ev->getDeliveryTime());
  }
...
bool MESIDirectory::isRequestAddressValid(Addr addr) {
  if (!memLink)
    return network->isRequestAddressValid(addr);
  else
    return memLink->isRequestAddressValid(addr);
...
const char* MESIDirectory::printDirectoryEntryStatus(Addr baseAddr){
    ...
    if(!entry){
        ...
        if(0 == refs) sprintf(dirEntStatus, "[Noncacheable]");
            ...
            sprintf(dirEntStatus, "[owned by %s]",
nodeid_to_name[owner].c_str());
        }
