...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_API(SST::MemHierarchy::CoherenceController,
                                    Params &, bool)
  ...
  /* Send commands when their timestamp expires. Return whether queue is empty
   * or not */
   ...
   * Event contains information about the protocol configuration and so is
   * generated by coherence managers
   */
  virtual MemEventInitCoherence *getInitCoherenceEvent() = 0;

  /* Parse an incoming InitCoherence event */
  virtual void processInitCoherenceEvent(MemEventInitCoherence *event,
                                         bool source);
  ...
  void setCacheListener(std::vector<CacheListener *> &ptr,
                        size_t dropPrefetchLevel, size_t maxOutPrefetches) {
    listeners_ = ptr;
   ...
   */
  void setStatistics(Statistic<uint64_t> *prefetchdrop) {
    statPrefetchDrop = prefetchdrop;
  }

  /* Controller records received events, but valid types are determined by
   * coherence manager. Share those here */
  virtual std::set<Command> getValidReceiveEvents() = 0;

  /* Memory components are identified by their names (e.g., source, destination,
   * requestor) */
  void setName(std::string name) { cachename_ = name; }

  /* Call through to cache array to configure banking/slicing */
  virtual void setSliceAware(uint64_t interleaveSize,
...
protected:
  ...
  uint64_t forwardMessage(MemEvent *event, unsigned int requestSize,
  ...
  /* Initialization */
  ReplacementPolicy *createReplacementPolicy(uint64_t lines, uint64_t assoc,
                                             Params &params, bool L1,
                                             int slotnum = 0);
  ...
  Output *debug;  // Output stream for debug -> SST must be compiled with
                  // --enable-debug
  std::set<Addr> DEBUG_ADDR; // Addresses to print debug info for (all if empty)
  uint32_t
      dlevel; // Debug level -> used to determine output format/amount of output
  ...
  uint64_t lineSize_;
  bool writebackCleanBlocks_; // Writeback clean data as opposed to just a
                              // coherence msg
  bool silentEvictClean_; // Silently evict clean blocks (currently ok when just
                          // mem below us)
  ...
  bool lastLevel_; // Whether we are the lowest coherence level and should not
                   // send coherence messages down
  ...
  /* Statistics - some variables used by all are declared here, but they are
   * maintained by coherence protocols */
  Statistic<uint64_t>
      *stat_eventSent[(int)Command::LAST_CMD]; // Count events sent
  Statistic<uint64_t> *stat_evict[LAST_STATE]; // Count how many evictions
                                               // happened in a given state
  std::array<std::array<Statistic<uint64_t> *, LAST_STATE>,
             (int)Command::LAST_CMD>
      stat_eventState;
  ...
  virtual uint64_t sendResponseUp(MemEvent *event, vector<uint8_t> *data,
                                  bool replay, uint64_t baseTime,
                                  bool atomic = false);
  virtual uint64_t sendResponseUp(MemEvent *event, Command cmd,
  ...
  std::string getDestination(Addr addr) {
    return linkDown_->findTargetDestination(addr);
  }

  std::string getSrc();

private:
  /* Outgoing event queues - events are stalled here to account for access
   * latencies */
