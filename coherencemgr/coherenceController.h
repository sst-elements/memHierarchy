...
namespace SST {
    namespace MemHierarchy {
        ...
        public:
            ...
            virtual CacheAction handleReplacement(MemEvent *event, CacheLine *line,
                                                  MemEvent *reqEvent, bool replay) = 0;

            /* Handle a cache invalidation - Inv, FetchInv, etc. */
            virtual CacheAction handleInvalidationRequest(MemEvent *event, CacheLine *line,
                                                          MemEvent *collisionEvent,
                                                          bool replay) = 0;

            /* Handle an eviction */
            virtual CacheAction handleEviction(CacheLine *line, string rqstr,
                                               bool fromDataCache = false) = 0;

            /* Handle a response - AckInv, GetSResp, etc. */
            virtual CacheAction handleResponse(MemEvent *event, CacheLine *line,
                                               MemEvent *request) = 0;
            ...
            /* Send a response event up (towards CPU). L1s need to implement their own to split out requested bytes. */
            uint64_t sendResponseUp(MemEvent *event, Command cmd, vector <uint8_t> *data,
                                    bool replay, uint64_t baseTime, bool atomic = false);
            ...
            uint64_t forwardMessage(MemEvent *event, Addr baseAddr, unsigned int requestSize,
                                    uint64_t baseTime, vector <uint8_t> *data);
            ...
            void setupLowerStatus(bool silentEvict, bool isLastLevel, bool isNoninclusive,
                                  bool isDir);
        ...
        protected:
            ...
            /* Listener callback */
            virtual void notifyListenerOfAccess(MemEvent *event, NotifyAccessType accessT,
