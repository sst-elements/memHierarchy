 ...
 * -> Essentially coherent for a single thread/single core so will write back
 *dirty blocks but assumes no sharing States: I: not present E: present & clean
 * M: present & dirty
 *---------------------------------------------------------------------------------------------------------------------*/
...
bool IncoherentL1::handleGetS(MemEvent *event, bool inMSHR) {
  ...
  switch (state) {
  case I:
    status = processCacheMiss(event, line,
                              inMSHR); // Attempt to allocate a line if needed

    if (status == MemEventStatus::OK) { // Both MSHR insert & cache line
  ...
  case E:
  case M:
    eventProfileAndNotify(event, state, NotifyAccessType::READ,
                          NotifyResultType::HIT, inMSHR);
    if (localPrefetch) {
...
bool IncoherentL1::handleGetX(MemEvent *event, bool inMSHR) {
  ...
  case I:
    ...
    if (status == MemEventStatus::OK) { // Both MSHR insert & cache line
      ...
      // Profile
      eventProfileAndNotify(event, I, NotifyAccessType::WRITE,
                            NotifyResultType::MISS, true);
  ...
  case M:
    // Profile
    eventProfileAndNotify(event, state, NotifyAccessType::WRITE,
                          NotifyResultType::HIT, inMSHR);
    recordPrefetchResult(line, statPrefetchHit);
    recordLatencyType(event->getID(), LatType::HIT);
    // Handle
  ...
  if (evicted) {
    ...
    if (oldAddr != line->getAddr()) { // We're waiting for a new line now...
      if (is_debug_addr(oldAddr) || is_debug_addr(line->getAddr()) ||
        ...
        debug->debug(_L8_,
                     "\tAddr 0x%" PRIx64 " now waiting for 0x%" PRIx64
                     " instead of 0x%" PRIx64 "\n",
 ...
 * Handle a cache miss
 */
MemEventStatus IncoherentL1::processCacheMiss(MemEvent *event,
                                              L1CacheLine *line, bool inMSHR) {
  if (is_debug_event(event))
 ...
 */
MemEventStatus IncoherentL1::allocateMSHR(MemEvent *event, bool fwdReq,
                                          int pos) {
  int insert_pos =
      mshr_->insertEvent(event->getBaseAddr(), event, pos, fwdReq, false);
  if (is_debug_event(event))
    ...
    if (mshr_->getFrontType(addr) == MSHREntryType::Event) {
      if (!mshr_->getInProgress(addr)) {
        if (is_debug_addr(addr))
          debug->debug(_L5_, "    CleanUpAfterRequest: Waiting Event exists in "
                             "MSHR and is ready to retry\n");
        retryBuffer_.push_back(mshr_->getFrontEvent(addr));
      } else {
        if (is_debug_addr(addr))
          debug->debug(_L5_, "    CleanUpAfterRequest: Waiting Event exists in "
                             "MSHR but is already in progress\n");
      }
      ...
      if (mshr_->getFrontType(addr) == MSHREntryType::Evict) {
        if (is_debug_addr(addr))
          debug->debug(_L5_, "    CleanUpAfterRequest: Waiting Evict in MSHR, "
                             "retrying eviction(s)\n");
        ...
        if (is_debug_addr(addr))
          debug->debug(
              _L5_,
              "    CleanUpAfterRequest: Waiting Writeback in MSHR, stalling\n");
      }
    }
  } else {
    if (is_debug_addr(addr))
      debug->debug(
          _L5_,
          "    CleanUpAfterRequest: MSHR is now empty for this address\n");
  }
}
...
void IncoherentL1::cleanUpAfterResponse(MemEvent *event, bool inMSHR) {
  ...
  if (mshr_->exists(addr)) {
    if (mshr_->getFrontType(addr) == MSHREntryType::Event) {
      if (!mshr_->getInProgress(addr)) {
        if (is_debug_addr(addr))
          debug->debug(_L5_, "    CleanUpAfterResponse: Waiting Event in MSHR "
                             "and is ready to retry\n");
        retryBuffer_.push_back(mshr_->getFrontEvent(addr));
      } else {
        if (is_debug_addr(addr))
          debug->debug(_L5_, "    CleanUpAfterResponse: Waiting Event in MSHR "
      ...
      if (is_debug_addr(addr))
        debug->debug(_L5_, "    CleanUpAfterResponse: Waiting Evict in MSHR, "
                           "retrying eviction\n");
      std::list<Addr> *evictPointers = mshr_->getEvictPointers(addr);
      for (std::list<Addr>::iterator it = evictPointers->begin();
           it != evictPointers->end(); it++) {
    ...
    if (is_debug_addr(addr))
      debug->debug(
          _L5_,
          "    CleanUpAfterResponse: MSHR is now empty for this address\n");
  }
}
...
uint64_t IncoherentL1::sendResponseUp(MemEvent *event, vector<uint8_t> *data,
  ...
  if (is_debug_event(responseEvent)) {
    debug->debug(_L3_,
                 "Sending response at cycle %" PRIu64
                 ". Current time = %" PRIu64 ". Response = (%s)\n",
...
void IncoherentL1::sendResponseDown(MemEvent *event, L1CacheLine *line,
                                    bool data) {
  MemEvent *responseEvent = event->makeResponse();
...
void IncoherentL1::forwardFlush(MemEvent *event, L1CacheLine *line,
                                bool evict) {
  ...
  if (is_debug_addr(event->getBaseAddr())) {
    debug->debug(_L3_,
                 "Forwarding Flush at cycle = %" PRIu64
                 ", Cmd = %s, Src = %s, %s\n",
...
void IncoherentL1::sendWriteback(Command cmd, L1CacheLine *line, bool dirty) {
  ...
  if (is_debug_addr(line->getAddr()))
    debug->debug(
        _L3_,
        "Sending Writeback at cycle = %" PRIu64 ", Cmd = %s. With%s data.\n",
        deliveryTime, CommandString[(int)cmd],
        ((cmd == Command::PutM || writebackCleanBlocks_) ? "" : "out"));
}
...
void IncoherentL1::recordPrefetchResult(L1CacheLine *line,
...
void IncoherentL1::eventProfileAndNotify(MemEvent *event, State state,
