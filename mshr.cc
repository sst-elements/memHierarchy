...
MSHR::MSHR(Output *debug, int maxSize, string cacheName,
           std::set<Addr> debugAddr) {
  d_ = debug;
  maxSize_ = maxSize;
  size_ = 0;
  prefetchCount_ = 0;
...
int MSHR::getMaxSize() { return maxSize_; }

int MSHR::getSize() { return size_; }
...
bool MSHR::exists(Addr addr) { return mshr_.find(addr) != mshr_.end(); }

MSHREntry MSHR::getEntry(Addr addr, size_t index) {
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getEntry(0x%" PRIx64
              ", %zu). Address doesn't exist in MSHR.\n",
              ownerName_.c_str(), addr, index);
  }
  if (mshr_.find(addr)->second.entries.size() <= index) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getEntry(0x%" PRIx64
              ", %zu). Entry list size is %zu.\n",
              ownerName_.c_str(), addr, index,
              mshr_.find(addr)->second.entries.size());
  }
...
MSHREntry MSHR::getFront(Addr addr) {
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getFront(0x%" PRIx64
              "). Address doesn't exist in MSHR.\n",
...
void MSHR::removeEntry(Addr addr, size_t index) {
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::removeEntry(0x%" PRIx64
              ", %zu). Address doesn't exist in MSHR.\n",
              ownerName_.c_str(), addr, index);
  }
  ...
  if (reg->entries.size() <= index) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::removeEntry(0x%" PRIx64
              ", %zu). Entry list is shorter than requested index.\n",
              ownerName_.c_str(), addr, index);
  }
  ...
  if (reg->entries.empty()) {
    ...
    // d_->debug(_L10_, "M: %-41" PRIu64 " %-20s Erase        0x%-16" PRIx64 "
    // %-10d\n",
    //        Simulation::getSimulation()->getCurrentSimCycle(),
    //        ownerName_.c_str(), addr, size_);
...
void MSHR::removeFront(Addr addr) {
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::removeFront(0x%" PRIx64
              "). Address doesn't exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
  MSHRRegister *reg = &(mshr_.find(addr)->second);
  ...
  //     reg->entries.front().getString().c_str());
...
MSHREntryType MSHR::getEntryType(Addr addr, size_t index) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getEntryType(0x%" PRIx64
              ", %zu). Address doesn't exist in MSHR.\n",
              ownerName_.c_str(), addr, index);
  }
  if (mshr_.find(addr)->second.entries.size() <= index) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getEntryType(0x%" PRIx64
              ", %zu). Entry list is shoerter than index.\n",
              ownerName_.c_str(), addr, index);
  }
  std::list<MSHREntry>::iterator it = mshr_.find(addr)->second.entries.begin();
  std::advance(it, index);
...
MSHREntryType MSHR::getFrontType(Addr addr) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getFrontType(0x%" PRIx64
              "). Address doesn't exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
  if (mshr_.find(addr)->second.entries.empty()) {
...
MemEventBase *MSHR::getEntryEvent(Addr addr, size_t index) {
  ...
  if (mshr_.find(addr) == mshr_.end() ||
      mshr_.find(addr)->second.entries.size() <= index)
    return nullptr;
...
MemEventBase *MSHR::getFirstEventEntry(Addr addr, Command cmd) {
  //    if (is_debug_addr(addr))
  //        d_->debug(_L20_, "    MSHR::getFirstEventEntry(0x%" PRIx64 ",
  ...
  for (std::list<MSHREntry>::iterator it =
           mshr_.find(addr)->second.entries.begin();
       it != mshr_.find(addr)->second.entries.end(); it++) {
    if (it->getType() == MSHREntryType::Event &&
        it->getEvent()->getCmd() == cmd)
      return it->getEvent();
  }
  return nullptr;
}
...
std::list<Addr> *MSHR::getEvictPointers(Addr addr) {
  if (getFrontType(addr) != MSHREntryType::Evict)
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getEvictPointers(0x%" PRIx64
              "). Entry type is not Evict.\n",
              ownerName_.c_str(), addr);
...
bool MSHR::removeEvictPointer(Addr addr, Addr addrPtr) {
  if (getFrontType(addr) == MSHREntryType::Event)
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::removeEvictPointer(0x%" PRIx64 ", 0x%" PRIx64
              "). Front entry type is not Evict or Writeback.\n",
  ...
  // Sometimes we insert a WB before the Evict & then remove the Evict pointer,
  // othertimes the Evict is front
  if (getFrontType(addr) == MSHREntryType::Evict) {
    ...
    std::list<MSHREntry>::iterator it =
        mshr_.find(addr)->second.entries.begin();
    it++;
...
int MSHR::insertEvent(Addr addr, MemEventBase *event, int pos, bool fwdRequest,
                      bool stallEvict) {
  if ((size_ == maxSize_) || (!fwdRequest && (size_ == maxSize_ - 1))) {
    if (is_debug_addr(addr)) {
      stringstream reason;
      reason << "<" << event->getID().first << "," << event->getID().second
             << "> FAILED " << (fwdRequest ? "fwd, " : "")
             << "maxsz: " << maxSize_;
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    ...
    if (pos == -1 || pos > mshr_.find(addr)->second.entries.size()) {
      ...
      if (is_debug_addr(addr)) {
        stringstream reason;
        reason << "<" << event->getID().first << "," << event->getID().second
               << ">, pos=" << (mshr_.find(addr)->second.entries.size() - 1);
        printDebug(10, "InsEv", addr, reason.str());
      }
...
void MSHR::moveEntryToFront(Addr addr, unsigned int index) {
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::moveEntryToFront(0x%" PRIx64
              ", %u). Address doesn't exist in MSHR.\n",
              ownerName_.c_str(), addr, index);
  }
  MSHRRegister *reg = &(mshr_.find(addr)->second);
  if (reg->entries.size() <= index) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::moveEntryToFront(0x%" PRIx64
              ", %u). Entry list is shorter than requested index.\n",
              ownerName_.c_str(), addr, index);
  }
...
bool MSHR::insertEviction(Addr oldAddr, Addr newAddr) {
  ...
  //        d_->debug(_L10_, "    MSHR::insertEviction(0x%" PRIx64 ", 0x%"
  //        PRIx64 ")\n", oldAddr, newAddr);
  ...
  if (mshr_.find(oldAddr) == mshr_.end()) { // No MSHR entry for oldAddr
    ...
    if (!entries->empty() &&
        entries->back().getType() ==
            MSHREntryType::Evict) { // MSHR entry for oldAddr is an Evict
...
void MSHR::addPendingRetry(Addr addr) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::addPendingRetry(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
...
void MSHR::removePendingRetry(Addr addr) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::removePendingRetry(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
...
void MSHR::setInProgress(Addr addr, bool value) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::setInProgress(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
  if (mshr_.find(addr)->second.entries.empty()) {
...
void MSHR::setStalledForEvict(Addr addr, bool set) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::setStalledForEvict(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
  if (mshr_.find(addr)->second.entries.empty()) {
...
void MSHR::setProfiled(Addr addr) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::setProfiled(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
  if (mshr_.find(addr)->second.entries.empty()) {
...
bool MSHR::getProfiled(Addr addr) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getProfiled(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
  if (mshr_.find(addr)->second.entries.empty()) {
...
bool MSHR::getProfiled(Addr addr, SST::Event::id_type id) {
  if (mshr_.find(addr) == mshr_.end())
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getProfiled(0x%" PRIx64 ", (%" PRIu64
              ", %" PRId32 ")). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr, id.first, id.second);
  if (mshr_.find(addr)->second.entries.empty())
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getProfiled(0x%" PRIx64 ", (%" PRIu64
              ", %" PRId32 ")). Entry list is empty.\n",
              ownerName_.c_str(), addr, id.first, id.second);
  for (list<MSHREntry>::iterator jt = mshr_.find(addr)->second.entries.begin();
       jt != mshr_.find(addr)->second.entries.end(); jt++) {
    if (jt->getType() == MSHREntryType::Event &&
        jt->getEvent()->getID() == id) {
...
void MSHR::setProfiled(Addr addr, SST::Event::id_type id) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::setProfiled(0x%" PRIx64 ", (%" PRIu64
              ", %" PRId32 ")). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr, id.first, id.second);
  }
  if (mshr_.find(addr)->second.entries.empty()) {
...
MSHREntry *MSHR::getOldestEntry() {
  ...
  for (MSHRBlock::iterator it = mshr_.begin(); it != mshr_.end(); it++) {
    for (list<MSHREntry>::iterator jt = it->second.entries.begin();
         jt != it->second.entries.end(); jt++) {
      if (jt->getType() == MSHREntryType::Event) {
        if (!first) {
          entry = &(*jt);
...
bool MSHR::decrementAcksNeeded(Addr addr) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::decrementAcksNeeded(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
...
void MSHR::setData(Addr addr, vector<uint8_t> &data, bool dirty) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::setData(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
...
vector<uint8_t> &MSHR::getData(Addr addr) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getData(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
...
bool MSHR::getDataDirty(Addr addr) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::getDataDirty(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
...
void MSHR::setDataDirty(Addr addr, bool dirty) {
  ...
  if (mshr_.find(addr) == mshr_.end()) {
    d_->fatal(CALL_INFO, -1,
              "%s, Error: MSHR::setDataDirty(0x%" PRIx64
              "). Address does not exist in MSHR.\n",
              ownerName_.c_str(), addr);
  }
  mshr_.find(addr)->second.dataDirty = dirty;
...
void MSHR::printDebug(uint32_t lev, std::string action, Addr addr,
                      std::string reason) {
  if (lev == 10) {
    if (reason.empty())
      d_->debug(_L10_,
                "M: %-41" PRIu64 " %-20s MSHR:%-8s 0x%-16" PRIx64 " Sz: %-6d\n",
                Simulation::getSimulation()->getCurrentSimCycle(),
                ownerName_.c_str(), action.c_str(), addr, size_);
    else
      d_->debug(
          _L10_,
          "M: %-41" PRIu64 " %-20s MSHR:%-8s 0x%-16" PRIx64 " Sz: %-6d (%s)\n",
          Simulation::getSimulation()->getCurrentSimCycle(), ownerName_.c_str(),
          action.c_str(), addr, size_, reason.c_str());
  } else {
    if (reason.empty())
      d_->debug(_L20_,
                "M: %-41" PRIu64 " %-20s MSHR:%-8s 0x%-16" PRIx64 " Sz: %-6d\n",
...
// Print status. Called by cache controller on EmergencyShutdown and
// printStatus()
void MSHR::printStatus(Output &out) {
  out.output("    MSHR Status for %s. Size: %u. Prefetches: %u\b",
             ownerName_.c_str(), size_, prefetchCount_);
  for (std::map<Addr, MSHRRegister>::iterator it = mshr_.begin();
       it != mshr_.end(); it++) { // Iterate over addresses
    out.output("      Entry: Addr = 0x%" PRIx64 "\n", (it->first));
    for (std::list<MSHREntry>::iterator it2 = it->second.entries.begin();
         it2 != it->second.entries.end();
         it2++) { // Iterate over entries for each address
      out.output("        %s\n", it2->getString().c_str());
    }
  }
