...
/* Element Library Info */
            SST_ELI_REGISTER_COMPONENT(Bus,
            "memHierarchy", "Bus", SST_ELI_ELEMENT_VERSION(1,0,0), "Bus model for memHierarchy", COMPONENT_CATEGORY_MEMORY)
            ...
            {
                "low_network_%(low_network_ports)d", "Ports connected to lower level caches (closer to main memory)", {
                    "memHierarchy.MemEventBase"}
            },
            {
                "high_network_%(high_network_ports)d", "Ports connected to higher level caches (closer to CPU)", {
                    "memHierarchy.MemEventBase"}
            } )
