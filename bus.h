 ...
 *
 *  Connects one or more upper level components to one or more lower level
 * components over a bus like interface
 */

class Bus : public SST::Component {
public:
  /* Element Library Info */
  SST_ELI_REGISTER_COMPONENT(Bus, "memHierarchy", "Bus",
                             SST_ELI_ELEMENT_VERSION(1, 0, 0),
                             "Bus model for memHierarchy",
                             COMPONENT_CATEGORY_MEMORY)

  SST_ELI_DOCUMENT_PARAMS(
      {"bus_frequency", "(string) Bus clock frequency"},
      {"broadcast", "(bool) If set, messages are broadcast to all other ports",
       "0"},
      {"fanout",
       "(bool) If set, messages from the high network are replicated and sent "
       "to all low network ports",
       "0"},
      {"bus_latency_cycles", "(uint) Bus latency in cycles", "0"},
      {"idle_max",
       "(uint) Bus temporarily turns off clock after this number of idle "
       "cycles",
       "6"},
      {"drain_bus", "(bool) Drain bus on every cycle", "0"},
      {"debug",
       "(uint) Output location for debug statements. Requires core "
       "configuration flag '--enable-debug'. --0[None], 1[STDOUT], 2[STDERR], "
       "3[FILE]--",
       "0"},
      {"debug_level", "(uint) Debugging level: 0 to 10", "0"},
      {"debug_addr",
       "(comma separated uints) Address(es) to be debugged. Leave empty for "
       "all, otherwise specify one or more comma separated values. Start and "
       "end string with brackets",
       ""})

  SST_ELI_DOCUMENT_PORTS(
      {"low_network_%(low_network_ports)d",
       "Ports connected to lower level caches (closer to main memory)",
       {"memHierarchy.MemEventBase"}},
      {"high_network_%(high_network_ports)d",
       "Ports connected to higher level caches (closer to CPU)",
       {"memHierarchy.MemEventBase"}})
