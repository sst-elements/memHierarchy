...
ramulatorMemory::ramulatorMemory(ComponentId_t id, Params &params)
    : SimpleMemBackend(id, params),
      callBackFunc(std::bind(&ramulatorMemory::ramulatorDone, this,
                             std::placeholders::_1)) {
  build(params);
}

void ramulatorMemory::build(Params &params) {
  std::string ramulatorCfg =
      params.find<std::string>("configFile", NO_STRING_DEFINED);
  ...
  memSystem =
      new Gem5Wrapper(configs, m_reqWidth); // default cache line to 64 byte

  output->output(CALL_INFO, "Instantiated Ramulator from config file %s\n",
                 ramulatorCfg.c_str());
}

bool ramulatorMemory::issueRequest(ReqId reqId, Addr addr, bool isWrite,
                                   unsigned numBytes) {
  ramulator::Request::Type type = (isWrite) ? (ramulator::Request::Type::WRITE)
                                            : (ramulator::Request::Type::READ);

  ramulator::Request request(addr, type, callBackFunc,
                             0); /* context or core ID. ? */

  bool ok = memSystem->send(request);
#ifdef __SST_DEBUG_OUTPUT__
  output->debug(_L10_,
                "RamulatorBackend: Attempting to issue %s request for %" PRIx64
                ". Accepted: %d\n",
...
void ramulatorMemory::finish() { memSystem->finish(); }
...
void ramulatorMemory::ramulatorDone(ramulator::Request &ramReq) {
...
#ifdef __SST_DEBUG_OUTPUT__
  output->debug(_L10_,
                "RamulatorBackend: Memory Request for %" PRIx64
                " Finished [%zu reqs]\n",
                (Addr)addr, reqs.size());
#endif
  // Clean up dramReqs
  if (reqs.size() < 1)
    output->fatal(CALL_INFO, -1,
                  "RamulatorBackend: Error - ramulatorDone called but "
                  "dramReqs[addr] is empty. Addr: %" PRIx64 "\n",
                  (Addr)addr);
