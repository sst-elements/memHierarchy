...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      SimplePagePolicy, "memHierarchy", "simplePagePolicy",
      SST_ELI_ELEMENT_VERSION(1, 0, 0), "static page open or close policy",
      SST::MemHierarchy::TimingDRAM_NS::PagePolicy)

  SST_ELI_DOCUMENT_PARAMS(
      {"close", "Whether to use a closed (true) or open (false) page policy",
       "true"})
  ...
  bool shouldClose(SimTime_t current) { return m_close; }

  bool canClose() { return m_close; }
...
public:
  /* Element Library Info */
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      TimeoutPagePolicy, "memHierarchy", "timeoutPagePolicy",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "timeout based page open or close policy",
      SST::MemHierarchy::TimingDRAM_NS::PagePolicy)

  SST_ELI_DOCUMENT_PARAMS({"timeoutCycles",
                           "Timeout (close page) after this many cycles", "5"})

  /* Begin class definition */
  TimeoutPagePolicy(ComponentId_t id, Params &params)
      : PagePolicy(id, params), m_numCyclesLeft(0), m_lastCycle(-2) {
