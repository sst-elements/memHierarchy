...
FlashDIMMSimMemory::FlashDIMMSimMemory(ComponentId_t id, Params &params)
...
void FlashDIMMSimMemory::build(Params &params) {
  ...
  FDSim::Callback_t *readDataCB =
      new FDSim::Callback<FlashDIMMSimMemory, void, uint, uint64_t, uint64_t>(
          this, &FlashDIMMSimMemory::FlashDIMMSimDone);
  FDSim::Callback_t *writeDataCB =
      new FDSim::Callback<FlashDIMMSimMemory, void, uint, uint64_t, uint64_t>(
          this, &FlashDIMMSimMemory::FlashDIMMSimDone);
  ...
  output->verbose(CALL_INFO, 2, 0,
                  "Flash DIMM Backend Initialization complete.\n");
}

bool FlashDIMMSimMemory::issueRequest(ReqId id, Addr addr, bool isWrite,
                                      unsigned) {
  ...
  output->verbose(CALL_INFO, 4, 0,
                  "Backend issuing transaction for address %" PRIx64 "\n",
                  (Addr)addr);
...
void FlashDIMMSimMemory::finish() { memSystem->printStats(); }

void FlashDIMMSimMemory::FlashDIMMSimDone(unsigned int id, uint64_t addr,
                                          uint64_t clockcycle) {
  std::deque<ReqId> &reqs = dramReqs[addr];

  output->verbose(CALL_INFO, 4, 0,
                  "Backend retiring request for address %" PRIx64
                  ", Reqs: %" PRIu64 "\n",
