...
/*------------------------------- Simple Backend -------------------------------
 */
RequestReorderRow::RequestReorderRow(ComponentId_t id, Params &params)
...
void RequestReorderRow::build(Params &params) {
  ...
  maxReqsPerRow =
      params.find<unsigned int>("reorder_limit", 1); // No re-ordering
  UnitAlgebra requestSize(
  ...
  if (!(requestSize.hasUnits("B"))) {
    output->fatal(CALL_INFO, -1,
                  "Invalid param(%s): bank_interleave_granularity - must have "
                  "units of 'B' (bytes). You specified '%s'.\n",
                  getName().c_str(), requestSize.toString().c_str());
  }
  if (!isPowerOfTwo(requestSize.getRoundedValue())) {
    output->fatal(CALL_INFO, -1,
                  "Invalid param(%s): bank_interleave_granularity - must be a "
  ...
  if (!backend) {
    ...
    backend = loadAnonymousSubComponent<SimpleMemBackend>(
        ...
        ComponentInfo::INSERT_STATS | ComponentInfo::SHARE_PORTS,
        backendParams);
  }
  using std::placeholders::_1;
  backend->setResponseHandler(
      std::bind(&RequestReorderRow::handleMemResponse, this, _1));
...
bool RequestReorderRow::issueRequest(ReqId id, Addr addr, bool isWrite,
                                     unsigned numBytes) {
...
bool RequestReorderRow::clock(Cycle_t cycle) {
  ...
  if (!requestQueue.empty()) {
    ...
    for (unsigned int i = 0; i < banks; i++) {
      ...
      // Decide whether to try to re-order a request to this bank or issue a new
      // row
      bool reorderIssued = false;
      if (reorderCount[bank] != maxReqsPerRow) {
        std::list<Req> *bankList = requestQueue[bank];
        for (std::list<Req>::iterator it = bankList->begin();
             it != bankList->end(); it++) {
          unsigned int row = (*it).addr >> rowOffset;

          if (row == lastRow[bank]) {
            // Attempt issue, if we're blocked, this bank is busy & move to next
            // bank
            bool issued = backend->issueRequest((*it).id, (*it).addr,
                                                (*it).isWrite, (*it).numBytes);
            reorderIssued = true;
            if (issued) {
              reqsIssuedThisCycle++;
      ...
      if (!reorderIssued) {
        ...
        if (backend->issueRequest(req.id, req.addr, req.isWrite,
                                  req.numBytes)) {
          reqsIssuedThisCycle++;
          nextBank = (bank + 1) % banks;
          reorderCount[bank] = 1;
...
void RequestReorderRow::setup() { backend->setup(); }

void RequestReorderRow::finish() { backend->finish(); }
