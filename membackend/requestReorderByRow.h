...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      RequestReorderRow, "memHierarchy", "reorderByRow",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"max_issue_per_cycle",
       "Maximum number of requests to issue per cycle. 0 or negative is "
       "unlimited.",
       "-1"},
      {"banks", "Number of banks", "8"},
      {"bank_interleave_granularity",
       "Granularity of interleaving in bytes (B), generally a cache line. Must "
       "be a power of 2.",
       "64B"},
      {"row_size", "Size of a row in bytes (B). Must be a power of 2.", "8KiB"},
      {"reorder_limit",
       "Maximum number of request to reorder to a rwo before changing rows.",
       "1"},
...
private:
  ...
  unsigned int maxReqsPerRow; // Maximum number of requests to issue per row
                              // before moving to a new row
  ...
  unsigned int
      lineOffset;   // Offset for determining line (needed for finding bank)
  int reqsPerCycle; // Number of requests to issue per cycle (max) -> memCtrl
                    // limits how many we accept
