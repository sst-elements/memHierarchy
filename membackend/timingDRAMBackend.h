...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      TimingDRAM, "memHierarchy", "timingDRAM",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Moderately-detailed timing model for DRAM",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"addrMapper", "Address map subcomponent",
      ...
      {"channel.rank.bank.transactionQ",
       "Transaction queue model (subcomponent)",
       "memHierarchy.fifoTransactionQ"},
      {"channel.rank.bank.pagePolicy",
       "Policy subcomponent for managing row buffer",
       "memHierarchy.simplePagePolicy"})

  SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS(
      {"transactionQ", "Transaction queue model",
       "SST::MemHierarchy::TimingDRAM_NS::TransactionQ"},
      {"pagePolicy", "Policy subcomponent for managing row buffer",
       "SST::MemHierarchy::TimingDRAM_NS::PagePolicy"})
...
private:
  ...
  public:
    ...
    void pushTrans(Transaction *trans) { m_transQ->push(trans); }

    Cmd *popCmd(SimTime_t cycle, SimTime_t dataBusAvailCycle);

    void setLastCmd(Cmd *cmd) { m_lastCmd = cmd; }

    void clearLastCmd() { m_lastCmd = NULL; }

    Cmd *getLastCmd() { return m_lastCmd; }
    ...
    bool isIdle() {
      return (m_row == -1 || !m_pagePolicy->canClose()) && m_cmdQ.empty() &&
             m_transQ->empty();
    }
  ...
  public:
    ...
    Cmd(Bank *bank, Op op, unsigned cycles, unsigned row = -1,
        unsigned dataCycles = 0, Transaction *trans = NULL)
        : m_bank(bank), m_op(op), m_cycles(cycles), m_row(row),
          m_dataCycles(dataCycles), m_trans(trans) {
      switch (m_op) {
      case PRE:
        m_name = "PRE";
        break;
      ...
      if (is_debug)
        m_bank->verbose(__LINE__, __FUNCTION__,
                        "new %s for rank=%d bank=%d row=%d\n",
                        getName().c_str(), getRank(), getBank(), getRow());
    }

    ~Cmd() { m_bank->clearLastCmd(); }
  ...
  public:
    ...
    Rank(ComponentId_t, Params &, unsigned mc, unsigned chan, unsigned rank,
         Output *, AddrMapper *);
    ...
    bool hasActiveBanks() { return !m_banksActive.empty(); }
    ...
    bool issue(SimTime_t createTime, ReqId id, Addr addr, bool isWrite,
      ...
      if (is_debug)
        m_output->verbosePrefix(prefix(), CALL_INFO, 3, DBG_MASK,
                                "reqId=%" PRIu64 " rank=%d addr=%#" PRIx64
                                ", createTime=%" PRIu64 "\n",
                                id, rank, addr, createTime);

      Transaction *trans =
          new Transaction(createTime, id, addr, isWrite, numBytes,
                          m_mapper->getBank(addr), m_mapper->getRow(addr));
      m_pendingCount++;
