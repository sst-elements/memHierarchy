...
#include "../util.h"
#include "HBMpagedMultiBackend.h"
...
void HBMpagedMultiMemory::build(Params &params) {
  ...
  registerClock(clock_freq, new Clock::Handler<HBMpagedMultiMemory>(
                                this, &HBMpagedMultiMemory::quantaClock));

  // determine page replacement / addition strategy
  std::string stratStr =
      params.find<std::string>("page_replace_strategy", "FIFO");
  if (stratStr == "FIFO") {
    replaceStrat = FIFO;
    ...
    dbg.fatal(CALL_INFO, -1,
              "Invalid page replacement Strategy (page_replace_strategy)\n");
  }
  ...
  if (stratStr == "MFU") {
    ...
    dbg.fatal(CALL_INFO, -1,
              "Invalid page addition Strategy (page_add_strategy)\n");
  }

  if (addStrat == addMFU) {
    if ((replaceStrat != LFU) && (replaceStrat != LFU8)) {
      dbg.fatal(CALL_INFO, -1,
                "MFU page addition strategy requires LFU page replacement "
                "strategy\n");
    }
  }
  ...
  minAccTime =
      self_link->getDefaultTimeBase()->getFactor() /
  ...
  dbg.verbose(CALL_INFO, 1, 0,
              "Using Mersenne Generator with seed: %" PRIu32 "\n", seed);
  ...
  if (modelSwaps) {
    ...
    HBMDRAMSim::Callback<HBMpagedMultiMemory, void, unsigned int, uint64_t,
                         uint64_t> *readDataCB,
        *writeDataCB;

    readDataCB = new HBMDRAMSim::Callback<HBMpagedMultiMemory, void,
                                          unsigned int, uint64_t, uint64_t>(
        this, &HBMpagedMultiMemory::dramSimDone);
    writeDataCB = new HBMDRAMSim::Callback<HBMpagedMultiMemory, void,
                                           unsigned int, uint64_t, uint64_t>(
        this, &HBMpagedMultiMemory::dramSimDone);
...
bool HBMpagedMultiMemory::checkAdd(HBMpageInfo &page) {
  ...
  if (dramBackpressure && dramQ.size() >= 4)
    return false;
  ...
  case addMFRPU: {
    ...
    if (pageList.empty())
      return (page.lastTouch > threshold); // startup case
    ...
    if (myLastTouch > victimPage->lastTouch) {
      if (addStrat == addMFRPU) {
        ...
        return (page.touched > threshold) &&
               (page.touched > victimPage->touched);
      } else {
        // more recent
        return (page.touched > threshold);
      }
...
void HBMpagedMultiMemory::do_FIFO_LRU(HBMpageInfo &page, bool &inFast,
                                      bool &swapping) {
  swapping = 0;
  if (0 == page.inFast) {
    // not in fast
...
void HBMpagedMultiMemory::do_LFU(Addr addr, HBMpageInfo &page, bool &inFast,
                                 bool &swapping) {
          ...
          for (auto p = pageMap.begin(); p != endP; ++p) {
            if ((p->second.inFast == 1) && (p->first != pageAddr)) {
              ...
              if ((p->second.touched < page.touched) &&
                  (p->second.swapDir ==
                   HBMpageInfo::NONE)) { // make sure we don't bump someone in
                                         // motion
                found = 1;
...
bool HBMpagedMultiMemory::issueRequest(ReqId id, Addr addr, bool isWrite,
                                       unsigned numBytes) {
  ...
  page.record(addr, isWrite, getRequestor(id), collectStats, pageAddr,
              replaceStrat == LFU8);
  ...
  if (modelSwaps) {
    ...
    if (transferDelay > 0) {
      ...
      if (page.pageDelay > now) {
        ...
        extraDelay =
            max(extraDelay, minAccTime); // make sure it is always at least as
                                         // slow as the fast mem
      }
    }
    ...
    if (inFast) {
      ...
      return HBMDRAMSimMemory::issueRequest((ReqId)req, addr, isWrite,
                                            numBytes);
    }
  }
}
...
bool HBMpagedMultiMemory::clock(Cycle_t cycle) {
  ...
  while (!dramQ.empty()) {
    Req *req = dramQ.front();
    bool inserted = HBMDRAMSimMemory::issueRequest((ReqId)req, req->addr,
                                                   req->isWrite, req->numBytes);
    if (inserted) {
      dramQ.pop();
    } else {
      break;
...
void HBMpagedMultiMemory::dramSimDone(unsigned int id, uint64_t addr,
                                      uint64_t clockcycle) {
  ...
  if (modelSwaps && si != swapToSlow_Writes.end()) {
    ...
    // (void*)(req->baseAddr_+req->amtInProcess_), si_r->second,
    // si_r->second->swapsOut);
