...
CramSimMemory::CramSimMemory(ComponentId_t id, Params &params)
...
void CramSimMemory::build(Params &params) {
  ...
  if (isPortConnected("cramsim_link"))
    cramsim_link = configureLink("cramsim_link", access_time,
                                 new Event::Handler<CramSimMemory>(
                                     this, &CramSimMemory::handleCramsimEvent));
  else
...
bool CramSimMemory::issueRequest(ReqId reqId, Addr addr, bool isWrite,
                                 unsigned numBytes) {

  if (memReqs.size() >= m_maxNumOutstandingReqs) {
    output->verbose(CALL_INFO, 1, 0,
                    "the number of outstanding requests reaches to max %d\n",
  ...
  cramsim_link->send(
      new CramSim::MemReqEvent(reqId, addr, isWrite, numBytes, 0));
  return true;
}
