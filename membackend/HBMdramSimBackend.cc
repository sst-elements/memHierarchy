...
HBMDRAMSimMemory::HBMDRAMSimMemory(ComponentId_t id, Params &params)
...
void HBMDRAMSimMemory::build(Params &params) {
  ...
  if (ramSize.getRoundedValue() % (1024 * 1024) != 0) {
    output->fatal(CALL_INFO, -1,
                  "For HBMDRAMSim, backend.mem_size must be a multiple of "
                  "1MiB. Note: for units in base-10 use 'MB', for base-2 use "
                  "'MiB'. You specified '%s'\n",
  ...
  readDataCB =
      new HBMDRAMSim::Callback<HBMDRAMSimMemory, void, unsigned int, uint64_t,
                               uint64_t>(this, &HBMDRAMSimMemory::dramSimDone);
  writeDataCB =
      new HBMDRAMSim::Callback<HBMDRAMSimMemory, void, unsigned int, uint64_t,
                               uint64_t>(this, &HBMDRAMSimMemory::dramSimDone);
...
bool HBMDRAMSimMemory::issueRequest(ReqId id, Addr addr, bool isWrite,
                                    unsigned) {
...
void HBMDRAMSimMemory::finish() { memSystem->printStats(true); }

void HBMDRAMSimMemory::dramSimDone(unsigned int id, uint64_t addr,
                                   uint64_t clockcycle) {
  ...
  if (reqs.size() == 0)
    output->fatal(CALL_INFO, -1,
                  "Error: reqs.size() is 0 at HBMDRAMSimMemory done\n");
