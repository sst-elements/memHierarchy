...
#define MEMBACKENDCONVERTOR_ELI_PARAMS                                         \
  {"debug_level",                                                              \
   "(uint) Debugging level: 0 (no output) to 10 (all output). Output also "    \
   "requires that SST Core be compiled with '--enable-debug'",                 \
   "0"},                                                                       \
      {"debug_mask", "(uint) Mask on debug_level", "0"}, {                     \
    "debug_location", "(uint) 0: No debugging, 1: STDOUT, 2: STDERR, 3: FILE", \
        "0"                                                                    \
  }

#define MEMBACKENDCONVERTOR_ELI_STATS                                          \
  {"cycles_with_issue", "Total cycles with successful issue to back end",      \
   "cycles", 1},                                                               \
      {"cycles_attempted_issue_but_rejected",                                  \
       "Total cycles where an attempt to issue to backend was rejected "       \
       "(indicates backend full)",                                             \
       "cycles", 1},                                                           \
      {"total_cycles", "Total cycles called at the memory controller",         \
       "cycles", 1},                                                           \
      {"requests_received_GetS", "Number of GetS (read) requests received",    \
       "requests", 1},                                                         \
      {"requests_received_GetSX", "Number of GetSX (read) requests received",  \
       "requests", 1},                                                         \
      {"requests_received_GetX", "Number of GetX (read) requests received",    \
       "requests", 1},                                                         \
      {"requests_received_PutM", "Number of PutM (write) requests received",   \
       "requests", 1},                                                         \
      {"outstanding_requests",                                                 \
       "Total number of outstanding requests each cycle", "requests", 1},      \
      {"latency_GetS", "Total latency of handled GetS requests", "cycles", 1}, \
      {"latency_GetSX", "Total latency of handled GetSX requests", "cycles",   \
       1},                                                                     \
  ...
  public:
    ...
    virtual bool isDone() {
      return true;
    } /* If we're asking, the answer is yes */
    virtual bool issueDone() {
      return true;
    } /* If we're asking, the answer is yes */
  ...
  public:
    CustomReq(CustomCmdInfo *info, uint32_t reqId)
        : BaseReq(reqId, BaseReq::ReqType::CUSTOM), m_info(info) {}
    ~CustomReq() {}
  ...
  public:
    MemReq(MemEvent *event, uint32_t reqId)
        : BaseReq(reqId, BaseReq::ReqType::MEM), m_event(event), m_offset(0),
          m_numReq(0) {}
    ~MemReq() {}
    ...
    bool issueDone() { return m_offset >= m_event->getSize(); }
    bool isDone() { return (m_offset >= m_event->getSize() && 0 == m_numReq); }

    std::string getString() {
      std::ostringstream str;
      str << "addr: " << addr() << " baseAddr: " << baseAddr()
          << " processed: " << processed();
      str << " isWrite: " << isWrite();
...
public:
  MemBackendConvertor(ComponentId_t id, Params &params, MemBackend *backend,
  ...
  virtual void
  setCallbackHandlers(std::function<void(Event::id_type, uint32_t)> responseCB,
...
private:
  ...
  bool setupMemReq(MemEvent *ev) {
    if (Command::FlushLine == ev->getCmd() ||
        Command::FlushLineInv == ev->getCmd()) {
  ...
  inline void doClockStat() { stat_totalCycles->addData(1); }
  ...
  std::function<void(Event::id_type id, uint32_t)>
  ...
  std::map<SST::Event::id_type, std::set<MemEvent *, memEventCmp>>
      m_dependentRequests; // Reverse map, set of flushes for each request ID,
                           // for faster lookup
