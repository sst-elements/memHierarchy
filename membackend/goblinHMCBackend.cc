...
GOBLINHMCSimBackend::GOBLINHMCSimBackend(ComponentId_t id, Params &params)
    : ExtMemBackend(id, params) {
  build(params);
}
...
void GOBLINHMCSimBackend::build(Params &params) {
  ...
  hmc_capacity_per_device =
      params.find<uint32_t>("capacity_per_device", HMC_MIN_CAPACITY);
  ...
  hmc_dram_latency =
      params.find<uint32_t>("dram_latency", HMC_DEF_DRAM_LATENCY);
#endif
  ...
  int rc = hmcsim_init(&the_hmc, hmc_dev_count, hmc_link_count, hmc_vault_count,
                       hmc_queue_depth, hmc_bank_count, hmc_dram_count,
                       hmc_capacity_per_device, hmc_xbar_depth);

  if (0 != rc) {
    output->fatal(
        CALL_INFO, -1,
        "Unable to initialize HMC back end model, return code is %d\n", rc);
  } else {
  ...
  if (0 != rc) {
    output->fatal(CALL_INFO, -1,
                  "Unable to initialize HMC back end max block size to 128\n");
  } else {
  ...
  rc = hmcsim_power_config(&the_hmc, link_phy, link_local_route,
                           link_remote_route, xbar_rqst_slot, xbar_rsp_slot,
                           xbar_route_extern, vault_rqst_slot, vault_rsp_slot,
                           vault_ctrl, row_access);
  if (rc != 0) {
    output->fatal(CALL_INFO, -1,
                  "Unable to initialize the HMC-Sim power configuration; "
                  "return code is %d\n",
                  rc);
  }
#endif
  ...
  if (rc != 0) {
    output->fatal(CALL_INFO, -1,
                  "Unable to initialize the HMC-Sim dram latency "
                  "configuration; return code is %d\n",
                  rc);
  }
#endif
  ...
  for (unsigned i = 0; i < cmclibs.size(); i++) {
    if (cmclibs[i].length() > 0) {
      ...
      output->verbose(CALL_INFO, 1, 0, "Initializing HMC-Sim CMC Library: %s\n",
                      cmclibs[i].c_str());
  ...
  for (uint32_t i = 0; i < hmc_link_count; ++i) {
    ...
    rc = hmcsim_link_config(&the_hmc, hmc_dev_count + 1, 0, i, i,
                            HMC_LINK_HOST_DEV);
  ...
  output->verbose(CALL_INFO, 1, 0,
                  "Populating tag entries allowed at the controller, max tag "
                  "count is: %" PRIu32 "\n",
                  hmc_tag_count);
  ...
  if (NULL == hmc_trace_file_handle) {
    output->fatal(CALL_INFO, -1,
                  "Unable to create and open the HMC trace file at %s\n",
                  hmc_trace_file.c_str());
  }
  ...
  if (rc > 0) {
    ...
    output->verbose(CALL_INFO, 1, 0,
                    "Succesfully set the HMC trace file path.\n");
  }

  output->verbose(CALL_INFO, 1, 0,
                  "Setting the trace level of the HMC to %" PRIu32 "\n",
                  hmc_trace_level);
  rc = hmcsim_trace_level(&the_hmc, hmc_trace_level);

  if (rc > 0) {
    output->fatal(CALL_INFO, -1,
                  "Unable to set the HMC trace level to %" PRIu32 "\n",
                  hmc_trace_level);
  } else {
  ...
  output->verbose(CALL_INFO, 1, 0,
                  "Setting the maximum HMC request size to: %" PRIu32 "\n",
                  hmc_max_req_size);
  rc = hmcsim_util_set_all_max_blocksize(&the_hmc, hmc_max_req_size);

  if (rc > 0) {
    output->fatal(CALL_INFO, -1,
                  "Unable to set maximum HMC request size to %" PRIu32 "\n",
                  hmc_max_req_size);
  } else {
    output->verbose(CALL_INFO, 1, 0,
                    "Successfully set maximum request size.\n");
  }
  ...
  if (rc > 0) {
    ...
    output->verbose(CALL_INFO, 1, 0,
                    "Wrote the HMC trace file header successfully.\n");
  }
  ...
  output->verbose(CALL_INFO, 1, 0,
                  "Completed HMC Simulation Backend Initialization.\n");
}

bool GOBLINHMCSimBackend::HMCRqstToStr(hmc_rqst_t R, std::string *S) {
  int cur = 0;
...
bool GOBLINHMCSimBackend::strToHMCRqst(std::string S, hmc_rqst_t *R,
                                       bool isCMC) {
...
void GOBLINHMCSimBackend::handleCMCConfig() {
  ...
  for (unsigned i = 0; i < cmcconfigs.size(); i++) {
    ...
    if (vstr.size() != 4) {
      // formatting error
      output->fatal(CALL_INFO, -1, "Unable to process cmc config: %s\n",
                    s.c_str());
    }
    ...
    if (rc != 0) {
      output->fatal(CALL_INFO, -1,
                    "Unable to load the mapped HMC-Sim CMC Library %s and the "
                    "return code is %d\n",
                    path.c_str(), rc);
    }
...
void GOBLINHMCSimBackend::splitStr(const string &s, char c, vector<string> &v) {
...
void GOBLINHMCSimBackend::handleCmdMap() {
  ...
  for (unsigned i = 0; i < cmdmaps.size(); i++) {
    ...
    if (vstr.size() != 4) {
      // formatting error
      output->fatal(CALL_INFO, -1, "Unable to process command map: %s\n",
                    s.c_str());
    }
    ...
    if (ctype == "WR") {
      ...
      // error
      output->fatal(CALL_INFO, -1, "Erroneous command type in CMC config: %s\n",
                    ctype.c_str());
    }
    ...
    CmdMapping.push_back(
        new HMCSimCmdMap(ctype_int, copc_uint, csize_int, rqst));
  }
}
...
bool GOBLINHMCSimBackend::issueMappedRequest(ReqId reqId, Addr addr,
                                             bool isWrite,
                                             std::vector<uint64_t> ins,
                                             uint32_t flags, unsigned numBytes,
                                             uint8_t req_dev, uint16_t req_tag,
                                             bool *Success) {
  ...
  for (auto itr = CmdMapping.begin(); itr != CmdMapping.end(); ++itr) {
    ...
    if ((MapCmd->getSrcType() == Src) &&
      ...
      output->verbose(CALL_INFO, 8, 0, "Found mapped request target of %s\n",
                      name.c_str());
  ...
  output->verbose(CALL_INFO, 8, 0,
                  "Issuing mapped request: %" PRIu64 " %s tag: %" PRIu16
                  " dev: %" PRIu8 " link: %" PRIu8 "\n",
                  addr, (isWrite ? "WRITE" : "READ"), req_tag, req_dev,
                  req_dev);

  int rc =
      hmcsim_build_memrequest(&the_hmc, req_dev, addr, req_tag, req_type,
                              req_link, hmc_payload, &req_header, &req_tail);

  if (rc > 0) {
    output->fatal(CALL_INFO, -1,
                  "Unable to build a mapped request for address: %" PRIu64 "\n",
                  addr);
  }
  ...
  if (HMC_STALL == rc) {
    output->verbose(CALL_INFO, 2, 0,
                    "Issue revoked by HMC, reason: cube is stalled.\n");

    // Restore tag for use later, remember this request did not succeed
    tag_queue.push(req_tag);

    // Return false, the request was not issued so we must tell memory
    // controller to give it back to us when we are free
    *Success = false;
  } else if (0 == rc) {
    output->verbose(CALL_INFO, 4, 0,
                    "Issue of mapped request for address 0X%" PRIx64
                    " successfully accepted by HMC.\n",
                    addr);

    // Create the request entry which we keep in a table
    HMCSimBackEndReq *reqEntry =
        new HMCSimBackEndReq(reqId, addr, getCurrentSimTimeNano(), !isPosted);

    // Add the tag and request into our table of pending
    tag_req_map.insert(
        std::pair<uint16_t, HMCSimBackEndReq *>(req_tag, reqEntry));
    ...
    *Success = false;
    output->fatal(CALL_INFO, -1,
                  "Error issue request for address %" PRIu64
                  " into HMC on link %" PRIu8 "\n",
                  addr, req_link);
  }
...
bool GOBLINHMCSimBackend::issueCustomRequest(ReqId reqId, Addr addr,
                                             uint32_t cmd,
                                             std::vector<uint64_t> ins,
                                             uint32_t flags,
                                             unsigned numBytes) {
  ...
  output->verbose(CALL_INFO, 8, 0,
                  "Received custom request of opcode %" PRIu32
                  " of size: %" PRIu32 "\n",
                  cmd, numBytes);
  ...
  output->verbose(CALL_INFO, 8, 0,
                  "Issuing custom request: %" PRIu64 " cmd=%s tag: %" PRIu16
                  " dev: %" PRIu8 " link: %" PRIu8 "\n",
  ...
  if (rc > 0) {
    output->fatal(CALL_INFO, -1,
                  "Unable to build a custom request for address: %" PRIu64 "\n",
                  addr);
  }
  ...
  if (HMC_STALL == rc) {
    ...
    output->verbose(CALL_INFO, 4, 0,
                    "Issue of custom request for %s at address 0x%" PRIx64
                    " successfully accepted by HMC.\n",
                    name.c_str(), addr);
    ...
    output->fatal(CALL_INFO, -1,
                  "Error issue request for %s at address %" PRIu64
                  " into HMC on link %" PRIu8 "\n",
...
bool GOBLINHMCSimBackend::issueRequest(ReqId reqId, Addr addr, bool isWrite,
  ...
  output->verbose(CALL_INFO, 8, 0,
                  "Received request of size: %" PRIu32 " of type %s\n",
  ...
  if (CmdMapping.size() > 0) {
    ...
    if (issueMappedRequest(reqId, addr, isWrite, ins, flags, numBytes, req_dev,
                           req_tag, &Success)) {
  ...
  // Check if the request is for a read or write then transform this into
  // something for HMC simulator to use
  if ((isWrite) && (isPosted)) {
    switch (numBytes) {
    case 8:
    case 16:
  ...
  output->verbose(CALL_INFO, 8, 0,
                  "Issuing request: %" PRIu64 " %s tag: %" PRIu16
                  " dev: %" PRIu8 " link: %" PRIu8 "\n",
                  addr, (isWrite ? "WRITE" : "READ"), req_tag, req_dev,
                  req_dev);
  ...
  if (rc > 0) {
    output->fatal(CALL_INFO, -1,
                  "Unable to build a request for address: %" PRIu64 "\n", addr);
  }
  ...
  if (HMC_STALL == rc) {
    ...
    if (isPosted) {
      output->verbose(CALL_INFO, 4, 0,
                      "Issue of posted request %s for address 0X%" PRIx64
                      " successfully accepted by HMC.\n",
                      RqstStr.c_str(), addr);
    } else {
      output->verbose(CALL_INFO, 4, 0,
                      "Issue of request %s for address 0X%" PRIx64
                      " successfully accepted by HMC.\n",
                      RqstStr.c_str(), addr);
    }
...
void GOBLINHMCSimBackend::collectStats() {
#if defined(HMC_STAT)
  // stall stats
  xbar_rqst_stall_stat->addData(
      hmcsim_int_stat(&the_hmc, XBAR_RQST_STALL_STAT));
  xbar_rsp_stall_stat->addData(hmcsim_int_stat(&the_hmc, XBAR_RSP_STALL_STAT));
  vault_rqst_stall_stat->addData(
      hmcsim_int_stat(&the_hmc, VAULT_RQST_STALL_STAT));
  vault_rsp_stall_stat->addData(
      hmcsim_int_stat(&the_hmc, VAULT_RSP_STALL_STAT));
  route_rqst_stall_stat->addData(
      hmcsim_int_stat(&the_hmc, ROUTE_RQST_STALL_STAT));
  route_rsp_stall_stat->addData(
      hmcsim_int_stat(&the_hmc, ROUTE_RSP_STALL_STAT));
  ...
  // power & thermal stats
  link_phy_power_stat->addData(
      hmcsim_float_stat(&the_hmc, LINK_PHY_POWER_STAT) - s_link_phy_power);
  link_local_route_power_stat->addData(
      hmcsim_float_stat(&the_hmc, LINK_LOCAL_ROUTE_POWER_STAT) -
      s_link_local_route_power);
  link_remote_route_power_stat->addData(
      hmcsim_float_stat(&the_hmc, LINK_REMOTE_ROUTE_POWER_STAT) -
      s_link_remote_route_power);
  xbar_rqst_slot_power_stat->addData(
      hmcsim_float_stat(&the_hmc, XBAR_RQST_SLOT_POWER_STAT) -
      s_xbar_rqst_slot_power);
  xbar_rsp_slot_power_stat->addData(
      hmcsim_float_stat(&the_hmc, XBAR_RSP_SLOT_POWER_STAT) -
      s_xbar_rsp_slot_power);
  xbar_route_extern_power_stat->addData(
      hmcsim_float_stat(&the_hmc, XBAR_ROUTE_EXTERN_POWER_STAT) -
      s_xbar_route_extern_power);
  vault_rqst_slot_power_stat->addData(
      hmcsim_float_stat(&the_hmc, VAULT_RQST_SLOT_POWER_STAT) -
      s_vault_rqst_slot_power);
  vault_rsp_slot_power_stat->addData(
      hmcsim_float_stat(&the_hmc, VAULT_RSP_SLOT_POWER_STAT) -
      s_vault_rsp_slot_power);
  vault_ctrl_power_stat->addData(
      hmcsim_float_stat(&the_hmc, VAULT_CTRL_POWER_STAT) - s_vault_ctrl_power);
  row_access_power_stat->addData(
      hmcsim_float_stat(&the_hmc, ROW_ACCESS_POWER_STAT) - s_row_access_power);

  link_phy_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, LINK_PHY_THERM_STAT) - s_link_phy_therm);
  link_local_route_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, LINK_LOCAL_ROUTE_THERM_STAT) -
      s_link_local_route_therm);
  link_remote_route_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, LINK_REMOTE_ROUTE_THERM_STAT) -
      s_link_remote_route_therm);
  xbar_rqst_slot_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, XBAR_RQST_SLOT_THERM_STAT) -
      s_xbar_rqst_slot_therm);
  xbar_rsp_slot_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, XBAR_RSP_SLOT_THERM_STAT) -
      s_xbar_rsp_slot_therm);
  xbar_route_extern_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, XBAR_ROUTE_EXTERN_THERM_STAT) -
      s_xbar_route_extern_therm);
  vault_rqst_slot_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, VAULT_RQST_SLOT_THERM_STAT) -
      s_vault_rqst_slot_therm);
  vault_rsp_slot_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, VAULT_RSP_SLOT_THERM_STAT) -
      s_vault_rsp_slot_therm);
  vault_ctrl_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, VAULT_CTRL_THERM_STAT) - s_vault_ctrl_therm);
  row_access_therm_stat->addData(
      hmcsim_float_stat(&the_hmc, ROW_ACCESS_THERM_STAT) - s_row_access_therm);
...
void GOBLINHMCSimBackend::processResponses() {
  ...
  for (uint32_t i = 0; i < (uint32_t)the_hmc.num_links; ++i) {
    ...
    while (rc != HMC_STALL) {
      ...
      if (HMC_OK == rc) {
        ...
        int decode_rc = hmcsim_decode_memresponse(
            &the_hmc, &hmc_packet[0], &resp_head, &resp_tail, &resp_type,
            &resp_length, &resp_tag, &resp_return_tag, &resp_src_link,
            &resp_rrp, &resp_frp, &resp_seq, &resp_dinv, &resp_errstat,
            &resp_rtc, &resp_crc);

        if (HMC_OK == decode_rc) {
          output->verbose(
              CALL_INFO, 4, 0,
              "Successfully decoded an HMC memory response for tag: %" PRIu16
              "\n",
              resp_tag);

          std::map<uint16_t, HMCSimBackEndReq *>::iterator locate_tag =
              tag_req_map.find(resp_tag);

          if (locate_tag == tag_req_map.end()) {
            output->fatal(CALL_INFO, -1,
                          "Unable to find tag: %" PRIu16
            ...
            output->verbose(
                CALL_INFO, 4, 0,
                "Matched tag %" PRIu16 " to request for address: 0x%" PRIx64
                ", processing time: %" PRIu64 "ns\n",
                resp_tag, matchedReq->getAddr(),
                getCurrentSimTimeNano() - matchedReq->getStartTime());

            // Pass back to the controller to be handled, HMC sim is finished
            // with it
            handleMemResponse(matchedReq->getRequest(), flags);

            // Clear element from our map, it has been processed so no longer
            // needed
          ...
          // Should this situation happen? We have pulled the request now we
          // want it decoded, if it stalls is it lost?
          output->verbose(CALL_INFO, 8, 0,
                          "Attempted to decode an HMC memory request but HMC "
                          "returns stall.\n");
        } else {
          fflush(hmc_trace_file_handle);
          output->fatal(CALL_INFO, -1,
                        "Error: call to decode an HMC sim memory response "
                        "returned an error code (code=%d, possible tag=%" PRIu16
                        ")\n",
                        decode_rc, resp_tag);
        }
      } else if (HMC_STALL == rc) {
        output->verbose(CALL_INFO, 8, 0,
                        "Call to HMC simulator recv returns a stall, no "
                        "requests in flight.\n");
      } else {
        output->fatal(
            CALL_INFO, -1,
  ...
  for (it = tag_req_map.begin(); it != tag_req_map.end();) {
    ...
    if (!matchedReq->hasResponse()) {
      ...
      output->verbose(CALL_INFO, 4, 0,
                      "Handling posted memory response for tag: %" PRIu16 "\n",
                      resp_tag);
      handleMemResponse(matchedReq->getRequest(), flags);
...
void GOBLINHMCSimBackend::registerStatistics() {
  ...
  link_remote_route_power_stat =
      registerStatistic<float>("LinkRemoteRoutePower");
  ...
  xbar_route_extern_power_stat =
      registerStatistic<float>("XbarRouteExternPower");
  ...
  link_remote_route_therm_stat =
      registerStatistic<float>("LinkRemoteRouteTherm");
  ...
  xbar_route_extern_therm_stat =
      registerStatistic<float>("XbarRouteExternTherm");
...
void GOBLINHMCSimBackend::printPendingRequests() {
  ...
  for (pending_itr = tag_req_map.begin(); pending_itr != tag_req_map.end();
       pending_itr++) {
    output->verbose(CALL_INFO, 8, 0,
                    "Tag: %8" PRIu16 " for address 0X%" PRIx64 "\n",
