...
public:
  ...
  virtual void setNumChannels(unsigned int num) { m_numChannels = num; }

  virtual void setNumRanks(unsigned int num) { m_numRanks = num; }

  virtual void setNumBanks(unsigned int num) { m_numBanks = num; }
...
public:
  ...
  SST_ELI_REGISTER_MODULE(SimpleAddrMapper, "memHierarchy", "simpleAddrMapper",
                          SST_ELI_ELEMENT_VERSION(1, 0, 0),
                          "Simple address mapper",
                          "SST::MemHierarchy::AddrMapper")

  /* Begin class definition */
  SimpleAddrMapper(Params &params)
      : AddrMapper(), m_baseShift(6), m_columnShift(8) {}

  virtual void setNumChannels(unsigned int num) {
    if (!isPowerOfTwo(num)) {
      Output output("", 1, 0, Output::STDOUT);
      output.fatal(CALL_INFO, -1,
                   "SimpleAddrMapper, Error: memHierarchy.simpleAddrMapper "
                   "does not support non-power-of-2 channels...use "
                   "memHierarchy.roundRobinAddrMapper instead\n");
    }
    m_numChannels = num;
  }
  ...
  virtual void setNumRanks(unsigned int num) {
    if (!isPowerOfTwo(num)) {
      ...
      output.fatal(CALL_INFO, -1,
                   "SimpleAddrMapper, Error: memHierarchy.simpleAddrMapper "
                   "does not support non-power-of-2 ranks...use "
                   "memHierarchy.roundRobinAddrMapper instead\n");
    }
    m_numRanks = num;
  }
  ...
  virtual void setNumBanks(unsigned int num) {
    if (!isPowerOfTwo(num)) {
      ...
      output.fatal(CALL_INFO, -1,
                   "SimpleAddrMapper, Error: memHierarchy.simpleAddrMapper "
                   "does not support non-power-of-2 banks...use "
                   "memHierarchy.roundRobinAddrMapper instead\n");
    }
    m_numBanks = num;
  }
  ...
  int getChannel(Addr addr) { return (addr >> channelShift()) & channelMask(); }

  int getRank(Addr addr) { return (addr >> rankShift()) & rankMask(); }

  int getBank(Addr addr) { return (addr >> bankShift()) & bankMask(); }

  int getRow(Addr addr) { return addr >> rowShift(); }
...
public:
  /* Element Library Info */
  SST_ELI_REGISTER_MODULE(RoundRobinAddrMapper, "memHierarchy",
                          "roundRobinAddrMapper",
                          SST_ELI_ELEMENT_VERSION(1, 0, 0),
                          "Round-robin address mapper",
                          "SST::MemHierarchy::AddrMapper")

  SST_ELI_DOCUMENT_PARAMS(
      {"interleave_size",
       "(string) Granularity of interleaving in bytes (B). SI ok.", "64B"},
  ...
  /* Generic for any #ranks, channels, banks. interleave_size must be power of
   * 2, same as row/interleave_size */
  RoundRobinAddrMapper(Params &params) : AddrMapper() {
    UnitAlgebra ilSize = UnitAlgebra(params.find<std::string>(
        "interleave_size", "64B")); /* Default = 64B line interleaving */
    UnitAlgebra rowSize =
        UnitAlgebra(params.find<std::string>("row_size", "1KiB"));
    Output output("", 1, 0, Output::STDOUT);

    if (!ilSize.hasUnits("B")) {
      output.fatal(CALL_INFO, -1,
                   "RoundRobinAddrMapper, Invalid param - interleave_size: "
                   "must have units of 'B'. SI ok.\n");
    }
    if (!rowSize.hasUnits("B")) {
      output.fatal(CALL_INFO, -1,
                   "RoundRobinAddrMapper, Invalid param - row_size: must have "
                   "units of 'B'. SI ok.\n");
    }
    ...
    if (!isPowerOfTwo(rowSize.getRoundedValue())) {
      output.fatal(CALL_INFO, -1,
                   "RoundRobinAddrMapper, Invalid param - row_size: must be a "
                   "power-of-2.\n");
    }
    m_baseShift = log2Of(ilSize.getRoundedValue());
    m_rowDivider = m_numChannels * m_numRanks * m_numBanks *
                   (rowSize.getRoundedValue() / ilSize.getRoundedValue());
    m_rankDivider = m_numChannels * m_numBanks;
  }

  virtual void setNumChannels(unsigned int num) {
    m_rowDivider =
        m_rowDivider * num / m_numChannels; /* Swap num for m_numChannels */
    m_numChannels = num;
    m_rankDivider = m_numChannels * m_numBanks;
  }
  ...
  int getChannel(Addr addr) { return (addr >> m_baseShift) % m_numChannels; }
  ...
  int getRow(Addr addr) { return (addr >> m_baseShift) / m_rowDivider; }
...
public:
  /* Element Library Info */
  SST_ELI_REGISTER_MODULE(SandyBridgeAddrMapper, "memHierarchy",
  ...
  virtual void setNumChannels(unsigned int num) {
    if (num != 1) {
      ...
      output.fatal(
          CALL_INFO, -1,
          "SandyBridgeAddrMapper, Error: memHierarchy.sandyBridgeAddrMapper "
          "does not support multiple channels\n");
    }
    m_numChannels = num;
  }
  ...
  int getBank(Addr addr) { return ((addr >> 14) & 7) ^ (getRow(addr) & 7); }
