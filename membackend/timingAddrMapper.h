...
/* Element Library Info */
                SST_ELI_REGISTER_MODULE(SimpleAddrMapper,
                "memHierarchy", "simpleAddrMapper", SST_ELI_ELEMENT_VERSION(1,0,0),
...
/* Begin class definition */
                ...
                virtual void setNumChannels(unsigned int num) {
                    if (!isPowerOfTwo(num)) {
                        ...
                        output.fatal(CALL_INFO, -1,
                                     "SimpleAddrMapper, Error: memHierarchy.simpleAddrMapper does not support non-power-of-2 channels...use memHierarchy.roundRobinAddrMapper instead\n");
                    }
                    m_numChannels = num;
                }
                ...
                virtual void setNumRanks(unsigned int num) {
                    if (!isPowerOfTwo(num)) {
                        Output output("", 1, 0, Output::STDOUT);
                        output.fatal(CALL_INFO, -1,
                                     "SimpleAddrMapper, Error: memHierarchy.simpleAddrMapper does not support non-power-of-2 ranks...use memHierarchy.roundRobinAddrMapper instead\n");
                    }
                    m_numRanks = num;
                }
                ...
                virtual void setNumBanks(unsigned int num) {
                    if (!isPowerOfTwo(num)) {
                        Output output("", 1, 0, Output::STDOUT);
                        output.fatal(CALL_INFO, -1,
                                     "SimpleAddrMapper, Error: memHierarchy.simpleAddrMapper does not support non-power-of-2 banks...use memHierarchy.roundRobinAddrMapper instead\n");
                    }
                    m_numBanks = num;
                }
            ...
            public:
                /* Element Library Info */
                SST_ELI_REGISTER_MODULE(RoundRobinAddrMapper,
                "memHierarchy", "roundRobinAddrMapper", SST_ELI_ELEMENT_VERSION(1,0,0),
                ...
                RoundRobinAddrMapper(Params &params) : AddrMapper() {
                    UnitAlgebra ilSize = UnitAlgebra(params.find<std::string>("interleave_size",
                                                                              "64B")); /* Default = 64B line interleaving */
                    UnitAlgebra rowSize = UnitAlgebra(params.find<std::string>("row_size", "1KiB"));
                    Output output("", 1, 0, Output::STDOUT);

                    if (!ilSize.hasUnits("B")) {
                        output.fatal(CALL_INFO, -1,
                                     "RoundRobinAddrMapper, Invalid param - interleave_size: must have units of 'B'. SI ok.\n");
                    }
                    if (!rowSize.hasUnits("B")) {
                        output.fatal(CALL_INFO, -1,
                                     "RoundRobinAddrMapper, Invalid param - row_size: must have units of 'B'. SI ok.\n");
                    }
                    if (!isPowerOfTwo(ilSize.getRoundedValue())) {
                        output.fatal(CALL_INFO, -1,
                                     "RoundRobinAddrMapper, Invalid param - interleave_size: must be a power-of-2.\n");
                    }
                    if (!isPowerOfTwo(rowSize.getRoundedValue())) {
                        output.fatal(CALL_INFO, -1,
                                     "RoundRobinAddrMapper, Invalid param - row_size: must be a power-of-2.\n");
                    }
            ...
            public:
/* Element Library Info */
                SST_ELI_REGISTER_MODULE(SandyBridgeAddrMapper,
                "memHierarchy", "sandyBridgeAddrMapper", SST_ELI_ELEMENT_VERSION(1,0,0),
...
/* Begin class definition */
                ...
                virtual void setNumChannels(unsigned int num) {
                    if (num != 1) {
                        Output output("", 1, 0, Output::STDOUT);
                        output.fatal(CALL_INFO, -1,
                                     "SandyBridgeAddrMapper, Error: memHierarchy.sandyBridgeAddrMapper does not support multiple channels\n");
                    }
                    m_numChannels = num;
                }
