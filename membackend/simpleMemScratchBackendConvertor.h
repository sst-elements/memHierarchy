...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      SimpleMemScratchBackendConvertor, "memHierarchy",
      "simpleMemScratchBackendConvertor", SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Convert a MemEventBase to a base MemBacked but uses a different "
      "interface than MemBackendConvertor",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"request_width",
       "(uint) Max size of a request that can be accepted by the memory "
       "controller",
       "64"},
      {"backend",
  ...
  SST_ELI_DOCUMENT_STATISTICS(
      {"cycles_with_issue", "Total cycles with successful issue to backend",
       "cycles", 1},
      {"cycles_attempted_issue_but_rejected",
       "Total cycles where an issue was attempted but backed rejected it",
       "cycles", 1},
