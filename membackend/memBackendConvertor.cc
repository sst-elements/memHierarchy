...
MemBackendConvertor::MemBackendConvertor(ComponentId_t id, Params &params,
  ...
  m_backend->setGetRequestorHandler(
      std::bind(&MemBackendConvertor::getRequestor, this, _1));
 ...
 */
void MemBackendConvertor::turnClockOff() { m_clockOn = false; }
...
void MemBackendConvertor::doResponse(ReqId reqId, uint32_t flags) {
  ...
  if (req->isDone()) {
    ...
    if (!req->isMemEv()) {
      ...
      delete info; // NOTE move this if needed, currently memController doesn't
                   // need it
      ...
      sendResponse(event->getID(), flags); // Needs to occur before a flush is
                                           // completed since flush is dependent
      ...
      if (m_dependentRequests.find(evID) != m_dependentRequests.end()) {
        std::set<MemEvent *, memEventCmp> flushes =
            m_dependentRequests.find(evID)->second;
        ...
        for (std::set<MemEvent *, memEventCmp>::iterator it = flushes.begin();
          ...
          if ((m_waitingFlushes.find(*it)->second).empty()) {
            MemEvent *flush = *it;
            sendResponse(flush->getID(),
                         (flush->getFlags() | MemEvent::F_SUCCESS));
...
size_t MemBackendConvertor::getMemSize() { return m_backend->getMemSize(); }
