 ...
 * latency differences between accessing open/closed rows and implements
 * banking. The latencies are as follows: Correct row already open: tCAS No row
 * open: tRCD + tCAS Wrong row open: tRP + tRCD + tCAS
 *
 ...
 *  SimpleDRAM relies on external (memController or other backend) request
 * limiting. SimpleDRAM will block banks but will accept multiple requests per
 * cycle if they don't have a bank conflict. SimpleDRAM assumes transfer
 * granularity is a cache line -> this can be changed by changing
 * 'cache_line_size_in_bytes' SimpleDRAM does not model bus contention back to
 * the controller -> possible for multiple requests to return in the same cycle
 *
 *  TODO
 *   * Randomize bank bits?
...
SimpleDRAM::SimpleDRAM(ComponentId_t id, Params &params)
...
void SimpleDRAM::build(Params &params) {
  ...
  UnitAlgebra lineSize(
      params.find<std::string>("bank_interleave_granularity", "64B"));
  UnitAlgebra rowSize(params.find<std::string>("row_size", "8KiB"));
  bool found = false;
  std::string policyStr =
      params.find<std::string>("row_policy", "closed", found);
  ...
  if (policyStr != "closed" && policyStr != "open") {
    output->fatal(CALL_INFO, -1,
                  "Invalid param(%s): row_policy - must be 'closed' or 'open'. "
                  "You specified '%s'.\n",
  ...
  if (!isPowerOfTwo(banks)) {
    output->fatal(CALL_INFO, -1,
                  "Invalid param(%s): banks - must be a power of two. You "
                  "specified %" PRIu64 ".\n",
                  getName().c_str(), banks);
  }
  bankMask = banks - 1;
  ...
  if (!(lineSize.hasUnits("B"))) {
    output->fatal(CALL_INFO, -1,
                  "Invalid param(%s): cache_line_size_in_bytes - must have "
                  "units of 'B' (bytes). The units you specified were '%s'.\n",
                  getName().c_str(), lineSize.toString().c_str());
  }
  if (!isPowerOfTwo(lineSize.getRoundedValue())) {
    output->fatal(CALL_INFO, -1,
                  "Invalid param(%s): cache_line_size_in_bytes - must be a "
                  "power of two. You specified %s.\n",
                  getName().c_str(), lineSize.toString().c_str());
  }
  ...
  if (!(rowSize.hasUnits("B"))) {
    output->fatal(CALL_INFO, -1,
                  "Invalid param(%s): row_size - must have units of 'B' "
                  "(bytes). You specified %s.\n",
                  getName().c_str(), rowSize.toString().c_str());
  }
  if (!isPowerOfTwo(rowSize.getRoundedValue())) {
    output->fatal(CALL_INFO, -1,
                  "Invalid param(%s): row_size - must be a power of two. You "
                  "specified %s.\n",
                  getName().c_str(), rowSize.toString().c_str());
  }
  ...
  self_link = configureSelfLink(
      "Self", cycTime,
      new Event::Handler<SimpleDRAM>(this, &SimpleDRAM::handleSelfEvent));
...
bool SimpleDRAM::issueRequest(ReqId reqId, Addr addr, bool isWrite,
                              unsigned numBytes) {
  ...
  output->debug(
      _L10_,
      "SimpleDRAM (%s) received request for address %" PRIx64
      " which maps to bank: %d, row: %d. Bank status: %s, open row is %d\n",
      getName().c_str(), addr, bank, row, (busy[bank] ? "busy" : "idle"),
      openRow[bank]);
#endif
