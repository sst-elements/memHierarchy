...
Messier::Messier(ComponentId_t id, Params &params)
...
void Messier::build(Params &params) {
  std::string access_time =
      "1ns"; // params.find<std::string>("access_time", "1 ns");
  nvm_link = configureLink(
      "nvm_link", access_time,
      new Event::Handler<Messier>(this, &Messier::handleMessierResp));

  //	using std::placeholders::_1;
  //	using std::placeholders::_2;
  //	static_cast<MessierBackend*>(m_backend)->setResponseHandler( std::bind(
  //&Messier::handleMemResponse, this, _1,_2 ) );
  // output->init("VaultSimMemory[@p:@l]: ", 10, 0, Output::STDOUT);
}

bool Messier::issueRequest(ReqId reqId, Addr addr, bool isWrite,
                           unsigned numBytes) {
  ...
  nvm_link->send(new SST::MessierComponent::MemReqEvent(reqId, addr, isWrite,
                                                        numBytes, 0));
  return true;
}
...
void Messier::handleMessierResp(SST::Event *event) {
  ...
  MessierComponent::MemRespEvent *ev =
      dynamic_cast<MessierComponent::MemRespEvent *>(event);
  ...
  if (ev) {
    if (outToNVM.find(ev->getReqId()) != outToNVM.end()) {
      ...
      ; // output->fatal(CALL_INFO, -1, "Could not match incoming request from
        // cubes\n");
    }
  } else {
