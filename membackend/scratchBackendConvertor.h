...
#define SCRATCHBACKENDCONVERTOR_ELI_SLOTS                                      \
  { "backend", "Backend memory model", "SST::MemHierarchy::MemBackend" }
  ...
  public:
    MemReq(MemEvent *event, uint32_t reqId)
        : m_event(event), m_reqId(reqId), m_offset(0), m_numReq(0) {}
    ~MemReq() { delete m_event; }
