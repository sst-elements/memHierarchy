...
public:
  BackingMMAP(std::string memoryFile, size_t size, size_t offset = 0)
    ...
    m_buffer =
        (uint8_t *)mmap(NULL, size, PROT_READ | PROT_WRITE, flags, m_fd, 0);
  ...
  void set(Addr addr, uint8_t value) { m_buffer[addr - m_offset] = value; }
  ...
  uint8_t get(Addr addr) { return m_buffer[addr - m_offset]; }
