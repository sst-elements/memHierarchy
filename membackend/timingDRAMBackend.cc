...
TimingDRAM::TimingDRAM(ComponentId_t id, Params &params)
    : SimpleMemBackend(id, params), m_cycle(0) {
  build(params);
}
...
void TimingDRAM::build(Params &params) {
  ...
  std::string addrMapper =
      params.find<std::string>("addrMapper", "memHierarchy.simpleAddrMapper");
  ...
  for (unsigned i = 0; i < numChannels; i++) {
    ...
    m_channels.push_back(loadComponentExtension<Channel>(
        std::bind(&TimingDRAM::handleResponse, this, _1), tmpParams, id, i,
        output, m_mapper));
  }
}

bool TimingDRAM::issueRequest(ReqId id, Addr addr, bool isWrite,
                              unsigned numBytes) {
  ...
  if (ret) {
    output->verbose(CALL_INFO, 2, DBG_MASK,
                    "chan=%d reqId=%" PRIu64 " addr=%#" PRIx64 "\n", chan, id,
                    addr);
  } else {
    output->verbose(CALL_INFO, 5, DBG_MASK,
                    "chan=%d reqId=%" PRIu64 " addr=%#" PRIx64 " failed\n",
                    chan, id, addr);
  }
  return ret;
}
...
TimingDRAM::Channel::Channel(ComponentId_t id,
                             std::function<void(ReqId)> handler, Params &params,
                             unsigned mc, unsigned myNum, Output *output,
                             AddrMapper *mapper)
    : ComponentExtension(id), m_responseHandler(handler), m_output(output),
      m_mapper(mapper), m_nextRankUp(0), m_dataBusAvailCycle(0) {
  std::ostringstream tmp;
  ...
  if (m_printConfig)
    m_printConfig = params.find<bool>("printconfig", true);
  if (m_printConfig) {
    m_output->verbosePrefix(prefix(), CALL_INFO, 1, DBG_MASK,
                            "max pending trans: %d\n", m_maxPendingTrans);
    m_output->verbosePrefix(prefix(), CALL_INFO, 1, DBG_MASK,
                            "number of ranks:   %d\n", numRanks);
    m_printConfig = false;
  ...
  for (unsigned i = 0; i < numRanks; i++) {
    m_ranks.push_back(
        loadComponentExtension<Rank>(tmpParams, mc, myNum, i, output, mapper));
  }
}
...
void TimingDRAM::Channel::clock(SimTime_t cycle) {
  ...
  while (iter != m_issuedCmds.end()) {
    if ((*iter)->isDone(cycle)) {
      ...
      if (is_debug)
        m_output->verbosePrefix(prefix(), CALL_INFO, 2, DBG_MASK,
                                "cycle=%" PRIu64
                                " retire %s for rank=%d bank=%d row=%d\n",
                                cycle, cmd->getName().c_str(), cmd->getRank(),
                                cmd->getBank(), cmd->getRow());
  ...
  if (!m_retiredTrans.empty()) {
    if (is_debug)
      m_output->verbosePrefix(
          prefix(), CALL_INFO, 3, DBG_MASK,
          "send response: reqId=%" PRIu64 " bank=%d addr=%#" PRIx64
          ", createTime=%" PRIu64 "\n",
          m_retiredTrans.front()->id, m_retiredTrans.front()->bank,
          m_retiredTrans.front()->addr, m_retiredTrans.front()->createTime);
  ...
  if (cmd) {
    if (is_debug)
      m_output->verbosePrefix(prefix(), CALL_INFO, 2, DBG_MASK,
                              "cycle=%" PRIu64
                              " issue %s for rank=%d bank=%d row=%d\n",
                              cycle, cmd->getName().c_str(), cmd->getRank(),
                              cmd->getBank(), cmd->getRow());
...
TimingDRAM::Cmd *TimingDRAM::Channel::popCmd(SimTime_t cycle,
                                             SimTime_t dataBusAvailCycle) {
  Cmd *cmd = nullptr;
    ...
    if (m_ranks[current]->hasActiveBanks()) {
      ...
      if (cmd) {
        ...
        if (current == m_nextRankUp) {
          ...
          if (is_debug)
            m_output->verbosePrefix(prefix(), CALL_INFO, 3, DBG_MASK,
                                    "rank %d next up\n", m_nextRankUp);
        }
...
TimingDRAM::Rank::Rank(ComponentId_t id, Params &params, unsigned mc,
                       unsigned chan, unsigned myNum, Output *output,
                       AddrMapper *mapper)
    : ComponentExtension(id), m_output(output), m_mapper(mapper),
      m_nextBankUp(0) {
  std::ostringstream tmp;
  tmp << "@t:TimingDRAM:Rank:@p():@l:mc=" << mc << ":chan=" << chan
      << ":rank=" << myNum << ": ";
  m_pre = tmp.str();
  ...
  for (unsigned i = 0; i < banks; i++) {
    m_banks.push_back(
        loadComponentExtension<Bank>(tmpParams, mc, chan, myNum, i, output));
  }
}

TimingDRAM::Cmd *TimingDRAM::Rank::popCmd(SimTime_t cycle,
                                          SimTime_t dataBusAvailCycle) {
  if (is_debug)
    m_output->verbosePrefix(prefix(), CALL_INFO, 5, DBG_MASK, "\n");
...
TimingDRAM::Bank::Bank(ComponentId_t id, Params &params, unsigned mc,
                       unsigned chan, unsigned rank, unsigned myNum,
                       Output *output)
    : ComponentExtension(id), m_output(output), m_lastCmd(nullptr),
      m_bank(myNum), m_rank(rank), m_row(-1) {
  std::ostringstream tmp;
  tmp << "@t:TimingDRAM:Bank:@p():@l:mc=" << mc << ":chan=" << chan
      << ":rank=" << rank << ":bank=" << myNum << ": ";
  m_pre = tmp.str();
  ...
  std::string name =
      params.find<std::string>("transactionQ", "memHierarchy.fifoTransactionQ");
  Params tmpParams = params.find_prefix_params("transactionQ.");
  m_transQ = loadAnonymousSubComponent<TransactionQ>(
      name, "transactionQ", 0, ComponentInfo::INSERT_STATS, tmpParams);

  std::string ppName =
      params.find<std::string>("pagePolicy", "memHierarchy.simplePagePolicy");
  tmpParams = params.find_prefix_params("pagePolicy.");
  m_pagePolicy = loadAnonymousSubComponent<PagePolicy>(
      ppName, "pagePolicy", 0, ComponentInfo::INSERT_STATS, tmpParams);
...
TimingDRAM::Cmd *TimingDRAM::Bank::popCmd(SimTime_t cycle,
                                          SimTime_t dataBusAvailCycle) {
  if (is_debug)
...
void TimingDRAM::Bank::update(SimTime_t current) {
  if (nullptr == m_lastCmd && m_row != -1 &&
      m_pagePolicy->shouldClose(current)) {
  ...
  if (is_debug)
    m_output->verbosePrefix(prefix(), CALL_INFO, 2, DBG_MASK,
                            "addr=%#" PRIx64
                            " current row=%d trans row=%d, time=%" PRIu64 "\n",
