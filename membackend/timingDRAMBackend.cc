...
TimingDRAM::TimingDRAM(Component *comp, Params &params) : SimpleMemBackend(comp, params),
                                                          m_cycle(0) { build(params); }

TimingDRAM::TimingDRAM(ComponentId_t id, Params &params) : SimpleMemBackend(id, params),
                                                           m_cycle(0) { build(params); }
...
void TimingDRAM::build(Params &params) {
    ...
    std::string addrMapper = params.find<std::string>("addrMapper",
                                                      "memHierarchy.simpleAddrMapper");
    ...
    for (unsigned i = 0; i < numChannels; i++) {
        ...
        m_channels.push_back(
            loadComponentExtension<Channel>(std::bind(&TimingDRAM::handleResponse, this, _1),
                                            tmpParams, id, i, output, m_mapper));
    }
}
...
bool TimingDRAM::issueRequest(ReqId id, Addr addr, bool isWrite, unsigned numBytes) {
    ...
    if (ret) {
        output->verbose(CALL_INFO, 2, DBG_MASK, "chan=%d reqId=%"
        PRIu64
        " addr=%#"
        PRIx64
        "\n", chan, id, addr);
    } else {
        output->verbose(CALL_INFO, 5, DBG_MASK, "chan=%d reqId=%"
        PRIu64
        " addr=%#"
        PRIx64
        " failed\n", chan, id, addr);
...
bool TimingDRAM::clock(Cycle_t cycle) {
    output->verbose(CALL_INFO, 5, DBG_MASK, "cycle %"
    PRIu64
    "\n", m_cycle);
...
TimingDRAM::Channel::Channel(ComponentId_t id, std::function<void(ReqId)> handler, Params &params,
                             unsigned mc, unsigned myNum, Output *output, AddrMapper *mapper) :
    ComponentExtension(id), m_responseHandler(handler), m_output(output), m_mapper(mapper),
    m_nextRankUp(0), m_dataBusAvailCycle(0) {
    std::ostringstream tmp;
...
void TimingDRAM::Channel::clock(SimTime_t cycle) {
    m_output->verbosePrefix(prefix(), CALL_INFO, 5, DBG_MASK, "cycle %"
    PRIu64
    "\n", cycle);
    ...
    while (iter != m_issuedCmds.end()) {
        if ((*iter)->isDone(cycle)) {
            ...
            m_output->verbosePrefix(prefix(), CALL_INFO, 2, DBG_MASK, "cycle=%"
            PRIu64
    ...
    if (!m_retiredTrans.empty()) {
        m_output->verbosePrefix(prefix(), CALL_INFO, 3, DBG_MASK, "send response: reqId=%"
        PRIu64
        " bank=%d addr=%#"
        PRIx64
        ", createTime=%"
...
TimingDRAM::Rank::Rank(ComponentId_t id, Params &params, unsigned mc, unsigned chan, unsigned myNum,
                       Output *output, AddrMapper *mapper) :
...
TimingDRAM::Bank::Bank(ComponentId_t id, Params &params, unsigned mc, unsigned chan, unsigned rank,
                       unsigned myNum, Output *output) :
    ComponentExtension(id), m_output(output), m_lastCmd(nullptr), m_bank(myNum), m_rank(rank),
    m_row(-1) {
    std::ostringstream tmp;
    tmp << "@t:TimingDRAM:Bank:@p():@l:mc=" << mc << ":chan=" << chan << ":rank=" << rank
        << ":bank=" << myNum << ": ";
    m_pre = tmp.str();
    ...
    m_transQ = loadAnonymousSubComponent<TransactionQ>(name, "transactionQ", 0,
                                                       ComponentInfo::INSERT_STATS, tmpParams);
    ...
    m_pagePolicy = loadAnonymousSubComponent<PagePolicy>(ppName, "pagePolicy", 0,
                                                         ComponentInfo::INSERT_STATS, tmpParams);
...
void TimingDRAM::Bank::update(SimTime_t current) {
    ...
    m_output->verbosePrefix(prefix(), CALL_INFO, 2, DBG_MASK, "addr=%#"
    PRIx64
    " current row=%d trans row=%d, time=%"
    PRIu64
    "\n",
