...
pagedMultiMemory::pagedMultiMemory(ComponentId_t id, Params &params)
    : DRAMSimMemory(id, params), pagesInFast(0), lastMin(0) {
  build(params);
}
...
void pagedMultiMemory::build(Params &params) {
  ...
  self_link = configureSelfLink("Self", access,
                                new Event::Handler<pagedMultiMemory>(
                                    this, &pagedMultiMemory::handleSelfEvent));
  ...
  registerClock(clock_freq, new Clock::Handler<pagedMultiMemory>(
                                this, &pagedMultiMemory::quantaClock));
  ...
  if (modelSwaps) {
    ...
    readDataCB =
        new DRAMSim::Callback<pagedMultiMemory, void, unsigned int, uint64_t,
                              uint64_t>(this, &pagedMultiMemory::dramSimDone);
    writeDataCB =
        new DRAMSim::Callback<pagedMultiMemory, void, unsigned int, uint64_t,
                              uint64_t>(this, &pagedMultiMemory::dramSimDone);
...
void pagedMultiMemory::do_FIFO_LRU(pageInfo &page, bool &inFast,
                                   bool &swapping) {
  swapping = 0;
  if (0 == page.inFast) {
    // not in fast
...
void pagedMultiMemory::do_LFU(Addr addr, pageInfo &page, bool &inFast,
                              bool &swapping) {
          ...
          for (auto p = pageMap.begin(); p != endP; ++p) {
            if ((p->second.inFast == 1) && (p->first != pageAddr)) {
              ...
              if ((p->second.touched < page.touched) &&
                  (p->second.swapDir ==
                   pageInfo::NONE)) { // make sure we don't bump someone in
                                      // motion
                found = 1;
...
bool pagedMultiMemory::issueRequest(ReqId id, Addr addr, bool isWrite,
                                    unsigned numBytes) {
...
bool pagedMultiMemory::clock(Cycle_t cycle) {
  ...
  while (!dramQ.empty()) {
    Req *req = dramQ.front();
    bool inserted = DRAMSimMemory::issueRequest((ReqId)req, req->addr,
...
void pagedMultiMemory::dramSimDone(unsigned int id, uint64_t addr,
                                   uint64_t clockcycle) {
