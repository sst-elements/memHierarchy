...
ScratchBackendConvertor::ScratchBackendConvertor(ComponentId_t id,
                                                 Params &params)
...
void ScratchBackendConvertor::build(Params &params) {
  m_dbg.init("", params.find<uint32_t>("debug_level", 0),
  ...
  if (!m_backend) {
    ...
    string backendName =
        params.find<std::string>("backend", "memHierarchy.simpleMem");
    Params backendParams = params.find_prefix_params("backend.");
    m_backend = loadAnonymousSubComponent<MemBackend>(
  ...
  m_backend->setGetRequestorHandler(
      std::bind(&ScratchBackendConvertor::getRequestor, this, _1));

  m_frontendRequestWidth = params.find<uint32_t>("request_width", 64);
  m_backendRequestWidth =
      static_cast<SimpleMemBackend *>(m_backend)->getRequestWidth();
  ...
  stat_cyclesAttemptIssueButRejected =
      registerStatistic<uint64_t>("cycles_attempted_issue_but_rejected");
...
void ScratchBackendConvertor::setCallbackHandler(
    std::function<void(Event::id_type)> respCB) {
...
void ScratchBackendConvertor::handleMemEvent(MemEvent *ev) {
  ...
  Debug(_L10_,
        "Creating MemReq. BaseAddr = %" PRIx64 ", Size: %" PRIu32 ", %s\n",
...
bool ScratchBackendConvertor::doResponse(ReqId reqId,
...
void ScratchBackendConvertor::finish(void) { m_backend->finish(); }

size_t ScratchBackendConvertor::getMemSize() { return m_backend->getMemSize(); }
