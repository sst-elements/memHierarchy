...
struct HBMpageInfo {
  ...
  void record(Addr addr, bool isWrite, const std::string &requestor,
              const bool collectStats, const uint64_t pAddr,
              const bool limitTouch) {
  ...
  HBMpageInfo()
      : pageAddr(0), touched(0), inFast(0), lastTouch(0), lastRef(0),
        scanLeng(0), pageDelay(0), swapDir(NONE), swapsOut(0) {
    for (int i = 0; i < LAST_CASE; ++i) {
      accPat[i] = 0;
    }
  }
...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      HBMpagedMultiMemory, "memHierarchy", "HBMpagedMultiMemory",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "HBM DRAMSim-driven memory timings with a fixed timing multi-level "
      "memory using paging",
      SST::MemHierarchy::SimpleMemBackend)

  SST_ELI_DOCUMENT_PARAMS(
      HBMDRAMSIMMEMORY_ELI_PARAMS,
      /* Own parameters */
      ...
      {"access_time", "Constant time memory access for \"fast\" memory",
       "35ns"},
  ...
  SST_ELI_DOCUMENT_STATISTICS(
      HBMDRAMSIMMEMORY_ELI_STATS,
      {"fast_hits", "Number of accesses that 'hit' a fast page", "count", 1},
      {"fast_swaps", "Number of pages swapped between 'fast' and 'slow' memory",
       "count", 1},
      {"fast_acc", "Number of total accesses to the memory backend", "count",
       1},
      {"t_pages", "Number of total pages", "count", 1},
      {"cant_swap",
       "Number of times a page could not be swapped in because no victim page "
       "could be found because all candidates were swapping",
       "count", 1},
      {"swap_delays",
       "Number of an access is delayed because the page is swapping", "count",
       1})
...
private:
  ...
  typedef enum {
    LFU,   // threshold + MFU addition, LFU replacement
  ...
  void printAccStats();
  queue<Req *> dramQ;
  void queueRequest(Req *r) { dramQ.push(r); }
...
public:
  ...
  public:
    void serialize_order(SST::Core::Serialization::serializer &ser) override {
      ...
      ser &req; // Cannot serialize pointers unless they are a serializable
                // object
    }
