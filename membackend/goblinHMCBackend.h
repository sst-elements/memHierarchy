...
public:
  ...
  MemBackend::ReqId getRequest() const { return req; }
  Addr getAddr() const { return addr; }
  bool hasResponse() const { return hasResp; }
...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      GOBLINHMCSimBackend, "memHierarchy", "goblinHMCSim",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "GOBLIN HMC Simulator driven memory timings",
      SST::MemHierarchy::ExtMemBackend)
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"device_count",
       "Sets the number of HMCs being simulated, default=1, max=8", "1"},
      {"link_count",
       "Sets the number of links being simulated, min=4, max=8, default=4",
       "4"},
      {"vault_count",
       "Sets the number of vaults being simulated, min=16, max=32, default=16",
       "16"},
      {"queue_depth",
       "Sets the depth of the HMC request queue, min=2, max=65536, default=2",
       "2"},
      ...
      {"dram_latency", "Sets the internal DRAM fetch latency in clock cycles",
       "2"},
#endif
      {"trace-banks",
       "Decides where tracing for memory banks is enabled, \"yes\" or \"no\", "
       "default=\"no\"",
       "no"},
      {"trace-queue",
       "Decides where tracing for queues is enabled, \"yes\" or \"no\", "
       "default=\"no\"",
       "no"},
      {"trace-cmds",
       "Decides where tracing for commands is enabled, \"yes\" or \"no\", "
       "default=\"no\"",
       "no"},
      {"trace-latency",
       "Decides where tracing for latency is enabled, \"yes\" or \"no\", "
       "default=\"no\"",
       "no"},
      {"trace-stalls",
       "Decides where tracing for memory stalls is enabled, \"yes\" or \"no\", "
       "default=\"no\"",
       "no"},
#ifdef HMC_TRACE_POWER
      {"trace-power",
       "Decides where tracing for memory power is enabled, \"yes\" or \"no\", "
       "default=\"no\"",
       "no"},
#endif
      {"tag_count",
       "Sets the number of inflight tags that can be pending at any point in "
       "time",
       "16"},
      {"capacity_per_device",
       "Sets the capacity of the device being simulated in GiB, min=2, max=8, "
       "default is 4",
       "4"},
      {"cmc-config", "Enables a CMC library command in HMCSim", "NONE"},
      {"cmd-map",
       "Maps an existing HMC or CMC command to the target command type",
       "NONE"})
  ...
  SST_ELI_DOCUMENT_STATISTICS(
      ...
      {"LinkLocalRoutePower", "HMC Link local quadrant route power",
       "milliwatts", 1},
      {"LinkRemoteRoutePower", "HMC Link remote quadrante route power",
       "milliwatts", 1},
      ...
      {"XbarRouteExternPower", "HMC Crossbar external route power",
       "milliwatts", 1},
      ...
      {"LinkLocalRouteTherm", "HMC Link local quadrant route thermal", "btus",
       1},
      {"LinkRemoteRouteTherm", "HMC Link remote quadrante route thermal",
       "btus", 1},
      {"XbarRqstSlotTherm", "HMC Crossbar request slot thermal", "btus", 1},
      {"XbarRspSlotTherm", "HMC Crossbar response slot thermal", "btus", 1},
      {"XbarRouteExternTherm", "HMC Crossbar external route thermal", "btus",
       1},
  ...
  ~GOBLINHMCSimBackend();
  bool issueRequest(ReqId, Addr, bool, std::vector<uint64_t>, uint32_t,
                    unsigned);
  bool issueCustomRequest(ReqId, Addr, uint32_t, std::vector<uint64_t>,
                          uint32_t, unsigned);
  void setup();
  void finish();
...
private:
  ...
  void splitStr(const string &s, char delim, vector<string> &v);
  ...
  bool issueMappedRequest(ReqId, Addr, bool, std::vector<uint64_t>, uint32_t,
                          unsigned, uint8_t, uint16_t, bool *);
