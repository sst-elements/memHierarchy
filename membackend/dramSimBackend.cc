...
DRAMSimMemory::DRAMSimMemory(ComponentId_t id, Params &params)
...
void DRAMSimMemory::build(Params &params) {
  ...
  if (ramSize.getRoundedValue() % (1024 * 1024) != 0) {
    output->fatal(
        CALL_INFO, -1,
        "For DRAMSim, backend.mem_size must be a multiple of 1MiB. Note: for "
        "units in base-10 use 'MB', for base-2 use 'MiB'. You specified '%s'\n",
  ...
  readDataCB =
      new DRAMSim::Callback<DRAMSimMemory, void, unsigned int, uint64_t,
                            uint64_t>(this, &DRAMSimMemory::dramSimDone);
  writeDataCB =
      new DRAMSim::Callback<DRAMSimMemory, void, unsigned int, uint64_t,
                            uint64_t>(this, &DRAMSimMemory::dramSimDone);
...
void DRAMSimMemory::finish() { memSystem->printStats(true); }

void DRAMSimMemory::dramSimDone(unsigned int id, uint64_t addr,
                                uint64_t clockcycle) {
