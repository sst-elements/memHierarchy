...
/* MemNIC Base class
 *  Base class to handle initialization and endpoint management for different
 * NICs
 */
class MemNICBase : public MemLinkBase {
public:
#define MEMNICBASE_ELI_PARAMS                                                  \
  MEMLINKBASE_ELI_PARAMS,                                                      \
      {"group",                                                                \
       "(int) Group ID. See params 'sources' and 'destinations'. If not "      \
       "specified, the parent component will guess.",                          \
       "1"},                                                                   \
      {"sources",                                                              \
       "(comma-separated list of ints) List of group IDs that serve as "       \
       "sources for this component. If not specified, defaults to 'group - "   \
       "1'.",                                                                  \
       "group-1"},                                                             \
      {"destinations",                                                         \
       "(comma-separated list of ints) List of group IDs that serve as "       \
       "destinations for this component. If not specified, defaults to "       \
       "'group + 1'.",                                                         \
       "group+1"}

  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED_API(SST::MemHierarchy::MemNICBase,
  ...
  virtual bool isSource(std::string str) { /* Note this is only used during init
                                              so doesn't need to be fast */
  ...
  virtual bool isDest(std::string str) { /* Note this is only used during init
                                            so doesn't need to be fast */
  ...
  virtual std::string findTargetDestination(Addr addr) {
    ...
    stringstream error;
    error << getName() + " (MemNICBase) cannot find a destination for address "
          << addr << endl;
...
protected:
  ...
  /* NIC initialization so that subclasses don't have to do this. Subclasses
   * should call this during init() */
  virtual void nicInit(SST::Interfaces::SimpleNetwork *linkcontrol,
                       unsigned int phase) {
    ...
    while (SST::Interfaces::SimpleNetwork::Request *req =
      ...
      if (imre) {
        ...
        networkAddressMap.insert(
            std::make_pair(imre->info.name, imre->info.addr));
        ...
        if (ev->getInitCmd() == MemEventInit::InitCommand::Region) {
          ...
          delete ev;
          delete mre;
        } else if ((ev->getCmd() == Command::NULLCMD &&
                    (isSource(mre->event->getSrc()) ||
                     isDest(mre->event->getSrc()))) ||
                   ev->getDst() == info.name) {
  ...
  virtual uint64_t lookupNetworkAddress(const std::string &dst) const {
    std::unordered_map<std::string, uint64_t>::const_iterator it =
        networkAddressMap.find(dst);
    if (it == networkAddressMap.end()) {
      dbg.fatal(CALL_INFO, -1,
                "%s (MemNICBase), Network address for destination '%s' not "
                "found in networkAddressMap.\n",
  ...
  size_t extractPacketHeaderSize(Params &params, std::string pname,
                                 std::string defsize = "8B") {
    UnitAlgebra size = UnitAlgebra(params.find<std::string>(pname, defsize));
    if (!size.hasUnits("B"))
      dbg.fatal(CALL_INFO, -1,
                "Invalid param(%s): %s - must have units of bytes (B). SI "
                "units OK. You specified '%s'\n.",
                getName().c_str(), pname.c_str(), size.toString().c_str());
    return size.getRoundedValue();
  }

  // Drain a send queue
  void drainQueue(std::queue<SST::Interfaces::SimpleNetwork::Request *> *queue,
                  SST::Interfaces::SimpleNetwork *linkcontrol) {
    while (!(queue->empty())) {
      SST::Interfaces::SimpleNetwork::Request *head = queue->front();
#ifdef __SST_DEBUG_OUTPUT__
      MemEventBase *ev =
          (static_cast<MemRtrEvent *>(head->inspectPayload()))->event;
      std::string debugEvStr = ev ? ev->getBriefString() : "";
      uint64_t dst = head->dest;
      bool doDebug = ev ? is_debug_event(ev) : false;
#endif
      if (linkcontrol->spaceToSend(0, head->size_in_bits) &&
          linkcontrol->send(head, 0)) {
#ifdef __SST_DEBUG_OUTPUT__
        if (!debugEvStr.empty() && doDebug) {
          dbg.debug(_L9_,
                    "%s (memNICBase), Sending message %s to dst addr %" PRIu64
                    "\n",
  ...
  MemRtrEvent *doRecv(SST::Interfaces::SimpleNetwork *linkcontrol) {
    ...
    if (req != nullptr) {
      ...
      if (mre->hasClientData()) {
        ...
        if (networkAddressMap.find(imre->info.name) ==
            networkAddressMap.end()) {
          dbg.fatal(CALL_INFO, -1,
                    "%s received information about previously unknown "
                    "endpoint. This case is not handled. Endpoint name: %s\n",
  ...
  std::unordered_map<std::string, uint64_t>
      networkAddressMap; // Map of name -> address for each network endpoint
  ...
  std::queue<SST::Interfaces::SimpleNetwork::Request *>
      initSendQueue; // Queue of events waiting to be sent after network
                     // (linkcontrol) initializes

  // Other parameters
  std::unordered_set<uint32_t> sourceIDs,
      destIDs; // IDs which this endpoint cares about

private:
  void build(Params &params) {
    // Get source/destination parameters
    // Each NIC has a group ID and talks to those with IDs in sources and
    // destinations If no source/destination provided, source = group ID - 1,
    // destination = group ID + 1
    bool found;
    info.id = params.find<uint32_t>("group", 0, found);
    if (!found) {
      dbg.fatal(CALL_INFO, -1,
                "Param not specified(%s): group - group ID (or hierarchy "
                "level) for this NIC's component. Example: L2s in group 1, "
                "directories in group 2, memories (on network) in group 3.\n",
                getName().c_str());
    }
