...
public:
  CustomCmdEvent(std::string src, Addr addr, Addr baseAddr, Command cmd,
                 uint32_t opc = 0, uint32_t size = 0)
      : MemEventBase(src, cmd), addr_(addr), baseAddr_(baseAddr),
        addrGlobal_(true), opCode_(opc), size_(size), instPtr_(0), vAddr_(0) {}
  ...
  virtual Addr getRoutingAddress() { return baseAddr_; }

  virtual CustomCmdEvent *clone(void) override {
    return new CustomCmdEvent(*this);
  }

  virtual size_t getPayloadSize() { return payload_.size(); }
