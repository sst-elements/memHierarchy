 ...
 *  Remote Read     Read from remote memory remoteMemLineSize Remote Write Write
 * to remote memory                      remoteMemLineSize
 *
 * Ordering:
 *  - Only concerned with coherence (same address)
 *  - No write acks from scratch or memory
 *  - scratchpad can't reorder reads and writes to the same line or we'd need
 * acks
 *  - network can't reorder or we'll need acks and mshr structures for remote
 * accesses
 *
 *
 * Conflict resolution:
 ...
 * Scratchpad will ensure that once CPU receives an ack for the event, a future
 * event will be ordered later
 *
 */
...
Scratchpad::Scratchpad(ComponentId_t id, Params &params) : Component(id) {
  ...
  if (!found)
    out.fatal(CALL_INFO, -1,
              "Param not specified (%s): clock - clock rate (with units, e.g., "
              "GHz)\n",
              getName().c_str());
  ...
  if (!found)
    out.fatal(CALL_INFO, -1,
              "Param not specified (%s): size - size of the scratchpad (with "
              "units, e.g., MiB)\n",
              getName().c_str());
  if (!size.hasUnits("B"))
    out.fatal(CALL_INFO, -1,
              "Invalid param (%s): size - units must be bytes ('B'). SI units "
              "ok. You specified '%s'\n",
  ...
  if (scratchSize_ % scratchLineSize_ != 0) {
    out.fatal(CALL_INFO, -1,
              "Invalid param combination (%s): size, scratch_line_size - size "
              "must be divisible by scratch_line_size. You speciified size: "
              "%s, scratch_line_size: %" PRIu64 "\n",
  ...
  if (!scratch_) {
    ...
    std::string bkName = params.find<std::string>(
        "backendConvertor", "memHierarchy.scratchBackendConvertor");
    ...
    scratch_ = loadAnonymousSubComponent<ScratchBackendConvertor>(
        bkName, "backendConvertor", 0,
        ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, bkParams);
  }

  using std::placeholders::_1;
  scratch_->setCallbackHandler(
      std::bind(&Scratchpad::handleScratchResponse, this, _1));
  ...
  registerClock(clock_freq,
                new Clock::Handler<Scratchpad>(this, &Scratchpad::clock));

  // Register statistics
  stat_ScratchReadReceived =
      registerStatistic<uint64_t>("request_received_scratch_read");
  stat_ScratchWriteReceived =
      registerStatistic<uint64_t>("request_received_scratch_write");
  stat_RemoteReadReceived =
      registerStatistic<uint64_t>("request_received_remote_read");
  stat_RemoteWriteReceived =
      registerStatistic<uint64_t>("request_received_remote_write");
  stat_ScratchGetReceived =
      registerStatistic<uint64_t>("request_received_scratch_get");
  stat_ScratchPutReceived =
      registerStatistic<uint64_t>("request_received_scratch_put");
  stat_ScratchReadIssued =
      registerStatistic<uint64_t>("request_issued_scratch_read");
  stat_ScratchWriteIssued =
      registerStatistic<uint64_t>("request_issued_scratch_write");

  // Figure out port connections and set up links
  // Options: cpu and network; or cpu and memory;
  // cpu is a MoveEvent interface, memory & network are MemEvent interfaces
  // (memory is a direct connect while network uses SimpleNetwork)
  ...
  if (!cpuDirect && !scratchNetwork) {
    out.fatal(CALL_INFO, -1,
              "Invalid port configuration (%s): Did not detect port for "
              "cpu-side events. Connect either 'cpu' or 'network'\n",
              getName().c_str());
  } else if (!memoryDirect && !scratchNetwork) {
    out.fatal(CALL_INFO, -1,
              "Invalid port configuration (%s): Did not detect port for "
              "memory-side events. Connect either 'memory' or 'network'\n",
              getName().c_str());
  } else if (cpuDirect && scratchNetwork && memoryDirect) {
    out.fatal(CALL_INFO, -1,
              "Invalid port configuration (%s): Too many connected ports. "
              "Connect either 'cpu' or 'network' for cpu-side events and "
              "either 'memory' or 'network' for memory-side events\n",
              getName().c_str());
  }
  ...
  if (cpuDirect) {
    ...
    linkUp_ = loadAnonymousSubComponent<MemLinkBase>(
        ...
        ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, cpulink);
    linkUp_->setRecvHandler(new Event::Handler<Scratchpad>(
        this, &Scratchpad::processIncomingCPUEvent));
  }
  if (memoryDirect) {
    ...
    linkDown_ = loadAnonymousSubComponent<MemLinkBase>(
        ...
        ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, memlink);
    linkDown_->setRecvHandler(new Event::Handler<Scratchpad>(
        this, &Scratchpad::processIncomingRemoteEvent));
  }
  ...
  if (scratchNetwork) {
    ...
    if (fixupParam(params, "min_packet_size", "memNIC.min_packet_size"))
      out.output(
          CALL_INFO,
          "Note (%s): Changed 'min_packet_size' to 'memNIC.min_packet_size' in "
          "params. Change your input file to remove this notice.\n",
          getName().c_str());
    ...
    nicParams.insert(
        "group", "3",
        false); // 3 is the default for anything that talks to memory but this
                // can be set by user too so don't overwrite
    ...
    if (!memoryDirect) { /* Connect mem side to network */
      ...
      if (!cpuDirect) {
        ...
        linkDown_->setRecvHandler(new Event::Handler<Scratchpad>(
            this, &Scratchpad::processIncomingNetworkEvent));
      }
    } else {
      linkUp_ = loadAnonymousSubComponent<MemLinkBase>(
 ...
 * 2. Inform our neighbors who we are (line size, endpoint type, whether to
 * expect writeback acks, etc.)
 * 3. Discover who our neighbors are, especially if any are caches and we need
 * to be sending shoot downs
 * 4. Forward any memory initialization events to memory
 */
void Scratchpad::init(unsigned int phase) {

  // Init MemNIC if we have one - must be done before attempting to send
  ...
  if (!phase) {
    linkDown_->sendInitData(new MemEventInitCoherence(
        getName(), Endpoint::Scratchpad, true, true, scratchLineSize_, true));
    if (linkUp_ != linkDown_)
      linkUp_->sendInitData(new MemEventInitCoherence(
          getName(), Endpoint::Scratchpad, true, true, scratchLineSize_, true));
  }
  ...
  while (MemEventInit *initEv = linkUp_->recvInitData()) {
    if (initEv->getCmd() == Command::NULLCMD) {
      ...
      MemEventInit *memRequest = new MemEventInit(
          getName(), initEv->getCmd(), initEv->getAddr() - remoteAddrOffset_,
          initEv->getPayload());
      memRequest->setDst(
          linkDown_->findTargetDestination(memRequest->getAddr()));
...
void Scratchpad::processIncomingCPUEvent(SST::Event *event) {
  ...
  if (is_debug_event(ev))
    dbg.debug(_L3_,
              ...
              Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
  ...
  default:
    dbg.fatal(
        CALL_INFO, -1,
        "Scratchpad (%s) received unhandled event: cmd = %s, cycles = %" PRIu64
        ".\n",
...
void Scratchpad::processIncomingRemoteEvent(SST::Event *event) {
  ...
  std::map<SST::Event::id_type, SST::Event::id_type>::iterator it =
      responseIDMap_.find(ev->getResponseToID());

  if (it == responseIDMap_.end()) {
    dbg.fatal(CALL_INFO, -1,
              "(%s) Received data response from remote but no matching request "
              "in responseIDMap_, id is (%" PRIu64 ", %" PRIu32
              "), timestamp is %" PRIu64 "\n",
              getName().c_str(), ev->getResponseToID().first,
              ev->getResponseToID().second, timestamp_);
  }

  SST::Event::id_type requestID = it->second;
  responseIDMap_.erase(it);

  MemEventBase *requestBase =
      outstandingEventList_.find(requestID)->second.request;

  if (requestBase->getCmd() == Command::Get)
    handleRemoteGetResponse(ev, requestID);
  else
    handleRemoteReadResponse(ev, requestID);
}
  ...
  while (!procMsgQueue_.empty() && procMsgQueue_.begin()->first < timestamp_) {
    ...
    if (is_debug_event(sendEv)) {
      ...
      dbg.debug(_L4_,
                ...
                Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
                getName().c_str(), sendEv->getBriefString().c_str());
    }

    linkUp_->send(sendEv);
    procMsgQueue_.erase(procMsgQueue_.begin());
    responseThisCycle++;
    if (responseThisCycle == responsesPerCycle_)
      break;
  }
 ...
 *  Also send coherence reads that correspond to processor writes to the read
 * function
 */
 ...
 *      - GetSX
 *      - GetX, cacheable
 *  Put request in mshr, if no conflict, send to scratchpad and wait for
 * response
 */
void Scratchpad::handleScratchRead(MemEvent *ev) {
  ...
  if (caching_ &&
      !ev->queryFlag(
          MemEvent::F_NONCACHEABLE)) // Send data in exclusive state to let
                                     // caches decide what to do with it
    response->setCmd(Command::GetXResp);

  MemEvent *read = new MemEvent(getName(), ev->getAddr(), ev->getBaseAddr(),
                                Command::GetS, ev->getSize());
  ...
  if (mshr_.find(ev->getBaseAddr()) == mshr_.end()) {
    ...
    mshr_.insert(std::make_pair(
        ev->getBaseAddr(),
        std::list<MSHREntry>(
            1, MSHREntry(ev->getID(), Command::GetS, true, false))));
    ...
    mshr_.find(ev->getBaseAddr())
        ->second.push_back(MSHREntry(ev->getID(), Command::GetS, read));
    if (is_debug_addr(addr)) {
  ...
  if (is_debug_event(ev)) {
    dbg.debug(_L10_,
              "M: %-20" PRIu64 " %-20" PRIu64
              " %-20s MSHR:InsEv    0x%-16" PRIx64 " %s\n",
              Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
              getName().c_str(), ev->getBaseAddr(),
 ...
 * If from directory, replacements don't neccessarily mean that block is no
 * longer cached, so don't clear cached state.
 */
void Scratchpad::handleScratchWrite(MemEvent *ev) {
  ...
  bool doWrite = false; // Decide whether to handle this write immediately EVEN
                        // if a conflict
  bool inserted = false;
  /* Check for writeback/invalidation races */
  if (!directory_ && ev->isWriteback() &&
      mshr_.find(ev->getBaseAddr()) != mshr_.end()) {
    MSHREntry *entry = &(mshr_.find(ev->getBaseAddr())->second.front());
    if (outstandingEventList_.find(entry->id)->second.request->getCmd() ==
        Command::Get) {
      handleAckInv(ev);
      return;
      // TODO handle corner cases where Get only writes partial line
    } else if (outstandingEventList_.find(entry->id)
                   ->second.request->getCmd() == Command::Put) {
  ...
  MemEvent *write = new MemEvent(getName(), ev->getAddr(), ev->getBaseAddr(),
  ...
  if (directory_ && ev->isWriteback() &&
      mshr_.find(ev->getBaseAddr()) != mshr_.end()) {
    /* For directory - jump write ahead of a Put so we have correct data but
     * otherwise do not resolve race by treating writeback as ackinv since it
     * may not actually signal that the block is not present in caches */
    std::list<MSHREntry> *entry = &(mshr_.find(ev->getBaseAddr())->second);
    for (std::list<MSHREntry>::iterator it = entry->begin(); it != entry->end();
         it++) {
      if (it->cmd == Command::Put) {
        if (it == entry->begin()) {
          doScratchWrite(write);
          sendResponse(response); /* Send response when request is sent to
                                     scratch, since scratch doesn't respond */
          delete ev;
        } else {
          outstandingEventList_.insert(
              std::make_pair(ev->getID(), OutstandingEvent(ev, response)));
          ...
          if (is_debug_event(ev))
            dbg.debug(_L10_,
                      ...
                      timestamp_, getName().c_str(), ev->getBaseAddr(),
                      it->getString().c_str());
        }
        return;
      }
  ...
  if (mshr_.find(ev->getBaseAddr()) == mshr_.end()) {
    doScratchWrite(write);
    sendResponse(response); /* Send response when request is sent to scratch
                               since scratch doesn't respond */
    delete ev;
    /* Update cache state */
    if (caching_ && !ev->queryFlag(MemEvent::F_NONCACHEABLE)) {
      cacheStatus_.at(ev->getBaseAddr() / scratchLineSize_) = directory_;
    ...
    outstandingEventList_.insert(
        std::make_pair(ev->getID(), OutstandingEvent(ev, response)));
    mshr_.find(ev->getBaseAddr())
        ->second.push_back(MSHREntry(ev->getID(), Command::GetX, write));

    if (is_debug_event(ev))
      dbg.debug(
          _L10_,
          "M: %-20" PRIu64 " %-20" PRIu64 " %-20s MSHR:InsEv    0x%-16" PRIx64
          " %s\n",
          Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
          getName().c_str(), ev->getBaseAddr(),
          mshr_.find(ev->getBaseAddr())->second.back().getString().c_str());
...
void Scratchpad::handleScratchGet(MemEventBase *event) {
  ...
  // Issue remote read
  ev->setSrcBaseAddr((ev->getSrcAddr() - remoteAddrOffset_) &
                     ~(remoteLineSize_ - 1));
  MemEvent *remoteRead =
      new MemEvent(getName(), ev->getSrcAddr() - remoteAddrOffset_,
  ...
  if (is_debug_event(remoteRead)) {
    dbg.debug(_L10_,
              "C: %-20" PRIu64 " %-20" PRIu64
              " %-20s Get           0x%-16" PRIx64 " 0x%-16" PRIx64
              " Remote Read (<%" PRIu64 ", %" PRIu32 ">, 0x%" PRIx64 ")\n",
              Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
              getName().c_str(), saddr, daddr, remoteRead->getID().first,
              remoteRead->getID().second, remoteRead->getBaseAddr());
  }
  ...
  uint32_t lineCount =
      1 + (ev->getDstAddr() + ev->getSize() - ev->getDstBaseAddr() - 1) /
              scratchLineSize_;
  for (uint32_t i = 0; i < lineCount; i++) {
    ...
    if (mshr_.find(baseAddr) == mshr_.end()) {
      ...
      mshr_.insert(std::make_pair(
          baseAddr, std::list<MSHREntry>(1, MSHREntry(ev->getID(), Command::Get,
                                                      true, needAck))));
    } else {
      mshr_.find(baseAddr)->second.push_back(
          MSHREntry(ev->getID(), Command::Get, true));
    }
 ...
 *    srcAddr & srcAddr+size.
 * 3. Collect data scratch & shootdown responses in the payload of a write
 * event. If a shootdown response arrives without data (i.e., was clean or
 * uncached), send a scratch read.
...
void Scratchpad::handleScratchPut(MemEventBase *event) {
  ...
  MoveEvent *response = ev->makeResponse();
  ev->setDstBaseAddr((ev->getDstBaseAddr() - remoteAddrOffset_) &
                     ~(remoteLineSize_ - 1));

  MemEvent *remoteWrite =
      new MemEvent(getName(), ev->getDstAddr() - remoteAddrOffset_,
  ...
  while (bytesLeft != 0) {
    ...
    if (mshr_.find(baseAddr) == mshr_.end()) {
      ...
      mshr_.insert(std::make_pair(
          baseAddr, std::list<MSHREntry>(1, MSHREntry(ev->getID(), Command::Put,
                                                      !needAck, needAck))));
    } else {
      mshr_.find(baseAddr)->second.push_back(
          MSHREntry(ev->getID(), Command::Put));
    }
...
void Scratchpad::handleScratchResponse(SST::Event::id_type responseID) {
  ...
  if (is_debug_addr(baseAddr))
    dbg.debug(_L5_,
              "C: %-20" PRIu64 " %-20" PRIu64
              " %-20s Scratch:Recv  0x%-16" PRIx64 " <%" PRIu64 ", %" PRIu32
              ">\n",
              Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
              getName().c_str(), baseAddr, responseID.first, responseID.second);

  if (outstandingEventList_.find(requestID)->second.request->getCmd() ==
      Command::Put) {
    updatePut(requestID);
 ...
 * 4. If this was the last shootdown response and we're not waiting for a
 * scratch response, finish the request
 */
void Scratchpad::handleAckInv(MemEventBase *event) {
  ...
  MoveEvent *request = static_cast<MoveEvent *>(
      outstandingEventList_.find(requestID)->second.request);

  /* Update cache status */
  if (is_debug_addr(baseAddr))
    dbg.debug(_L9_,
              "C: %-20" PRIu64 " %-20" PRIu64 " %-20s UpdCache   0x%-16" PRIx64
              "  Uncached\n",
              Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
              getName().c_str(), baseAddr);
  ...
  if (entry->cmd == Command::Get) {
    ...
    if (is_debug_addr(baseAddr))
      dbg.debug(_L10_,
                "M: %-20" PRIu64 " %-20" PRIu64
                " %-20s MSHR:Update   0x%-16" PRIx64 " %s\n",
                Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
                getName().c_str(), baseAddr, entry->getString().c_str());
    ...
    uint32_t size =
        deriveSize(addr, baseAddr, request->getSrcAddr(), request->getSize());

    MemEvent *read =
        new MemEvent(getName(), addr, baseAddr, Command::GetS, size);
    ...
    std::vector<uint8_t> payload =
        outstandingEventList_.find(requestID)->second.remoteWrite->getPayload();
    uint32_t offset = addr - request->getSrcAddr();
    for (uint32_t i = 0; i < size; i++) {
      payload[i + offset] = data[i];
    }
    outstandingEventList_.find(requestID)->second.remoteWrite->setPayload(
        payload);
  } else {
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: unhandled case in handleAckInv. Time = %" PRIu64
              ", Event = (%s).\n",
 ...
 * Can only be in response to a Put since we don't request data for Get
 * shootdowns.
 *
 * 1. Set cached state to false
 * 2. If data is dirty, send scratch write
 * 3. Update payload for remote write
 * 4. If there are no more scratch or shootdown responses to wait for, finish
 * the put (updatePut())
 */
void Scratchpad::handleFetchResp(MemEventBase *event) {
  ...
  MSHREntry *entry = &(mshr_.find(baseAddr)->second.front());
  SST::Event::id_type requestID = entry->id;
  MoveEvent *put = static_cast<MoveEvent *>(
      outstandingEventList_.find(requestID)->second.request);

  /* Update cache status */
  cacheStatus_.at(baseAddr / scratchLineSize_) = false;

  // Send a write to scratch if the line was dirty since we forcefully
  // invalidated
  if (response->getDirty()) {
    MemEvent *write = new MemEvent(getName(), response->getAddr(), baseAddr,
                                   Command::PutM, response->getPayload());
...
void Scratchpad::handleNack(MemEventBase *event) {
  ...
  if (entry->needAck) {
    // Determine whether nackedEvent actually matches request -> if not, don't
    // resend resend inv
...
void Scratchpad::handleRemoteRead(MemEvent *event) {
  ...
  event->setBaseAddr((event->getAddr() - remoteAddrOffset_) &
                     ~(remoteLineSize_ - 1));
  MemEvent *request =
      new MemEvent(getName(), event->getAddr() - remoteAddrOffset_,
  ...
  outstandingEventList_.insert(
      std::make_pair(event->getID(), OutstandingEvent(event, response)));
 ...
 */
void Scratchpad::handleRemoteGetResponse(MemEvent *response,
                                         SST::Event::id_type requestID) {
  ...
  while (bytesLeft != 0) {
    ...
    MemEvent *write =
        new MemEvent(getName(), addr, baseAddr, Command::PutM, data);
    ...
    if (mshr_.find(baseAddr)->second.front().id == requestID) {
      ...
      if (mshr_.find(baseAddr) == mshr_.end()) {
        dbg.fatal(CALL_INFO, -1,
                  "ERROR: remoteGetResponse but no matching entry in mshr for "
                  "address 0x%" PRIx64 "\n",
                  baseAddr);
      }
      for (std::list<MSHREntry>::iterator it =
               mshr_.find(baseAddr)->second.begin();
        ...
        if (it->id == requestID) {
          ...
          if (is_debug_addr(baseAddr))
            dbg.debug(_L10_,
                      ...
                      " %-20s MSHR:Update   0x%-16" PRIx64 " %s\n",
                      Simulation::getSimulation()->getCurrentSimCycle(),
                      timestamp_, getName().c_str(), baseAddr,
                      mshr_.find(baseAddr)->second.front().getString().c_str());
        }
      }
    }
...
void Scratchpad::handleRemoteReadResponse(MemEvent *response,
                                          SST::Event::id_type requestID) {
  // Update response with payload and finish request
  MemEvent *fwdResponse = static_cast<MemEvent *>(
      outstandingEventList_.find(requestID)->second.response);
...
void Scratchpad::updateMSHR(Addr baseAddr) {
  ...
  while (!mshr_.find(baseAddr)->second.empty()) {
    ...
    if (entry->cmd == Command::GetS) {
      ...
      static_cast<MemEvent *>(
          outstandingEventList_.find(entry->id)->second.response)
          ->setPayload(readData);
      ...
      if (caching_ &&
          (outstandingEventList_.find(entry->id)->second.request->queryFlag(
      ...
      if (is_debug_addr(baseAddr))
        dbg.debug(_L10_,
                  "M: %-20" PRIu64 " %-20" PRIu64
                  " %-20s MSHR:Remove   0x%-16" PRIx64 "\n",
                  Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
                  getName().c_str(), baseAddr);

    } else if (entry->cmd == Command::Get) {
      entry->needAck = startGet(
          baseAddr, static_cast<MoveEvent *>(
      ...
      dbg.fatal(
          CALL_INFO, -1,
          "(%s) Updating MSHR but do not handle this case. Addr: 0x%" PRIx64
          "\n",
          getName().c_str(), baseAddr);
    }
  }
...
std::vector<uint8_t> Scratchpad::doScratchRead(MemEvent *event) {
  ...
  dbg.debug(_L5_,
            "C: %-20" PRIu64 " %-20" PRIu64 " %-20s Scratch:Send  0x%-16" PRIx64
            " (%s)\n",
            Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
...
bool Scratchpad::startGet(Addr baseAddr, MoveEvent *get) {
  if (caching_ && cacheStatus_.at(baseAddr / scratchLineSize_) == true) {
    MemEvent *inv = new MemEvent(getName(), baseAddr, baseAddr,
                                 Command::ForceInv, scratchLineSize_);
    ...
    dbg.debug(_L10_,
              "C: %-20" PRIu64 " %-20" PRIu64
              " %-20s Get            0x%-16" PRIx64 " 0x%-16" PRIx64
              " Inv         (<%" PRIu64 ", %" PRIu32 ">, 0x%" PRIx64 ")\n",
              Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
              getName().c_str(), get->getSrcBaseAddr(), get->getDstBaseAddr(),
              inv->getID().first, inv->getID().second, inv->getBaseAddr());
...
bool Scratchpad::startPut(Addr baseAddr, MoveEvent *put) {
  if (caching_ && cacheStatus_.at(baseAddr / scratchLineSize_) == true) {
    MemEvent *inv = new MemEvent(getName(), baseAddr, baseAddr,
                                 Command::FetchInv, scratchLineSize_);
    ...
    dbg.debug(_L10_,
              "C: %-20" PRIu64 " %-20" PRIu64
              " %-20s Put            0x%-16" PRIx64 " 0x%-16" PRIx64
              " Inv         (<%" PRIu64 ", %" PRIu32 ">, 0x%" PRIx64 ")\n",
              Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
              getName().c_str(), put->getSrcBaseAddr(), put->getDstBaseAddr(),
              inv->getID().first, inv->getID().second, inv->getBaseAddr());
    procMsgQueue_.insert(std::make_pair(timestamp_, inv));
    return true;
  } else {
    ...
    if (baseAddr == put->getSrcBaseAddr())
      addr = put->getSrcAddr();
    uint32_t size =
        deriveSize(addr, baseAddr, put->getSrcAddr(), put->getSize());
    ...
    std::vector<uint8_t> payload = outstandingEventList_.find(put->getID())
                                       ->second.remoteWrite->getPayload();
    uint32_t offset = addr - put->getSrcAddr();
    for (uint32_t i = 0; i < size; i++) {
      payload[i + offset] = data[i];
    }
    outstandingEventList_.find(put->getID())
        ->second.remoteWrite->setPayload(payload);
    return false;
  }
}
...
void Scratchpad::updatePut(SST::Event::id_type putID) {
  ...
  if (count == 0) {
    MoveEvent *put = static_cast<MoveEvent *>(
        outstandingEventList_.find(putID)->second.request);
    dbg.debug(
        _L10_,
        "C: %-20" PRIu64 " %-20" PRIu64 " %-20s Put            0x%-16" PRIx64
        " 0x%-16" PRIx64 " Scratch Done (<%" PRIu64 ", %" PRIu32 ">, 0x%" PRIx64
        ")\n",
        Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
        getName().c_str(), put->getSrcBaseAddr(), put->getDstBaseAddr(),
    ...
    //        dbg.debug(_L5_, "C: %-20" PRIu64 " %-20" PRIu64 " %-20s Finish
    //        0x%-16" PRIx64 " <%" PRIu64 ", %" PRIu32 ">\n",
    //                Simulation::getSimulation()->getCurrentSimCycle(),
    //                timestamp_, getName().c_str(),
    //                outstandingEventList_.find(putID)->second.remoteWrite->getBaseAddr(),
    //                baseAddr, responseID.first, responseID.second);
    memMsgQueue_.insert(std::make_pair(
        timestamp_, outstandingEventList_.find(putID)->second.remoteWrite));
...
uint32_t Scratchpad::deriveSize(Addr addr, Addr baseAddr, Addr requestAddr,
