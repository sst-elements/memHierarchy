...
void Bus::sendSingleEvent(SST::Event *ev) {
  ...
  if (is_debug_event(event)) {
    ...
    dbg_.debug(_L3_, "Incoming Event. Name: %s, LinkID: %d, Event: %s\n",
               this->getName().c_str(), event->getDeliveryLink()->getId(),
  ...
  if (it != nameMap_.end()) {
    if (it->second != id)
      dbg_.fatal(CALL_INFO, -1,
                 "%s, Error: Bus attempting to map node that has already been "
                 "mapped\n",
                 getName().c_str());
    return;
  }
...
LinkId_t Bus::lookupNode(const std::string &name) {
  ...
  if (nameMap_.end() == it) {
    dbg_.fatal(CALL_INFO, -1,
               "%s, Error: Bus lookup of node %s returned no mapping\n",
...
void Bus::configureLinks() {
  ...
  while (isPortConnected(linkname)) {
    ...
    linkIdMap_[highNetPorts_[numHighNetPorts_]->getId()] =
        highNetPorts_[numHighNetPorts_];
  ...
  while (isPortConnected(linkname)) {
    ...
    linkIdMap_[lowNetPorts_[numLowNetPorts_]->getId()] =
        lowNetPorts_[numLowNetPorts_];
    dbg_.output(CALL_INFO, "Port %d = Link %d\n", numLowNetPorts_,
...
void Bus::configureParameters(SST::Params &params) {
  ...
  fanout_ = params.find<bool>(
      "fanout",
      0); /* TODO:  Fanout: Only send messages to lower level caches */
  drain_ = params.find<bool>("drain_bus", 0);

  if (busFrequency_ == "Invalid")
    dbg_.fatal(CALL_INFO, -1, "Bus Frequency was not specified\n");
  ...
  /* Multiply Frequency times two.  This is because an SST Bus components has
     ...
     transaction (a real bus should be allowed to have 1 cycle latency).  To
     overcome this we clock the bus 2x the speed of the cores */
  ...
  for (int i = 0; i < numHighNetPorts_; i++) {
    while ((ev = highNetPorts_[i]->recvInitData())) {
      ...
      if (memEvent && memEvent->getCmd() == Command::NULLCMD) {
        dbg_.debug(_L10_,
                   "bus %s broadcasting upper event to lower ports (%d): %s\n",
  ...
  for (int i = 0; i < numLowNetPorts_; i++) {
    while ((ev = lowNetPorts_[i]->recvInitData())) {
      ...
      if (!memEvent)
        ...
        dbg_.debug(_L10_,
                   "bus %s broadcasting lower event to upper ports (%d): %s\n",
