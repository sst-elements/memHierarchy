...
void MemLink::build(Params &params) {
  ...
  link = configureLink(port, latency,
                       new Event::Handler<MemLink>(this, &MemLink::recvNotify));
  ...
  dbg.debug(_L10_,
            "%s memLink info is: Name: %s, addr: %" PRIu64 ", id: %" PRIu32
            "\n",
...
void MemLink::init(unsigned int phase) {
  if (!phase) {
    MemEventInitRegion *ev =
        new MemEventInitRegion(info.name, info.region, false);
 ...
 * send event on link
 */
void MemLink::send(MemEventBase *ev) { link->send(ev); }
...
std::string MemLink::findTargetDestination(Addr addr) {
  for (std::set<EndpointInfo>::const_iterator it = remotes.begin();
       it != remotes.end(); it++) {
  ...
  stringstream error;
  error << getName() + " (MemLink) cannot find a destination for address "
        << addr << endl;
