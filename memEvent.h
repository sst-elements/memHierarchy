...
public:
  /****** Old calls will now throw deprecated warnings since parent pointer is
   * not available *************/
  ...
  bool isLoadLink() {
    return cmd_ == Command::GetS && queryFlag(MemEventBase::F_LLSC);
  }
  ...
  /** Returns true if this is a CPU-side event (i.e., sent from CPU side of
   * hierarchy) */
...
private:
  ...
  bool blocked_;    // Whether this request blocked for another pending request
                    // (for profiling) TODO move to mshrs
  ...
  bool inProgress_; // Whether this request is currently being handled, if in
                    // MSHR TODO move to mshrs
