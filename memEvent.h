 ...
 */
        ...
        public:
            ...
            MemEvent(const Component *src, Addr addr, Addr baseAddr, Command cmd) : MemEventBase(
                src->getName(), cmd) {
                initialize();
                addr_ = addr;
                baseAddr_ = baseAddr;
            ...
            MemEvent(const Component *src, Addr addr, Addr baseAddr, Command cmd, uint32_t size)
            ...
            MemEvent(const Component *src, Addr addr, Addr baseAddr, Command cmd,
                     std::vector <uint8_t> &data) : MemEventBase(src->getName(), cmd) {
                initialize();
                addr_ = addr;
                baseAddr_ = baseAddr;
                initTime_ = src->getCurrentSimTimeNano();
            ...
            /************ New calls - use these! *****************/
            MemEvent(std::string src, Addr addr, Addr baseAddr, Command cmd, uint64_t timeInNano)
            ...
            MemEvent(std::string src, Addr addr, Addr baseAddr, Command cmd, uint32_t size,
                     uint64_t timeInNano) : MemEventBase(src, cmd) {
                initialize();
                addr_ = addr;
                baseAddr_ = baseAddr;
                initTime_ = timeInNano;
            ...
            MemEvent(std::string src, Addr addr, Addr baseAddr, Command cmd,
                     std::vector <uint8_t> &data, uint64_t timeInNano) : MemEventBase(src, cmd) {
                initialize();
                addr_ = addr;
                baseAddr_ = baseAddr;
                initTime_ = timeInNano;
            ...
            bool isStoreConditional() {
                return cmd_ == Command::GetX && queryFlag(MemEventBase::F_LLSC);
            }

            void setSuccess(bool b) {
                b ? setFlag(MemEventBase::F_SUCCESS) : clearFlag(MemEventBase::F_SUCCESS);
            }
...
// Information about command types
            ...
            virtual std::string getBriefString() override {
                std::ostringstream str;
                str << " Addr: 0x" << std::hex << addr_ << " BaseAddr: 0x" << baseAddr_ << std::dec
                    << " Size: " << size_;
