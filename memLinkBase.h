...
#define MEMLINKBASE_ELI_PARAMS                                                 \
  {"debug",                                                                    \
   "(int) Where to print debug output. Options: 0[no output], 1[stdout], "     \
   "2[stderr], 3[file]",                                                       \
   "0"},                                                                       \
  ...
  {                                                                            \
    "accept_region",                                                           \
        "(bool) Set by parent component but user should unset if region "      \
        "(addr_range_start/end, interleave_size/step) params are provided to " \
        "memory. Provides backward compatibility for address translation "     \
        "between memory controller and directory.",                            \
        "false"                                                                \
  }
  ...
  struct EndpointInfo {
    ...
    uint32_t id; /* Which memory level or group this component belongs to - for
                    determining which components are sources or destinations */
  ...
  virtual void setRecvHandler(Event::HandlerBase *handler) {
    recvHandler = handler;
  }
  ...
  virtual bool isRequestAddressValid(Addr addr) {
    return info.region.contains(addr);
  }
...
protected:
  ...
  SST::Event::HandlerBase
      *recvHandler; // Event handler to call when an event is received

  // Data structures
  std::queue<MemEventInit *>
      initReceiveQ; // queue for messages received during init
  ...
  void build(Params &params) {
    ...
    if (foundany) {
      dbg.output("%s, Warning: Region parameters given to link managers "
                 "(addr_range_start/end, interleave_size/step) will be "
                 "overwritten if the component sets them; specify region via "
                 "component to eliminate this message\n",
                 getName().c_str());
    }
