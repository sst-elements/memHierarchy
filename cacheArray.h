...
template <class T> class CacheArray {
protected:
  Output *dbg_;
  ...
  State *setStates;
  std::map<unsigned int, std::vector<ReplacementInfo *>>
      rInfo; // Lookup a vector of replacementInfo by set ID
public:
  CacheArray(Output *dbg, unsigned int numLines, unsigned int associativity,
             uint32_t lineSize, ReplacementPolicy *replacementMgr,
             HashFunction *hash);
  ...
  /** Deallocate a line and notify replacement manager that it's been
   * deallocated */
...
template <class T>
CacheArray<T>::CacheArray(Output *dbg, unsigned int numLines,
                          unsigned int associativity, uint32_t lineSize,
                          ReplacementPolicy *replacementMgr, HashFunction *hash)
    : dbg_(dbg), numLines_(numLines), associativity_(associativity),
      lineSize_(lineSize), replacementMgr_(replacementMgr), hash_(hash) {

  // Error check parameters
  if (numLines_ == 0)
    dbg_->fatal(
        CALL_INFO, -1,
        "CacheArray, Error: number of lines is 0. Must be greater than 0.\n");

  if (associativity_ == 0)
    dbg_->fatal(CALL_INFO, -1,
                "CacheArray, Error: associativity is 0. Use 1 for direct "
                "mapped, 2 or more for set-associative.\n");
  ...
  if (numSets_ == 0)
    dbg_->fatal(
        CALL_INFO, -1,
        "CacheArray, Error: number of sets (number of lines / associativity) "
        "is 0. Must be at least 1. Number of lines = %u. Associativity = %u.\n",
        numLines_, associativity_);
  if ((numSets_ * associativity_) != numLines_)
    dbg_->fatal(CALL_INFO, -1,
                "CacheArray, Error: The number of cachelines is not divisible "
                "by the cache associativity. Ensure (lines mod associativiy = "
                "0). Number of lines = %u. Associativity = %u\n",
                numLines_, associativity_);
  ...
  if (!replacementMgr_->checkCompatibility(info))
    dbg_->fatal(CALL_INFO, -1,
                "CacheArray, Error: The replacement policy expects cache line "
                "state that is not provided by the cache line type of this "
                "cache. Check the type of the ReplacementInfo returned by the "
                "coherence protocol's line type and the ReplacementInfo type "
                "expected by the replacement policy.\n");
...
template <class T> Addr CacheArray<T>::toLineAddr(Addr addr) {
...
template <class T> T *CacheArray<T>::findReplacementCandidate(Addr addr) {
  Addr laddr = toLineAddr(addr);
  int set = hash_->hash(0, laddr) % numSets_;
...
template <class T> void CacheArray<T>::replace(Addr addr, T *candidate) {
...
template <class T> void CacheArray<T>::deallocate(T *candidate) {
  unsigned int index = candidate->getIndex();
  replacementMgr_->replaced(index);
  candidate->reset();
}

template <class T> void CacheArray<T>::setSliceAware(Addr size, Addr step) {
...
template <class T> void CacheArray<T>::setBanked(unsigned int numBanks) {
  banks_ = numBanks;
}

template <class T> void CacheArray<T>::printCacheArray(Output &out) {
  for (unsigned int i = 0; i < numLines_; i++) {
