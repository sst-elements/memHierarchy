 ...
 *  Initialize directory_ variable to indicate whether a directory is in the
 * system For a directory, a replacement is not synonymous with eviction
 */
CoherentMemController::CoherentMemController(ComponentId_t id, Params &params)
    : MemController(id, params) {
...
void CoherentMemController::init(unsigned int phase) {
  ...
  if (!phase) {
    ...
    link_->sendInitData(new MemEventInitCoherence(
        getName(), Endpoint::Memory, true, false,
        memBackendConvertor_->getRequestWidth(), true));
  }
 ...
 * 'timestamp_' is used to delay events in the queue and does not need to be
 * sync'd with 'cycle' when the clock is reenabled
 */
bool CoherentMemController::clock(Cycle_t cycle) {
  ...
  bool unclockBack =
      memBackendConvertor_->clock(cycle); /* OK to unclock backend? */
...
void CoherentMemController::handleRequest(MemEvent *ev) {
  ...
  if (mshr_.find(ev->getBaseAddr()) == mshr_.end()) {
    ...
    mshr_.find(ev->getBaseAddr())
        ->second.push_back(MSHREntry(ev->getID(), ev->getCmd()));
  }
}
...
void CoherentMemController::handleReplacement(MemEvent *ev) {
  ...
  if (!ev->getDirty()) {
    cacheStatus_.at(ev->getBaseAddr() / lineSize_) =
        directory_; // If directory, writeback does not imply eviction
    delete ev;
    return;
  }
  ...
  if (mshr_.find(ev->getBaseAddr()) == mshr_.end()) {
    ...
    /* Search for race with a shootdown where we might receive an Ack but not
     * data */
    std::list<MSHREntry> *entryList = &(mshr_.find(ev->getBaseAddr())->second);
    for (std::list<MSHREntry>::iterator it = entryList->begin();
         it != entryList->end(); it++) {
      if (it->cmd == Command::CustomReq && it->shootdown) {
        if (it ==
            entryList->begin()) { /* Shootdown in progress, we will receive an
                                     Ack but possibly no data with it */
          it->writebacks.insert(ev->getID());
          memBackendConvertor_->handleMemEvent(ev);
        } else {
          it = entryList->insert(
              it,
              MSHREntry(ev->getID(), ev->getCmd())); /* Process replacement
                                                        before next shootdown */
        }
        return;
      }
    }
...
void CoherentMemController::handleFlush(MemEvent *ev) {
   ...
   * - Directory handles a FetchInv that raced with FlushLineInv by sending
   * AckInv immediately
   * - Directory handles a FetchInv that raced with FlushLine by invalidating
   * caches and eventually responding with AckInv
   * - Caches handle a FetchInv that raced with FlushLineInv by dropping the
   * FetchInv
   * - Caches handle a FetchInv that raced with FlushLine by invalidating caches
   * and eventually responding with AckInv
   */
  if (mshr_.find(ev->getBaseAddr()) != mshr_.end()) {
    MSHREntry *entry = &(mshr_.find(ev->getBaseAddr())->second.front());
    if (entry->cmd == Command::CustomReq &&
        entry->shootdown) { // Race with shootdown
      if (!directory_) {
        if (ev->getCmd() == Command::FlushLineInv) {
          MemEvent *resp = new MemEvent(ev->getSrc(), ev->getBaseAddr(),
                                        ev->getBaseAddr(), Command::AckInv);
          ...
          if (ev->getPayloadSize() != 0) {
            entry->writebacks.insert(
                ev->getID()); // We're about to writeback, just make sure we
                              // tell the shootdown about it
          }
        }
      } else { // Directory
        if (ev->getPayloadSize() != 0)
  ...
  if (ev->getPayloadSize() != 0) {
    ...
    outstandingEventList_.insert(std::make_pair(
        put->getID(), OutstandingEvent(put, put->getBaseAddr())));
    notifyListeners(ev);

    if (mshr_.find(put->getBaseAddr()) == mshr_.end()) {
      mshr_.insert(std::make_pair(
          put->getBaseAddr(),
          std::list<MSHREntry>(1, MSHREntry(put->getID(), put->getCmd()))));
      memBackendConvertor_->handleMemEvent(put);
    } else {
      mshr_.find(put->getBaseAddr())
          ->second.push_back(MSHREntry(put->getID(), put->getCmd()));
    }
  }
  ...
  if (mshr_.find(ev->getBaseAddr()) == mshr_.end()) {
    ...
    memBackendConvertor_->handleMemEvent(ev);
  } else { // TODO resolve potential race with a not-yet-started shootdown
           // sitting in the MSHR?
    mshr_.find(ev->getBaseAddr())
        ->second.push_back(MSHREntry(ev->getID(), ev->getCmd()));
  }
...
/* Response to shootdown indicating block is no longer cached and is clean at
 * memory */
void CoherentMemController::handleAckInv(MemEvent *ev) {
  ...
  if (entry->writebacks.empty()) {
    if (outEv->decrementCount() ==
        0) { /* All shootdowns for this event are done */
...
void CoherentMemController::handleFetchResp(MemEvent *ev) {
  ...
  if (ev->getDirty()) {
    MemEvent *write = new MemEvent(getName(), ev->getAddr(), baseAddr,
                                   Command::PutM, ev->getPayload());
...
void CoherentMemController::handleNack(MemEvent *ev) {
  ...
  Addr baseAddr = nackedEvent->isAddrGlobal()
                      ? translateToLocal(nackedEvent->getBaseAddr())
...
void CoherentMemController::handleCustomCmd(MemEventBase *evb) {
  ...
  CustomCmdMemHandler::MemEventInfo evInfo =
      customCommandHandler_->receive(evb);
  outstandingEventList_.insert(
      std::make_pair(evb->getID(), OutstandingEvent(evb, evInfo.addrs)));
  OutstandingEvent *outEv = &(outstandingEventList_.find(evb->getID())->second);

  /* Custom commands may touch multiple (base) addresses */
  for (std::set<Addr>::iterator it = outEv->addrs.begin();
       it != outEv->addrs.end(); it++) {
    if (mshr_.find(*it) == mshr_.end()) {
      if (!evInfo.shootdown) {
        mshr_.insert(std::make_pair(
            *it,
            std::list<MSHREntry>(1, MSHREntry(evb->getID(), evb->getCmd()))));
        outEv->decrementCount();
      } else if (doShootdown(*it, evb)) {
        mshr_.insert(std::make_pair(
            *it, std::list<MSHREntry>(1, MSHREntry(evb->getID(), evb->getCmd(),
                                                   evInfo.shootdown))));
      } else {
        mshr_.insert(std::make_pair(
            *it,
...
bool CoherentMemController::doShootdown(Addr addr, MemEventBase *ev) {
  if (cacheStatus_.at(addr / lineSize_) == true) {
    ...
    MemEvent *inv = new MemEvent(getName(), globalAddr, globalAddr,
                                 Command::FetchInv, lineSize_);
    inv->setRqstr(ev->getRqstr());
    inv->setDst(ev->getSrc());

    msgQueue_.insert(std::make_pair(
        timestamp_, inv)); /* Send on next clock. TODO timing needed? */
    return true;
  }
  return false;
}
...
/* Handle MemResponse */
void CoherentMemController::handleMemResponse(SST::Event::id_type id,
                                              uint32_t flags) {
  std::map<SST::Event::id_type, OutstandingEvent>::iterator it =
      outstandingEventList_.find(id);
  if (it == outstandingEventList_.end()) {
    dbg.fatal(CALL_INFO, -1,
              "Coherent Memory controller (%s) received unrecgonized response "
              "ID: %" PRIu64 ", %" PRIu32 "",
              getName().c_str(), id.first, id.second);
  }

  if (outstandingEventList_.find(id)->second.request->getCmd() ==
      Command::CustomReq) {
...
/* Finish a MemEvent */
void CoherentMemController::finishMemReq(SST::Event::id_type id,
                                         uint32_t flags) {
  ...
  if (it == outstandingEventList_.end())
    dbg.fatal(
        CALL_INFO, -1,
        "Memory controller (%s) received unrecgonized response ID: %" PRIu64
        ", %" PRIu32 "",
        getName().c_str(), id.first, id.second);
  ...
  if (backing_ && (ev->getCmd() == Command::PutM ||
                   (ev->getCmd() == Command::GetX && noncacheable)))
  ...
  if (entry->writebacks.find(id) != entry->writebacks.end()) {
    ...
    delete ev;
    if (entry->writebacks.empty() &&
        !entry->shootdown) { /* Request now ready to issue */
 ...
 *  1. Translating local (contiguous/start at 0) addresses to global using
 * translateAddrToGlobal()
 *  2. Updating or reading from backing store using readData() and writeData()
 *  3. Creating and returning response message if needed
 */
void CoherentMemController::finishCustomReq(SST::Event::id_type id,
                                            uint32_t flags) {
...
void CoherentMemController::updateMSHR(Addr baseAddr) {
  ...
  if (mshr_.find(baseAddr)->second.empty()) {
    ...
    if (outEv->request->getCmd() == Command::CustomReq) {
      if (entry->shootdown &&
          doShootdown(baseAddr, outEv->request)) { /* Start shootdown */
        return;
      } else {
      ...
      replayMemEvent(static_cast<MemEvent *>(
          (outstandingEventList_.find(entry->id)->second).request));
    }
  }
}
...
void CoherentMemController::replayMemEvent(MemEvent *ev) {
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Attempt to replay unknown event: %s. Time = %" PRIu64
              "ns.\n",
              getName().c_str(), ev->getVerboseString().c_str(),
              getCurrentSimTimeNano());
    break;
