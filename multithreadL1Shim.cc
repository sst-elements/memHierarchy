...
MultiThreadL1::MultiThreadL1(ComponentId_t id, Params &params) : Component(id) {
  ...
  if (isPortConnected("cache")) {
    cacheLink = configureLink("cache", "50ps",
                              new Event::Handler<MultiThreadL1>(
                                  this, &MultiThreadL1::handleResponse));
    if (!cacheLink)
      output.fatal(CALL_INFO, -1,
                   "%s, Error: unable to configure link on port 'cache'\n",
                   getName().c_str());
  } else {
    output.fatal(CALL_INFO, -1,
                 "%s, Error: no connected cache port. Please connect a cache "
                 "to port 'cache'\n",
                 getName().c_str());
  }
  ...
  if (!isPortConnected("thread0"))
    output.fatal(CALL_INFO, -1,
                 "%s, Error: no connected CPU ports. Please connect a CPU to "
                 "port 'thread0'.\n",
                 getName().c_str());
  ...
  while (isPortConnected(linkname)) {
    SST::Link *link =
        configureLink(linkname, "50ps",
                      new Event::Handler<MultiThreadL1, unsigned int>(
                          this, &MultiThreadL1::handleRequest, linkid));
    if (!link)
      output.fatal(CALL_INFO, -1,
...
bool MultiThreadL1::tick(SST::Cycle_t cycle) {
  ...
  uint64_t sendcount =
      (requestsPerCycle == 0) ? requestQueue.size() : requestsPerCycle;
  ...
  sendcount =
      (responsesPerCycle == 0) ? responseQueue.size() : responsesPerCycle;
  ...
  while (!responseQueue.empty() && sendcount > 0) {
    ...
    unsigned int linkid =
        threadRequestMap.find(event->getResponseToID())->second;
...
void MultiThreadL1::init(unsigned int phase) {
  ...
  // Pass CPU events to memory hierarchy, generally these are memory
  // initialization
