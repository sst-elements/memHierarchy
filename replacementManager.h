...
namespace SST {
    namespace MemHierarchy {
        ...
        public:
            ...
            virtual uint findBestCandidate(uint setBegin, State *state, uint *sharers, bool *owned,
                                           bool sharersAware) = 0;
 ...
 * ------------------------------------------------------------------------------------------*/
        ...
        public:
            LRUReplacementMgr(Output *_dbg, uint _numLines, uint _numWays, bool _sharersAware)
                : timestamp(1), bestCandidate(-1), numLines(_numLines), numWays(_numWays) {
            ...
            uint findBestCandidate(uint setBegin, State *state, uint *sharers, bool *owned,
                                   bool sharersAware) {
                uint setEnd = setBegin + numWays;
                bestCandidate = setBegin;
                Rank bestRank = {array[setBegin], (sharersAware) ? sharers[0] : 0,
                                 (sharersAware) ? owned[0] : false, state[0]};
                if (state[0] == I) return (uint) bestCandidate;
                setBegin++;
                int i = 1;
                for (uint id = setBegin; id < setEnd; id++) {
                    Rank candRank = {array[id], (sharersAware) ? sharers[i] : 0,
                                     (sharersAware) ? owned[i] : false, state[i]};
 ...
 * ------------------------------------------------------------------------------------------*/
            ...
            struct Rank {
                ...
                inline bool lessThan(const Rank &other, const uint64_t curTs) const {
                    ...
                    if (sharers == 0 && other.sharers > 0) {
                        ...
                        uint64_t ownInvFreq =
                            (curTs - lfuInfo.ts) / lfuInfo.acc; //inverse frequency, lower is better
        ...
        public:
            ...
            LFUReplacementMgr(Output *_dbg, uint _numLines, uint _numWays) : timestamp(1),
                                                                             bestCandidate(-1),
                                                                             numLines(_numLines),
                                                                             numWays(_numWays) {
 ...
 * ------------------------------------------------------------------------------------------*/
        ...
        public:
            MRUReplacementMgr(Output *_dbg, uint _numLines, uint _numWays, bool _sharersAware)
                : timestamp(1), bestCandidate(-1), numLines(_numLines), numWays(_numWays) {
                array = (uint64_t *) calloc(numLines, sizeof(uint64_t));
            }
 ...
 * ------------------------------------------------------------------------------------------*/
        ...
        public:
            RandomReplacementMgr(Output *_dbg, uint _numWays) : numWays(_numWays),
                                                                randomGenerator_(1, 1) {
            }
 ...
 * ------------------------------------------------------------------------------------------*/
        ...
        public:
            NMRUReplacementMgr(Output *_dbg, uint _numLines, uint _numWays) : bestCandidate(-1),
                                                                              numLines(_numLines),
                                                                              numWays(_numWays),
                                                                              randomGenerator(1,
                                                                                              1) {
