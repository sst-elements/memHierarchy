...
/*
 * Generic/extendable class for passing information between a cache line & a
 * replacement policy To date the coherence policies in memHierarchy only use
 * cache line state and sometimes owned/shared information but other components
 * are able to extend this to pass arbitrary information as needed
 */
...
public:
  CoherenceReplacementInfo(unsigned int i, State s, bool sh, bool o)
      : shared(sh), owned(o), ReplacementInfo(i, s) {}
...
public:
  ...
  ReplacementPolicy(ComponentId_t id, Params &params, uint64_t lines,
                    uint64_t associativity)
      : SubComponent(id) {}
  virtual ~ReplacementPolicy() {}

  /* Since we don't dynamic cast ReplacementInfo, do a check here to make sure
   * the type provided by the cache line & the type the replacement policy
   * expects are compatible */
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      LRU, "memHierarchy", "replacement.lru", SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "least-recently-used replacement policy",
      SST::MemHierarchy::ReplacementPolicy);

  LRU(ComponentId_t id, Params &params, uint64_t lines, uint64_t associativity)
      : ReplacementPolicy(id, params, lines, associativity), timestamp(1),
        bestCandidate(0) {
    ways = associativity;
  ...
  /* Too expensive to constantly dynamic_cast. Check once during construction
   * instead. */
  ...
  /** Lines are selected for replacement according to the following criteria
   * (and in this order):
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      LRUOpt, "memHierarchy", "replacement.lru-opt",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "least-recently-used replacement policy with consideration for coherence "
      "state",
      SST::MemHierarchy::ReplacementPolicy);

  LRUOpt(ComponentId_t id, Params &params, uint64_t lines,
         uint64_t associativity)
      : ReplacementPolicy(id, params, lines, associativity), timestamp(1),
        bestCandidate(0) {
    ways = associativity;
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      LFU, "memHierarchy", "replacement.lfu", SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "least-frequently-used replacement policy, recently used accesses are "
      "more heavily weighted",
      SST::MemHierarchy::ReplacementPolicy);

  LFU(ComponentId_t id, Params &params, uint64_t lines, uint64_t associativity)
      : ReplacementPolicy(id, params, lines, associativity), timestamp(1),
        bestCandidate(0) {
    ways = associativity;
  ...
  // timestamp increments by 1000 every time to make sure there's sufficient
  // space between timestamps
...
private:
  struct LFUInfo {
    uint64_t ts;  // timestamp, function of accesses with more recent ones being
                  // more heavily weighted
    ...
    inline bool lessThan(LFUInfo &other, const uint64_t curTs) {
      ...
      if (other.acc == 0)
        return false;
      uint64_t ownInvFreq =
          (curTs - ts) / acc; // inverse frequency, lower is better
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      LFUOpt, "memHierarchy", "replacement.lfu-opt",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "least-frequently-used replacement policy, recently used accesses are "
      "more heavily weighted. Also considers coherence state in replacement "
      "decision",
      SST::MemHierarchy::ReplacementPolicy);

  LFUOpt(ComponentId_t id, Params &params, uint64_t lines,
         uint64_t associativity)
      : ReplacementPolicy(id, params, lines, associativity), timestamp(1),
        bestCandidate(0) {
    ways = associativity;
  ...
  struct Rank {
    ...
    inline bool lessThan(const Rank &other, const uint64_t curTs) const {
      ...
      if (!shared && other.shared) {
        ...
        if (lfuInfo.acc == 0)
          return true;
        if (other.lfuInfo.acc == 0)
          return false;
        uint64_t ownInvFreq = (curTs - lfuInfo.ts) /
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(MRU, "memHierarchy", "replacement.mru",
                                        SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                        "most-recently-used replacement policy",
                                        SST::MemHierarchy::ReplacementPolicy);

  MRU(ComponentId_t id, Params &params, uint64_t lines, uint64_t associativity)
      : ReplacementPolicy(id, params, lines, associativity), timestamp(1),
        bestCandidate(0) {
    ways = associativity;
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      MRUOpt, "memHierarchy", "replacement.mru-opt",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "most-recently-used replacement policy, with consideration for coherence "
      "state",
      SST::MemHierarchy::ReplacementPolicy);

  MRUOpt(ComponentId_t id, Params &params, uint64_t lines,
         uint64_t associativity)
      : ReplacementPolicy(id, params, lines, associativity), timestamp(1),
        bestCandidate(0) {
    ways = associativity;
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(Random, "memHierarchy",
                                        "replacement.random",
                                        SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                        "random replacement policy",
                                        SST::MemHierarchy::ReplacementPolicy);

  SST_ELI_DOCUMENT_PARAMS({"seed_a", "Seed for random number generator", "1"},
                          {"seed_b", "Seed for random number generator", "1"})

  Random(ComponentId_t id, Params &params, uint64_t lines,
         uint64_t associativity)
      : ReplacementPolicy(id, params, lines, associativity), bestCandidate(0) {
    ways = associativity;
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      NMRU, "memHierarchy", "replacement.nmru",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "not-most-recently-used, random replacement among all but the most "
      "recently used line in a set",
      SST::MemHierarchy::ReplacementPolicy);

  SST_ELI_DOCUMENT_PARAMS({"seed_a", "Seed for random number generator", "1"},
                          {"seed_b", "Seed for random number generator", "1"})

  NMRU(ComponentId_t id, Params &params, uint64_t lines, uint64_t associativity)
      : ReplacementPolicy(id, params, lines, associativity), bestCandidate(0) {
    ways = associativity;
  ...
  void update(uint64_t id, ReplacementInfo *rInfo) {
    array[id / ways] = id % ways;
  }
  // void replaced(uint64_t id) {}
  void replaced(uint64_t id) {}

  // Return an empty slot if one exists, otherwise return any slot that is not
  // the most-recently used in the set
  uint64_t findBestCandidate(std::vector<ReplacementInfo *> &rInfo) {
