...
MemNetBridge::MemNetBridge(SST::ComponentId_t id, SST::Params &params,
  ...
  dbg.init("@t:Bridge::@p():@l " + getName() + ": ", debugLevel, 0,
...
SimpleNetwork::Request *MemNetBridge::initTranslate(SimpleNetwork::Request *req,
                                                    uint8_t fromNet) {
  dbg.debug(CALL_INFO, 2, 0, "Received init phase event on interface %d\n",
            fromNet);
  Event *payload = req->inspectPayload();
  MemNIC::InitMemRtrEvent *imre =
      dynamic_cast<MemNIC::InitMemRtrEvent *>(payload);
  if (imre) {
...
SimpleNetwork::Request *MemNetBridge::translate(SimpleNetwork::Request *req,
                                                uint8_t fromNet) {
  Net_t &inNet = networks[fromNet];
  Net_t &outNet = networks[fromNet ^ 1];

  MemNIC::MemRtrEvent *mre =
      static_cast<MemNIC::MemRtrEvent *>(req->inspectPayload());
...
SimpleNetwork::nid_t MemNetBridge::getAddrFor(Net_t &net,
