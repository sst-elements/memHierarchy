...
MemHierarchyInterface::MemHierarchyInterface(SST::ComponentId_t id,
                                             Params &params,
                                             TimeConverter *time,
                                             HandlerBase *handler)
    : SimpleMem(id, params) {
  setDefaultTimeBase(
      time); // Required for link since we no longer inherit it from our parent
  ...
  if (NULL == recvHandler_)
    ...
    link_ = configureLink(portname,
                          new Event::Handler<MemHierarchyInterface>(
                              this, &MemHierarchyInterface::handleIncoming));

  if (!link_)
    output.fatal(CALL_INFO, -1,
                 "%s, Error: unable to configure link on port '%s'\n",
...
void MemHierarchyInterface::init(unsigned int phase) {
  ...
  if (!phase) {
    ...
    MemEventInitCoherence *event = new MemEventInitCoherence(
        getName(), Endpoint::CPU, false, false, 0, false);
...
MemEventBase *
MemHierarchyInterface::createMemEvent(SimpleMem::Request *req) const {
  ...
  if (SimpleMem::Request::Write == req->cmd) {
    ...
    if (req->data.size() != req->size)
      output.output("Warning: In memHierarchyInterface, write request size "
                    "does not match payload size. Request size: %zu. Payload "
                    "size: %zu. MemEvent will use payload size\n",
...
MemEventBase *
MemHierarchyInterface::createCustomEvent(SimpleMem::Request *req) const {
  Addr baseAddr = (req->addrs[0]) & baseAddrMask_;
  CustomCmdEvent *cme =
      new CustomCmdEvent(getName().c_str(), req->addrs[0], baseAddr,
  ...
  if (req->data.size() != 0) {
    cme->setPayload(
        req->data); // Note this updates cme->size to payload.size()...
    cme->setSize(
        req->size); // Assume this is what we want, not the copied payload size
  }
...
/* Match response to request. Update request with results. Return request to
 * processor */
SimpleMem::Request *MemHierarchyInterface::processIncoming(MemEventBase *ev) {
  ...
  std::map<MemEventBase::id_type, SimpleMem::Request *>::iterator i =
  ...
  if (i != requests_.end()) {
    ...
    output.fatal(CALL_INFO, -1,
                 "(%s interface) Unable to find matching request. Event: %s\n",
                 getName().c_str(), ev->getVerboseString().c_str());
  }
  return req;
}

void MemHierarchyInterface::updateRequest(SimpleMem::Request *req,
                                          MemEvent *me) const {
  switch (me->getCmd()) {
...
void MemHierarchyInterface::updateCustomRequest(SimpleMem::Request *req,
...
bool MemHierarchyInterface::initialize(const std::string &linkName,
                                       HandlerBase *handler) {
  recvHandler_ = handler;
  if (NULL == recvHandler_)
