...
MemHierarchyScratchInterface::MemHierarchyScratchInterface(
    SST::ComponentId_t id, Params &params, TimeConverter *time,
    HandlerBase *handler)
    : SimpleMem(id, params) {
  ...
  bool found;
  UnitAlgebra size =
      UnitAlgebra(params.find<std::string>("scratchpad_size", "0B", found));
  if (!found)
    output.fatal(
        CALL_INFO, -1,
  ...
  if (!size.hasUnits("B"))
    output.fatal(CALL_INFO, -1,
                 "Invalid param (%s): scratchpad_size - must have units of "
                 "'B'. SI units ok. You specified '%s'\n",
  ...
  if (NULL == recvHandler_)
    ...
    link_ = configureLink(
        "port", new Event::Handler<MemHierarchyScratchInterface>(
                    this, &MemHierarchyScratchInterface::handleIncoming));

  if (!link_)
    output.fatal(CALL_INFO, -1,
                 "%s, Error: unable to configure link on port 'port'\n",
                 getName().c_str());
}

void MemHierarchyScratchInterface::init(unsigned int phase) {
  if (!phase) {
    // Name, NULLCMD, Endpoint type, inclusive of all upper levels, will send
    // writeback acks, line size
    link_->sendInitData(new MemEventInitCoherence(getName(), Endpoint::CPU,
                                                  false, false, 0, false));
  }
  ...
  while (SST::Event *ev = link_->recvInitData()) {
    ...
    if (memEvent) {
      if (memEvent->getInitCmd() == MemEventInit::InitCommand::Coherence) {
        MemEventInitCoherence *memEventC =
...
void MemHierarchyScratchInterface::sendInitData(SimpleMem::Request *req) {
  MemEventInit *event =
      new MemEventInit(getName(), Command::GetX, req->addrs[0], req->data);
  if (initDone_)
...
MemEvent *
MemHierarchyScratchInterface::createMemEvent(SimpleMem::Request *req) const {
  ...
  default:
    output.fatal(CALL_INFO, -1,
                 "MemHierarchyScratchInterface received unknown command in "
                 "createMemEvent\n");
  }
  ...
  /* Set remote memory accesses to noncacheable so that any cache avoids trying
  ...
  if (SimpleMem::Request::Write == req->cmd) {
    ...
    if (req->data.size() != req->size)
      output.output("Warning: In memHierarchyScratchInterface, write request "
                    "size does not match payload size. Request size: %zu, "
                    "Payload size: %zu. Using payload size.\n",
...
MoveEvent *
MemHierarchyScratchInterface::createMoveEvent(SimpleMem::Request *req) const {
  Command cmd = Command::NULLCMD;
  ...
  MoveEvent *me = new MoveEvent(getName(), req->addrs[1], req->addrs[1],
                                req->addrs[0], req->addrs[0], cmd);
...
SimpleMem::Request *
MemHierarchyScratchInterface::processIncoming(MemEventBase *ev) {
  SimpleMem::Request *req = NULL;
  Command cmd = ev->getCmd();
  SST::Event::id_type origID = ev->getResponseToID();

  std::map<SST::Event::id_type, SimpleMem::Request *>::iterator i =
      requests_.find(origID);
  if (i != requests_.end()) {
    req = i->second;
    requests_.erase(i);
    ...
    output.fatal(CALL_INFO, -1,
                 "Unable to find matching request.  Cmd = %s, respID = %" PRIx64
                 "\n",
...
void MemHierarchyScratchInterface::updateRequest(SimpleMem::Request *req,
                                                 MemEventBase *me) const {
  switch (me->getCmd()) {
...
bool MemHierarchyScratchInterface::initialize(const std::string &linkName,
                                              HandlerBase *handler) {
  recvHandler_ = handler;
  if (NULL == recvHandler_)
    link_ = configureLink(linkName);
  else
    link_ = configureLink(
        linkName, new Event::Handler<MemHierarchyScratchInterface>(
                      this, &MemHierarchyScratchInterface::handleIncoming));
