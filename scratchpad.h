...
public:
  ...
  SST_ELI_REGISTER_COMPONENT(Scratchpad, "memHierarchy", "Scratchpad",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"size", "(string) Size of the scratchpad in bytes (B), SI units ok",
       NULL},
      {"scratch_line_size",
       "(string) Number of bytes in a scratch line with units. 'size' must be "
       "divisible by this number.",
       "64B"},
      {"memory_line_size",
       "(string) Number of bytes in a remote memory line with units. Used to "
       "set base addresses for routing.",
       "64B"},
      {"backing",
       "(string) Type of backing store to use. Options: 'none' - no backing "
       "store (only use if simulation does not require correct memory values), "
       "'malloc', or 'mmap'",
       "malloc"},
      {"backing_size_unit",
       "(string) For 'malloc' backing stores, malloc granularity", "1MiB"},
      {"memory_addr_offset",
       "(uint) Amount to offset remote addresses by. Default is 'size' so that "
       "remote memory addresses start at 0",
       "size"},
      {"response_per_cycle",
       "(uint) Maximum number of responses to return to processor each cycle. "
       "0 is unlimited",
       "0"},
      {"backendConvertor",
       "(string) Backend convertor to use for the scratchpad",
       "memHierarchy.scratchpadBackendConvertor"},
      {"debug",
       "(uint) Where to print debug output. Options: 0[no output], 1[stdout], "
       "2[stderr], 3[file]",
       "0"},
      {"debug_level", "(uint) Debug verbosity level. Between 0 and 10", "0"})

  SST_ELI_DOCUMENT_PORTS({"cpu",
                          "Link to cpu/cache on the cpu side",
                          {"memHierarchy.MemEventBase"}},
                         {"memory",
                          "Direct link to a memory or bus",
                          {"memHierarchy.MemEventBase"}},
                         {"network",
                          "Network link to memory",
                          {"memHierarchy.MemRtrEvent"}})

  SST_ELI_DOCUMENT_STATISTICS(
      {"request_received_scratch_read",
       "Number of scratchpad reads received from CPU", "count", 1},
      {"request_received_scratch_write",
       "Number of scratchpad writes received from CPU", "count", 1},
      {"request_received_remote_read",
       "Number of remote memory reads received from CPU", "count", 1},
      {"request_received_remote_write",
       "Number of remote memory writes received from CPU", "count", 1},
      {"request_received_scratch_get",
       "Number of scratchpad Gets received from CPU (copy from memory to "
       "scratch)",
       "count", 1},
      {"request_received_scratch_put",
       "Number of scratchpad Puts received from CPU (copy from scratch to "
       "memory)",
       "count", 1},
      {"request_issued_scratch_read",
       "Number of scratchpad reads issued to scratchpad", "count", 1},
      {"request_issued_scratch_write",
       "Number of scratchpad writes issued to scratchpad", "count", 1})

  SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS(
      {"backendConvertor",
       "Convertor to interface to memory timing model (backend)",
...
private:
  ...
  uint64_t scratchSize_; // Size of the total scratchpad in bytes - any address
                         // above this is assumed to address remote memory
  uint64_t scratchLineSize_; // Size of each line in the scratchpad in bytes

  // Parameters - memory
  uint64_t
      remoteAddrOffset_; // Offset for remote addresses, defaults to scratchSize
                         // (i.e., CPU addr scratchSize = mem addr 0)
  ...
  uint32_t deriveSize(Addr addr, Addr baseAddr, Addr requestAddr,
  ...
  public:
    ...
    MemEvent
        *remoteWrite; // For Put requests, collect scratch read responses here
    uint32_t count;   // Number of lines we are waiting on - when 0, the request
                    // is complete i.e., for a read or write, just 1, for a get
                    // or put, the size/lineSize

    OutstandingEvent(MemEventBase *request, MemEventBase *response)
        : request(request), response(response), remoteWrite(nullptr), count(0) {
    }
    OutstandingEvent(MemEventBase *request, MemEventBase *response,
                     MemEvent *write)
  ...
  public:
    ...
    MemEvent *scratch; // If we are waiting to send an event to scratch, the
                       // event to send
    ...
    MSHREntry(SST::Event::id_type id, Command cmd, MemEvent *scratch = nullptr)
        : id(id), scratch(scratch), cmd(cmd), needData(false), needAck(false) {}

    // For Get events which have not yet started but remote read has been sent
    MSHREntry(SST::Event::id_type id, Command cmd, bool needData)
        : id(id), scratch(nullptr), cmd(cmd), needData(needData),
          needAck(false) {}

    // For events that have started
    MSHREntry(SST::Event::id_type id, Command cmd, bool needData, bool needAck)
        : id(id), scratch(nullptr), cmd(cmd), needData(needData),
          needAck(needAck) {}
  ...
  std::map<SST::Event::id_type, SST::Event::id_type>
      responseIDMap_; // Map a forwarded request ID to a original request ID
  std::map<SST::Event::id_type, Addr>
      responseIDAddrMap_; // Map an outstanding scratch request ID to the
                          // request's baseAddr
  std::map<SST::Event::id_type, OutstandingEvent>
      outstandingEventList_;                  // List of all outstanding events
  ...
  bool
      directory_; // Whether or not a directory is managing the caches - if so
                  // we cannot assume on a writeback that the data is not cached
  std::vector<bool>
      cacheStatus_; // One entry per scratchpad line, whether line may be cached
  std::map<SST::Event::id_type, uint64_t>
      cacheCounters_; // Map of a Get or Put ID to the number of cache acks/data
                      // responses we are waiting for
