...
public:
  ...
  SST_ELI_REGISTER_COMPONENT(
      BroadcastShim, "memHierarchy", "BroadcastShim",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Used to connect a processor to multiple sieves (e.g., for "
      "private/semi-private last-level cache modeling)",
      COMPONENT_CATEGORY_MEMORY)

  SST_ELI_DOCUMENT_PORTS({"cpu_alloc_link_%(port)d",
                          "Link to CPU's allocation port",
                          {"memHierarchy.AllocTrackEvent"}},
                         {"sieve_alloc_link_%(port)d",
                          "Link to sieve's allocation port",
                          {"memHierarchy.AllocTrackEvent"}})
