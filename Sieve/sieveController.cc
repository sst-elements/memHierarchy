...
void Sieve::processAllocEvent(SST::Event *event) {
  ...
  if (ev->getType() == AllocTrackEvent::ALLOC) {
    ...
    if (activeAllocMap.find(ev->getVirtualAddress()) != activeAllocMap.end()) {
      // sometimes ariel replaces both malloc() and _malloc(), so we get two
      // reports. Just ignore the first.
      output_->debug(_INFO_,
                     "Trying to add allocation event at an address (%p %" PRIx64
                     ") with an active allocation. %" PRIu64 "\n",
                     ev, ev->getVirtualAddress(),
                     (uint64_t)activeAllocMap.size());
    }
#endif
    delete ev;
    ...
    if (targ != activeAllocMap.end()) {
      ...
      // In this case ALWAYS delete the entry from active alloc & delete the
      // event
    ...
    output_->fatal(CALL_INFO, -1,
                   "Unrecognized Allocation Tracking Event Type \n");
  }
}
...
void Sieve::processEvent(SST::Event *ev) {
  ...
  if (miss) { /* Miss.  If needed, evict candidate */
    ...
    if (listener_) {
      CacheListenerNotification notify(
          ...
          event->getInstructionPointer(), event->getSize(),
          isRead ? READ : WRITE, MISS);
      listener_->notifyAccess(notify);
    }

  } else {
    if (cmd == Command::GetS)
      statReadHits->addData(1);
    else
      statWriteHits->addData(1);
  }
  ...
  output_->debug(
      _L4_,
      "%s, Src = %s, Cmd = %s, BaseAddr = %" PRIx64 ", Addr = %" PRIx64
      ", VA = %" PRIx64 ", PC = %" PRIx64 ", Size = %d: %s\n",
      getName().c_str(), event->getSrc().c_str(), CommandString[(int)cmd],
      baseAddr, event->getAddr(), event->getVirtualAddress(),
      event->getInstructionPointer(), event->getSize(), miss ? "MISS" : "HIT");
  if (miss)
    output_->debug(_L5_, "%s, Replaced address %" PRIx64 "\n",
...
void Sieve::init(unsigned int phase) {
  if (!phase) {
    for (int i = 0; i < cpuLinkCount_; i++) {
      cpuLinks_[i]->sendInitData(
          new Interfaces::StringEvent("SST::MemHierarchy::MemEvent"));
    }
  }
...
void Sieve::outputStats(int marker) {
  ...
  for (allocCountMap_t::iterator i = allocMap.begin(); i != allocMap.end();
       i++) {
  ...
  for (vector<uint64_t>::iterator it = entriesToErase.begin();
       it != entriesToErase.end(); it++) {
...
void Sieve::finish() { outputStats(-1); }
