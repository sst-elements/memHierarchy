...
MemoryManagerSieve::MemoryManagerSieve(ComponentId_t id, Params &params)
  ...
  // Load a memory manager to actually do the translation -> we're just
  // interception allocation events
  ...
  while (isPortConnected(linkname)) {
    SST::Link *link = configureLink(
        linkname, "50ps"); // No handler since never receives anything
  ...
  if (numPorts == 0) {
    output->output(
        "MemoryManagerSieve - WARNING: No allocation links found...did you "
        "mean to do this? Note: this subcomponent MUST be instantiated as a "
        "named subcomponent to work correctly!\n");
  }
}
...
bool MemoryManagerSieve::allocateMalloc(const uint64_t size,
  ...
  for (int i = 0; i < allocLink.size(); i++) {
    allocLink[i]->send(
        new AllocTrackEvent(AllocTrackEvent::ALLOC, addr, size, level, ip));
  }
  return true;
}
...
void MemoryManagerSieve::freeMalloc(const uint64_t virtualAddress) {
  ...
  for (int i = 0; i < allocLink.size(); i++) {
    allocLink[i]->send(
        new AllocTrackEvent(AllocTrackEvent::FREE, virtualAddress, 0, 0, 0));
  }
}

void MemoryManagerSieve::printStats() { memmgr->printStats(); }
