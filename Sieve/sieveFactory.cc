    ...
    namespace MemHierarchy {
        ...
        Sieve::Sieve(ComponentId_t id, Params &params) : Component(id) {
            ...
            if (debugLevel < 0 || debugLevel > 10)
                output_->fatal(CALL_INFO, -1, "Debugging level must be between 0 and 10. \n");
            ...
            if (-1 == lineSize)
                output_->fatal(CALL_INFO, -1,
                               "Param not specified: cache_line_size - number of bytes in a cacheline (block size)\n");

            fixByteUnits(sizeStr);
            UnitAlgebra ua(sizeStr);
            if (!ua.hasUnits("B")) {
                output_->fatal(CALL_INFO, -1,
                               "Invalid param: cache_size - must have units of bytes (e.g., B, KB,etc.)\n");
            }
            ...
            ReplacementMgr *replManager = new LRUReplacementMgr(output_, numLines, associativity,
                                                                true);
            cacheArray_ = new SetAssociativeArray(output_, numLines, lineSize, associativity,
                                                  replManager, ht, false);
        ...
        void Sieve::configureLinks() {
            ...
            for (int i = 0; i < cpuLinkCount_; i++) {
                ...
                std::string ln = linkName.str();
                allocLinks_[i] = configureLink(ln, "50ps", new Event::Handler<Sieve>(this,
                                                                                     &Sieve::processAllocEvent));
            }
        }
        ...
        void Sieve::createProfiler(const Params &params) {
            ...
            if (profiler.empty()) {
                ...
                listener_ = loadAnonymousSubComponent<CacheListener>(profiler, "proifiler", 0,
