...
Sieve::Sieve(ComponentId_t id, Params &params) : Component(id) {
  ...
  output_->init("--->  ", debugLevel, 0,
                (Output::output_location_t)params.find<int>("debug", 0));
  if (debugLevel < 0 || debugLevel > 10)
    output_->fatal(CALL_INFO, -1,
                   "Debugging level must be between 0 and 10. \n");
  ...
  if (-1 == lineSize_)
    output_->fatal(CALL_INFO, -1,
                   "Param not specified: cache_line_size - number of bytes in "
                   "a cacheline (block size)\n");

  fixByteUnits(sizeStr);
  UnitAlgebra ua(sizeStr);
  if (!ua.hasUnits("B")) {
    output_->fatal(CALL_INFO, -1,
                   "Invalid param: cache_size - must have units of bytes "
                   "(e.g., B, KB,etc.)\n");
  }
  ...
  /* ---------------- Initialization ----------------- */
  HashFunction *ht = loadAnonymousSubComponent<HashFunction>(
      "memHierarchy.hash.none", "hash", 0, ComponentInfo::SHARE_NONE, params);
  ReplacementPolicy *replManager = loadUserSubComponent<ReplacementPolicy>(
      "replacement", ComponentInfo::SHARE_NONE, numLines, associativity);
  if (!replManager)
    replManager = loadAnonymousSubComponent<ReplacementPolicy>(
        "memHierarchy.replacement.lru", "replacement", 0,
        ComponentInfo::SHARE_NONE, params, numLines, associativity);
  cacheArray_ = new CacheArray<SharedCacheLine>(
      output_, numLines, associativity, lineSize_, replManager, ht);

  output_->debug(_INFO_,
                 "--------------------------- Initializing [Sieve]: %s... \n",
  ...
  statUnassocWriteMisses =
      registerStatistic<uint64_t>("UnassociatedWriteMisses");
}
...
void Sieve::configureLinks() {
  ...
  if (cpuLinkCount_ < 1)
    output_->fatal(CALL_INFO, -1,
                   "Did not find any connected links on ports cpu_link_n\n");
  ...
  for (int i = 0; i < cpuLinkCount_; i++) {
    ...
    std::string ln = linkName.str();
    allocLinks_[i] = configureLink(
        ln, "50ps", new Event::Handler<Sieve>(this, &Sieve::processAllocEvent));
  }
}
...
void Sieve::createProfiler(const Params &params) {
  ...
  if (profiler.empty()) {
    ...
    listener_ = loadAnonymousSubComponent<CacheListener>(
        profiler, "profiler", 0, ComponentInfo::INSERT_STATS, profilerParams);
  }
}
