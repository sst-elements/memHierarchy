...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      MemoryManagerSieve, "memHierarchy", "MemoryManagerSieve",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Memory manager for interfacing to MemSieve",
      SST::ArielComponent::ArielMemoryManager)

  SST_ELI_DOCUMENT_PORTS({"alloc_link_%(corecound)d",
                          "Each core's link memSieve",
                          {"memHierarchy.AllocTrackEvent"}})

  SST_ELI_DOCUMENT_SUBCOMPONENT_SLOTS(
      {"memmgr", "Which memory manager to use for translation",
