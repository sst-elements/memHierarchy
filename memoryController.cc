...
/*************************** Memory Controller ********************/
MemController::MemController(ComponentId_t id, Params &params)
  ...
  if (found) {
    out.output("%s, ** Found deprecated parameter: direct_link ** The value of "
               "this parameter is now auto-detected by the link configuration "
               "in your input deck. Remove this parameter from your input deck "
  ...
  if (!memory) { /* Try to load from our parameters (legacy mode 1) */
    out.output("%s, WARNING: loading backend in legacy mode (from parameter "
    ...
    memory = loadAnonymousSubComponent<MemBackend>(
        name, "backend", 0,
      ...
      if (listenerMod != "") {
        ...
        CacheListener *loadedListener =
            loadAnonymousSubComponent<CacheListener>(
                listenerMod, "listener", i, ComponentInfo::INSERT_STATS,
                listenerParams);
  ...
  clockHandler_ =
      new Clock::Handler<MemController>(this, &MemController::clock);
  ...
  customCommandHandler_ = loadUserSubComponent<CustomCmdMemHandler>(
      ...
      std::bind(
          static_cast<void (MemController::*)(
              Addr, size_t, std::vector<uint8_t> &)>(&MemController::readData),
          this, _1, _2, _3),
      std::bind(
          static_cast<void (MemController::*)(Addr, std::vector<uint8_t> *)>(
              &MemController::writeData),
          this, _1, _2));
  if (nullptr == customCommandHandler_) {
    ...
    if (customHandlerName != "") {
      customCommandHandler_ = loadAnonymousSubComponent<CustomCmdMemHandler>(
          ...
          params,
          std::bind(static_cast<void (MemController::*)(
                        Addr, size_t, std::vector<uint8_t> &)>(
                        &MemController::readData),
                    this, _1, _2, _3),
          std::bind(
              static_cast<void (MemController::*)(
                  Addr, std::vector<uint8_t> *)>(&MemController::writeData),
              this, _1, _2));
    }
  }
}
...
void MemController::readData(Addr addr, size_t bytes,
...
Addr MemController::translateToLocal(Addr addr) {
  ...
  if (is_debug_addr(addr)) {
    Debug(_L10_,
          "\tConverting global address 0x%" PRIx64
          " to local address 0x%" PRIx64 "\n",
          addr, rAddr);
  }
  return rAddr;
}
...
Addr MemController::translateToGlobal(Addr addr) {
  ...
  if (is_debug_addr(rAddr)) {
    Debug(_L10_,
          "\tConverting local address 0x%" PRIx64
          " to global address 0x%" PRIx64 "\n",
          addr, rAddr);
  }
  return rAddr;
}
