...
public:
  ...
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      MemNetBridge, "memHierarchy", "MemNetBridge",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Merlin::Bridge::Translator for memory network bridging",
      SST::Merlin::Bridge::Translator)

  SST_ELI_DOCUMENT_PARAMS({"debug",
                           "(int) Print debug information. Options: 0[no "
                           "output], 1[stdout], 2[stderr], 3[file]",
                           "0"},
                          {"debug_level",
                           "(int) Debugging level. Between 0 and 10", "0"})
  ...
  SimpleNetwork::Request *translate(SimpleNetwork::Request *req,
                                    uint8_t fromNetwork);
  SimpleNetwork::Request *initTranslate(SimpleNetwork::Request *req,
                                        uint8_t fromNetwork);
