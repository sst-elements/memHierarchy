...
DMAEngine::DMAEngine(ComponentId_t id, Params &params) : Component(id) {
  dbg.init("@t:DMAEngine::@p():@l " + getName() + ": ", 0, 0,
           (Output::output_location_t)params.find<int>("debug", 0));
  statsOutputTarget =
      (Output::output_location_t)params.find<int>("printStats", 0);

  TimeConverter *tc =
      registerClock(params.find<std::string>("clockRate", "1 GHz"),
                    new Clock::Handler<DMAEngine>(this, &DMAEngine::clock));
  commandLink = configureLink("cmdLink", tc, NULL);
  if (NULL == commandLink)
    dbg.fatal(CALL_INFO, 1, "Missing cmdLink\n");

  if (!isPortConnected("netLink"))
    dbg.fatal(CALL_INFO, 1, "Missing netLink\n");
  /*
...
void DMAEngine::init(unsigned int phase) { networkLink->init(phase); }

void DMAEngine::setup(void) {
  networkLink->setup();
  bool blocksizeSet = false;

  /*    const std::vector<MemNIC::PeerInfo_t> &peers =
     networkLink->getPeerInfo(); for (
     std::vector<MemNIC::PeerInfo_t>::const_iterator i = peers.begin() ; i !=
     peers.end() ; ++i ) { switch (i->first.type) { case
     MemNIC::TypeDirectoryCtrl: break; case MemNIC::TypeCache: if (
     !blocksizeSet ) { blocksize = i->second.blocksize; blocksizeSet = true;
              }
          default:
              ;
          }
...
void DMAEngine::finish(void) {
  ...
  out.output("DMA Controller %s stats:\n"
             ...
             getName().c_str(), numTransfers, bytesTransferred);
}
...
bool DMAEngine::clock(Cycle_t cycle) {
  ...
  /*        Request* req = findRequest(me->getResponseToID());
          ...
          if ( NULL == req ) {
              dbg.debug(_L10_, "Received Packet for which we have no response ID
  waiting.  ID received: (%" PRIx64 ", %d)\n", me->getResponseToID().first,
  me->getResponseToID().second);
          }
  #endif
...
void DMAEngine::startRequest(Request *req) {
  ...
  MemHierarchyInterface dbg.debug(_L10_, "Received request to transfer from %#"
...
void DMAEngine::processPacket(Request *req, MemEventBase *ev) {
  assert(0);
  /*  Needs to be updated with current MemHierarchy Commands/States,
  MemHierarchyInterface if ( ev->getCmd() == GetSResp) { if (
  !req->loadKeys.count(ev->getResponseToID()) ) { dbg.fatal(CALL_INFO, 1,
  "Received GetSResp, but we don't have a corresponding request in
  loadKeys!\n");
      }
      size_t offset = ev->getAddr() - req->getSrc();
      req->loadKeys.erase(ev->getResponseToID());
      MemEvent *storeEV = new MemEvent(this, (req->getDst() + offset),
  (req->getDst() + offset), GetX); storeEV->setFlag(MemEvent::F_NONCACHEABLE);
      storeEV->setPayload(ev->getPayload());
      storeEV->setDst(networkLink->findTargetDestination(req->getDst() +
  offset)); req->storeKeys.insert(storeEV->getID()); networkLink->send(storeEV);
      ...
      if ( req->storeKeys.empty() ) {
          ...
          dbg.debug(_L10_, "Request to transfer 0x%" PRIx64 " to 0x%" PRIx64 "
  is complete.\n", req->getSrc(), req->getDst()); delete req;
      }
  } else {
      dbg.fatal(CALL_INFO, 1, "Received unexpected message %s 0x%" PRIx64 " from
