...
public:
  ...
  SST_ELI_REGISTER_COMPONENT(
      MemCacheController, "memHierarchy", "MemCacheController",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Memory controller, interfaces to a main memory model for timing",
      COMPONENT_CATEGORY_MEMORY)

#define MEMCACHECONTROLLER_ELI_PARAMS                                          \
  {"clock", "(string) Clock frequency of controller, with units (e.g., 1GHz)", \
   NULL},                                                                      \
      {"num_caches", "(uint) Total number of memory caches", "1"},             \
      {"cache_num", "(uint) Index of this cache between 0 and num_caches-1",   \
       "0"},                                                                   \
...
#define MEMCACHE_ELI_SUBCOMPONENTSLOTS                                         \
  {"backend", "Memory controller and/or memory timing model.",                 \
   "SST::MemHierarchy::MemBackend"},                                           \
      {"backendConvertor",                                                     \
       "Convertor to translate incoming memory events for the backend. "       \
       "Loaded automatically based on backend type.",                          \
      ...
      {"cpulink", "CPU-side link manager (e.g., to caches/cpu)",               \
...
protected:
   ...
   *  HIT_TAG: The lookup will be a hit but we need to check the tag first
   * (needed for a write) MISS: The lookup will be a miss; the current access is
   * to check the tag MISS_WB: The lookup will be a miss and require a
   * writeback; the current access is to check the tag STALL: Another access for
   * the same line is outstanding, stall until it finishes -> may not actually
   * be how MCDRAM works... DATA: Sent a request for data to the remote memroy
   */
  ...
  struct MemAccessRecord {
    ...
    MemAccessRecord()
        : event(nullptr), status(AccessStatus::MISS), reqev(nullptr) {}
    MemAccessRecord(MemEvent *ev, AccessStatus stat)
        : event(ev), status(stat), reqev(nullptr) {}
  };
