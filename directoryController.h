...
public:
  ...
  SST_ELI_DOCUMENT_STATISTICS(
      ...
      {"GetXResp_recv", "Event received: GetXResp (exclusive data)", "count",
       1},
      {"PutS_recv", "Event received: PutS (shared replacement)", "count", 2},
      {"PutE_recv", "Event received: PutE (clean exclusive replacement)",
       "count", 2},
      {"PutM_recv", "Event received: PutM (dirty exclusive replacement)",
       "count", 2},
      {"PutX_recv", "Event received: PutX (downgrade exclusive to shared)",
       "count", 2},
      {"ForceInv_recv", "Event received: ForceInv (invalidate & drop data)",
       "count", 2},
      {"FetchInv_recv", "Event received: FetchInv (invalidate & return data)",
       "count", 2},
      {"FetchResp_recv",
       "Event received: FetchResp (response to FetchInv/Fetch)", "count", 2},
      {"FetchXResp_recv", "Event received: FetchXResp (response to FetchInvX)",
       "count", 2},
      {"AckInv_recv",
       "Event received: AckInv (response to Inv/FetchInv/ForceInv)", "count",
       2},
      {"FlushLine_recv", "Event received: FlushLine (flush, don't invalidate)",
       "count", 2},
      {"FlushLineInv_recv",
       "Event received: FlushLineInv (flush and invalidate)", "count", 2},
      {"FlushLineResp_recv",
       "Event received: FlushLineResp (response to FlushLine/Inv)", "count", 2},
      ...
      {"eventSent_read_directory_entry",
       "Event sent: Read (GetS) a directory entry from memory", "count", 2},
      {"eventSent_write_directory_entry",
       "Event sent: Write (PutM) a directory entry to memory", "count", 2},
      ...
      {"eventSent_GetSResp", "Event sent: GetSResp (shared data response)",
       "count", 1},
      {"eventSent_GetXResp", "Event sent: GetXResp (exclusive data response)",
       "count", 1},
      ...
      {"default_stat",
       "Default statistic. If not 0 then a statistic is missing", "", 1})
...
private:
  ...
  struct DirEntry {
    ...
    std::string getString() {
      ...
      str << " Sharers: [";
      bool comma = false;
      for (std::set<std::string>::iterator it = sharers.begin();
           it != sharers.end(); it++) {
        if (comma)
          str << ",";
        str << *it;
    ...
    bool isSharer(std::string shr) {
      return sharers.find(shr) != sharers.end();
    }
  ...
  bool retrieveDirEntry(DirEntry *entry, MemEvent *event,
                        bool inMSHR); // Simulate fetching entry from memory
  ...
  void issueInvalidation(std::string dst, MemEvent *event, DirEntry *entry,
                         Command cmd);
  void sendDataResponse(MemEvent *event, DirEntry *entry,
                        std::vector<uint8_t> &data, Command cmd,
                        uint32_t flags = 0);
  ...
  MSHR *mshr;
  std::unordered_map<Addr, DirEntry *>
      directory; // Master list of all directory entries, including noncached
                 // ones
