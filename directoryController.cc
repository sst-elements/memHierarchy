...
DirectoryController::DirectoryController(ComponentId_t id, Params &params)
  ...
  if (found) {
    out.output(
        ...
        "does not use multiple virtual channels. Remove this parameter from "
  ...
  if (!found)
    region.start = params.find<uint64_t>("memNIC.addr_range_start", 0, found);
  if (!found)
    region.start = params.find<uint64_t>("memlink.addr_range_start", 0, found);
  gotRegion |= found;

  region.end = params.find<uint64_t>("addr_range_end", (uint64_t)-1, found);
  if (!found)
    region.end =
        params.find<uint64_t>("memNIC.addr_range_end", (uint64_t)-1, found);
  if (!found)
    region.end =
        params.find<uint64_t>("memlink.addr_range_end", (uint64_t)-1, found);
  gotRegion |= found;

  string ilSize = params.find<std::string>("interleave_size", "0B", found);
  if (!found)
    ilSize = params.find<std::string>("memNIC.interleave_size", "0B", found);
  if (!found)
    ilSize = params.find<std::string>("memlink.interleave_size", "0B", found);
  gotRegion |= found;

  string ilStep = params.find<std::string>("interleave_step", "0B", found);
  if (!found)
    ilStep = params.find<std::string>("memNIC.interleave_step", "0B", found);
  if (!found)
    ilStep = params.find<std::string>("memlink.interleave_step", "0B", found);
  gotRegion |= found;
  ...
  UnitAlgebra packetSize =
      UnitAlgebra(params.find<std::string>("min_packet_size", "8B"));
  if (!packetSize.hasUnits("B"))
    dbg.fatal(CALL_INFO, -1,
              "%s, Invalid param: min_packet_size - must have units of bytes "
              "(B). SI units are ok. You specified '%s'\n",
  ...
  if (!UnitAlgebra(ilSize).hasUnits("B") ||
      region.interleaveSize % cacheLineSize != 0) {
    dbg.fatal(
        CALL_INFO, -1,
        "Invalid param(%s): interleave_size - must be specified in bytes with "
        "units (SI units OK) and must also be a multiple of cache_line_size. "
        "This definition has CHANGED. Example: If you used to set this to '1', "
        "change it to '1KiB'. You specified %s\n",
        getName().c_str(), ilSize.c_str());
  }
  if (!UnitAlgebra(ilStep).hasUnits("B") ||
      region.interleaveStep % cacheLineSize != 0) {
    dbg.fatal(
        CALL_INFO, -1,
        "Invalid param(%s): interleave_step - must be specified in bytes with "
        "units (SI units OK) and must also be a multiple of cache_line_size. "
        "This definition has CHANGED. Example: If you used to set this to '4', "
        "change it to '4KiB'. You specified %s\n",
   ...
   *  Regions & memory name
   *  *****************************
   *  In earlier versions of the directoryController there was a 1-1
   * correspondence between a DC and a memory controller (MC). In this case, the
   * DC had an associated address region, a name of the MC it controlled, and
   * the DC passed the region to the MC.
   *
   *  Later, the requirement was relaxed so that there could be a many-1
   * correspondance. In this case, multiple DCs could address a single MC. To do
   * this, MCs declare their own region but the DC uses it's 'memory name' to
   * only address a single MC. For backward compatibility, the DC still sends
   * its region to the named MC, the MC can optionally ignore it.
   *
   *  Now, we want to enable many-many. To do this, DCs and MCs MUST declare
   * their own regions and no memory name is needed. Backward compatiblity is
   * not assured: if you declare a memory name, the DC assumes you want 1-1 or
   * many-1. If you don't, it assumes you gave the MCs their own region. We
   * cannot error check from the parameters...
   */
  ...
  if (cpuLink || memLink) {
    ...
    if (gotRegion) {
      ...
      if (cpuLink->getRegion() != region) {
        out.output(
            CALL_INFO,
            "%s, Warning: getting region parameters (addr_range_start/end, "
            "interleave_step/size) from link subcomponent."
            " In the future this will not be supported and region parameters "
            "should be declared in the directory's parameters instead.\n",
            getName().c_str());
      }
    ...
    cpuLink->setRecvHandler(new Event::Handler<DirectoryController>(
        this, &DirectoryController::handlePacket));
    ...
    if (!memLink) {
      ...
      if (memoryName != "")
        dbg.output(
            CALL_INFO,
            "%s, Warning: parameter 'net_memory_name' is deprecated in order "
            "to support many<->many communication between DCs and MCs.\n"
            "Instead of using net_memory_name, ensure that the directory and "
            "memory controllers all specify their own address regions "
            "(address_range_start/end, interleave_step/size).\n"
            "In the future, the directory controller will NOT automatically "
            "force its region parameters on its named memory controller\n",
            getName().c_str());
    } else {
      memLink->setRecvHandler(new Event::Handler<DirectoryController>(
          this, &DirectoryController::handlePacket));
      memLink->setName(getName());
    }
  } else {
    /* Set up links/network to cache & memory the old way -> and fixup params
     * accordingly */
    fixupParam(params, "network_bw", "memNIC.network_bw");
    fixupParam(params, "network_input_buffer_size",
               "memNIC.network_input_buffer_size");
    fixupParam(params, "network_output_buffer_size",
               "memNIC.network_output_buffer_size");
    ...
    if (isPortConnected("network_ack"))
      portCount++;
    if (isPortConnected("network_fwd"))
      portCount++;
    if (isPortConnected("network_data"))
      portCount++;
    if (portCount == 4) {
      ...
      cpuLink = loadAnonymousSubComponent<MemLinkBase>(
    ...
    if (isPortConnected("memory")) {
      ...
      memLink = loadAnonymousSubComponent<MemLinkBase>(
          "memHierarchy.MemLink", "memlink", 0,
          ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, memParams);
      memLink->setRecvHandler(new Event::Handler<DirectoryController>(
          this, &DirectoryController::handlePacket));
      if (!memLink) {
        dbg.fatal(
  ...
  clockHandler = new Clock::Handler<DirectoryController>(
      this, &DirectoryController::clock);
  defaultTimeBase =
  ...
  // Register statistics
  stat_replacementRequestLatency =
      registerStatistic<uint64_t>("replacement_request_latency");
  ...
  stat_eventRecv[(int)Command::GetSX] =
      registerStatistic<uint64_t>("GetSX_recv");
  ...
  stat_eventRecv[(int)Command::FetchResp] =
      registerStatistic<uint64_t>("FetchResp_recv");
  stat_eventRecv[(int)Command::FetchXResp] =
      registerStatistic<uint64_t>("FetchXResp_recv");
  stat_eventRecv[(int)Command::GetSResp] =
      registerStatistic<uint64_t>("GetSResp_recv");
  stat_eventRecv[(int)Command::GetXResp] =
      registerStatistic<uint64_t>("GetXResp_recv");
  stat_eventRecv[(int)Command::ForceInv] =
      registerStatistic<uint64_t>("ForceInv_recv");
  stat_eventRecv[(int)Command::FetchInv] =
      registerStatistic<uint64_t>("FetchInv_recv");
  stat_eventRecv[(int)Command::AckInv] =
      registerStatistic<uint64_t>("AckInv_recv");
  stat_eventRecv[(int)Command::FlushLine] =
      registerStatistic<uint64_t>("FlushLine_recv");
  stat_eventRecv[(int)Command::FlushLineInv] =
      registerStatistic<uint64_t>("FlushLineInv_recv");
  stat_eventRecv[(int)Command::FlushLineResp] =
      registerStatistic<uint64_t>("FlushLineResp_recv");
  stat_noncacheRecv[(int)Command::GetS] =
      registerStatistic<uint64_t>("GetS_uncache_recv");
  stat_noncacheRecv[(int)Command::GetX] =
      registerStatistic<uint64_t>("GetX_uncache_recv");
  stat_noncacheRecv[(int)Command::GetSX] =
      registerStatistic<uint64_t>("GetSX_uncache_recv");
  stat_noncacheRecv[(int)Command::GetSResp] =
      registerStatistic<uint64_t>("GetSResp_uncache_recv");
  stat_noncacheRecv[(int)Command::GetXResp] =
      registerStatistic<uint64_t>("GetXResp_uncache_recv");
  stat_noncacheRecv[(int)Command::CustomReq] =
      registerStatistic<uint64_t>("CustomReq_uncache_recv");
  stat_noncacheRecv[(int)Command::CustomResp] =
      registerStatistic<uint64_t>("CustomResp_uncache_recv");
  stat_noncacheRecv[(int)Command::CustomAck] =
      registerStatistic<uint64_t>("CustomAck_uncache_recv");
  // Events sent
  ...
  stat_dirEntryReads =
      registerStatistic<uint64_t>("eventSent_read_directory_entry");
  stat_dirEntryWrites =
      registerStatistic<uint64_t>("eventSent_write_directory_entry");
  ...
  if (protstr == "mesi" || protstr == "MESI")
    protocol = CoherenceProtocol::MESI;
  else if (protstr == "msi" || protstr == "MSI")
    protocol = CoherenceProtocol::MSI;
  else
    dbg.fatal(CALL_INFO, -1,
              "Invalid param(%s): coherence_protocol - must be 'MESI' or "
              "'MSI'. You specified: %s\n",
  ...
  if (mshrSize == 0)
    dbg.fatal(CALL_INFO, -1,
              "Invalid param(%s): mshr_num_entries - must be at least 1 or "
              "else negative to indicate an unlimited size MSHR\n",
              getName().c_str());
  ...
  if (MemEventTypeArr[(int)evb->getCmd()] != MemEventType::Cache ||
    ...
    if (is_debug_event(evb)) {
      dbg.debug(_L3_,
                ...
                Simulation::getSimulation()->getCurrentSimCycle(), timestamp,
...
bool DirectoryController::clock(SST::Cycle_t cycle) {
  ...
  while (it != retryBuffer.end()) {
    ...
    dbg.debug(_L3_,
              "E: %-20" PRIu64 " %-20" PRIu64 " %-20s Event:Retry   (%s)\n",
              Simulation::getSimulation()->getCurrentSimCycle(), timestamp,
...
bool DirectoryController::processPacket(MemEvent *ev, bool replay) {
  ...
  if (!isRequestAddressValid(ev->getAddr())) {
    dbg.fatal(
        CALL_INFO, -1,
        "%s, Error: Request address is not valid. Event: %s. Time = %" PRIu64
        "ns\n",
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Received unrecognized request: %s. Time = %" PRIu64
              "ns\n",
              getName().c_str(), ev->getVerboseString().c_str(),
              getCurrentSimTimeNano());
  }
...
void DirectoryController::handleNoncacheableResponse(MemEventBase *ev) {
  if (noncacheMemReqs.find(ev->getID()) == noncacheMemReqs.end()) {
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Received a noncacheable response that does not match "
              "a pending request. Event: %s\n. Time: %" PRIu64 "ns\n",
              getName().c_str(), ev->getVerboseString().c_str(),
              getCurrentSimTimeNano());
  }
...
void DirectoryController::printStatus(Output &statusOut) {
  ...
  statusOut.output("  Requests waiting to be handled:  %zu\n",
                   eventBuffer.size());
  //    for(std::list<std::pair<MemEvent*,bool> >::iterator i =
  //    workQueue.begin() ; i != workQueue.end() ; ++i){
...
bool DirectoryController::isRequestAddressValid(Addr addr) {
  ...
  if (0 == region.interleaveSize) {
    ...
    if (offset >= region.interleaveSize)
      return false;
    return true;
  }
}
...
void DirectoryController::turnClockOn() {
  ...
  timestamp--; // reregisterClock returns next cycle clock will be enabled, set
               // timestamp to current cycle
...
void DirectoryController::init(unsigned int phase) {
  ...
  // InitData: Name, NULLCMD, Endpoint type, inclusive of all upper levels, will
  // send writeback acks, line size
  if (!phase) {
    ...
    if (memoryName != "" || cpuLink != memLink) {
      MemEventInitRegion *reg =
          new MemEventInitRegion(getName(), memLink->getRegion(), true);
      reg->setDst(memoryName);
      memLink->sendInitData(reg);
    }
    if (cpuLink != memLink)
      cpuLink->sendInitData(
          new MemEventInitCoherence(getName(), Endpoint::Directory, true, true,
                                    false, cacheLineSize, true));
  ...
  while (MemEventInit *ev = cpuLink->recvInitData()) {
    if (ev->getCmd() == Command::NULLCMD) {
      ...
      /* Check that memory name is valid - really only need to do this once, but
       * since it's init, whatever */
      if (cpuLink == memLink && memoryName != "" &&
          !cpuLink->isDest(memoryName)) {
        dbg.fatal(
            CALL_INFO, -1,
            "%s, Invalid param: net_memory_name - must name a valid memory "
            "component in the system. You specified: %s.\nNOTE this parameter "
            "is deprecated anyways in favor of giving memory and directory "
            "controllers their own region parameters\n",
  ...
  if (cpuLink != memLink) {
    while ((ev = memLink->recvInitData())) {
      ...
      if (initEv && initEv->getCmd() == Command::NULLCMD) {
        dbg.debug(_L10_, "I: %-20s   Event:Init      (%s)\n", getName().c_str(),
                  initEv->getVerboseString().c_str());

        if (initEv->getInitCmd() == MemEventInit::InitCommand::Coherence) {
          MemEventInitCoherence *mEv =
              static_cast<MemEventInitCoherence *>(initEv);
          if (mEv->getSendWBAck())
            waitWBAck = true;
        }
      }
...
void DirectoryController::finish(void) { cpuLink->finish(); }

void DirectoryController::setup(void) {
  cpuLink->setup();
  // MemLinkBase * mem = memLink ? memLink : network;
  // dircc->configure(getName(), memoryName, sendWBAck, recvWBAck, network,
  // mem);
}

/**************************************************************************************************************************/
/***** The rest will eventually become a coherence manager so that we can use
 * the directory for different protocols *******/
/**************************************************************************************************************************/
...
bool DirectoryController::handleGetS(MemEvent *event, bool inMSHR) {
  ...
  case I:
    if (mshr->hasData(addr)) {
      if (!inMSHR)
        out.output("ALERT (%s): mshr should NOT have data for 0x%" PRIx64
                   " but it does...\n",
                   getName().c_str(), addr);
      else {
        if (protocol == CoherenceProtocol::MESI) {
          ...
          sendDataResponse(event, entry, mshr->getData(addr),
                           Command::GetXResp);
          mshr->clearData(addr);
        } else {
          entry->setState(S);
...
bool DirectoryController::handleGetX(MemEvent *event, bool inMSHR) {
  ...
  if (mshr->hasData(addr) &&
      mshr->getDataDirty(
          addr)) // Data was temporarily buffered here due to racing accesses
  ...
  case S:
    ...
    // Otherwise need data & invalidate sharers -> invalidate other sharers,
    // request data from Memory, SM_Inv
  ...
  case M_InvX:
    if (event->getEvict()) {
      ...
      if (mshr->decrementAcksNeeded(addr)) {
        entry->setState(I);
        retryBuffer.push_back(
            static_cast<MemEvent *>(mshr->getFrontEvent(addr)));
      }
    }
    break;
  case SD_Inv:
...
bool DirectoryController::handlePutS(MemEvent *event, bool inMSHR) {
  ...
  if (responses.find(addr) != responses.end() &&
      responses.find(addr)->second.find(event->getSrc()) !=
          responses.find(addr)->second.end()) {
    responses.find(addr)->second.erase(event->getSrc());
    if (responses.find(addr)->second.empty())
      responses.erase(addr);
  }
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Directory received PutS but state is %s. Event = %s. "
              "Time = %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool DirectoryController::handlePutX(MemEvent *event, bool inMSHR) {
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Directory received PutX but state is %s. Event = %s. "
              "Time = %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool DirectoryController::handlePutE(MemEvent *event, bool inMSHR) {
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Directory received PutE but state is %s. Event = %s. "
              "Time = %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool DirectoryController::handlePutM(MemEvent *event, bool inMSHR) {
  ...
  default:
    dbg.fatal(CALL_INFO, -1,
              "%s, Error: Directory received PutM but state is %s. Event = %s. "
              "Time = %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool DirectoryController::handleFetchInv(MemEvent *event, bool inMSHR) {
  ...
  case I:
    if (!(mshr->pendingWriteback(addr) ||
          (mshr->exists(addr) &&
           mshr->getFrontEvent(addr)->getCmd() == Command::FlushLineInv))) {
...
bool DirectoryController::handleGetSResp(MemEvent *event, bool inMSHR) {
  ...
  if (state != IS && state != S_D) {
    out.fatal(CALL_INFO, -1,
              "%s, Error: Received GetSResp in unhandled state '%s'. Event: "
              "%s. Time: %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
  ...
  mshr->setData(addr, event->getPayload(),
                false); // Save data for a subsequent GetS
...
bool DirectoryController::handleGetXResp(MemEvent *event, bool inMSHR) {
  ...
  case S_D:
    ...
    sendDataResponse(reqEv, entry, event->getPayload(), Command::GetSResp);
    mshr->setData(addr, event->getPayload(),
                  false); // So subsequent GetS can get data
    break;
  case IM:
    entry->setState(M);
    entry->setOwner(reqEv->getSrc());
  ...
  case SM_Inv:
    entry->setState(S_Inv);
    mshr->setData(addr, event->getPayload(),
                  false); // Save data for when the invalidations finish
    if (is_debug_addr(addr)) {
      eventDI.newst = entry->getState();
      eventDI.verboseline = entry->getString();
    }
  ...
  default:
    out.fatal(CALL_INFO, -1,
              "%s, Error: Received GetXResp in unhandled state '%s'. Event: "
              "%s. Time: %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool DirectoryController::handleFlushLineResp(MemEvent *event, bool inMSHR) {
  ...
  default:
    out.fatal(CALL_INFO, -1,
              "%s, Error: Received FlushLineResp in unhandled state '%s'. "
              "Event: %s. Time: %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool DirectoryController::handleAckInv(MemEvent *event, bool inMSHR) {
  ...
  default:
    out.fatal(CALL_INFO, -1,
              "%s, Error: Received AckInv in unhandled state '%s'. Event: %s. "
              "Time: %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
...
bool DirectoryController::handleFetchXResp(MemEvent *event, bool inMSHR) {
  ...
  if (state != M_InvX)
    out.fatal(CALL_INFO, -1,
              "%s, Error: Received FetchXResp in unhandled state '%s'. Event: "
              "%s. Time: %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
  ...
  mshr->setData(addr, event->getPayload(),
                event->getDirty()); // Save data for retry
...
bool DirectoryController::handleFetchResp(MemEvent *event, bool inMSHR) {
  ...
  if (state != S_Inv && state != M_Inv)
    out.fatal(CALL_INFO, -1,
              "%s, Error: Received FetchResp in unhandled state '%s'. Event: "
              "%s. Time: %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              event->getVerboseString().c_str(), getCurrentSimTimeNano());
...
bool DirectoryController::handleNACK(MemEvent *event, bool inMSHR) {
  ...
  case Command::ForceInv:
    ...
    if (responses.find(addr) != responses.end() &&
        responses.find(addr)->second.find(nackedEvent->getDst()) !=
            responses.find(addr)->second.end() &&
        responses.find(addr)->second.find(nackedEvent->getDst())->second ==
            nackedEvent->getID())
      break;
    delete nackedEvent;
    return true;
  default:
    out.fatal(CALL_INFO, -1,
              "%s, Error: Received NACK in unhandled state '%s'. Event: %s. "
              "Time: %" PRIu64 "ns\n",
              getName().c_str(), StateString[state],
              nackedEvent->getVerboseString().c_str(), getCurrentSimTimeNano());
  }
  // Resend nack'd event
  cpuMsgQueue.insert(
      std::make_pair(timestamp + mshrLatency,
                     nackedEvent)); // Resend after MSHR lookup (assuming we
                                    // store info about responses there)
...
bool DirectoryController::retrieveDirEntry(DirEntry *entry, MemEvent *event,
                                           bool inMSHR) {
  MemEventStatus status =
      inMSHR ? MemEventStatus::OK : allocateMSHR(event, false);
  if (status == MemEventStatus::Reject)
    return false;
  ...
  case M_d:
    return true;
  default:
    dbg.fatal(CALL_INFO, 01,
              "%s, Error: Attempting to retrieve a directory entry from memory "
              "in state '%s'. Address: 0x%" PRIx64 ". Time: %" PRIu64 "ns\n",
              getName().c_str(), StateString[state], entry->getBaseAddr(),
              getCurrentSimTimeNano());
  }
...
MemEventStatus DirectoryController::allocateMSHR(MemEvent *event, bool fwdReq,
                                                 int pos) {
  int end_pos =
      mshr->insertEvent(event->getBaseAddr(), event, pos, fwdReq, false);
  if (end_pos == -1) {
    if (is_debug_event(event)) {
...
void DirectoryController::cleanUpAfterRequest(MemEvent *event, bool inMSHR) {
  ...
  if (inMSHR) {
    if (mshr->getFrontType(addr) == MSHREntryType::Event)
      ...
      mshr->removeEntry(
          addr,
          1); // Sometimes we've inserted a writeback in response to a request
              // and we need to skip the writeback to remove the request
  }
...
void DirectoryController::updateCache(
    DirEntry *entry) { // TODO replace with a proper cache!
...
void DirectoryController::sendEntryToMemory(DirEntry *entry) {
  Addr entryAddr = 0;
  MemEvent *me =
      new MemEvent(getName(), entryAddr, entryAddr, Command::PutE, lineSize);
  me->setSize(entrySize);
...
void DirectoryController::issueMemoryRequest(MemEvent *event, DirEntry *entry) {
  ...
  if (memoryName == "")
    reqEvent->setDst(
        memLink->findTargetDestination(reqEvent->getRoutingAddress()));
  else
...
void DirectoryController::issueFlush(MemEvent *event) {
  ...
  if (mshr->hasData(addr) &&
      mshr->getDataDirty(addr)) { // also writeback dirty data
    flush->setEvict(true);
...
void DirectoryController::issueFetch(MemEvent *event, DirEntry *entry,
                                     Command cmd) {
  Addr addr = event->getBaseAddr();
  MemEvent *fetch =
      new MemEvent(getName(), event->getAddr(), addr, cmd, lineSize);
  ...
  if (responses.find(addr) == responses.end()) {
    ...
    responses.find(addr)->second.insert(
        std::make_pair(entry->getOwner(), fetch->getID()));
  }
...
void DirectoryController::issueInvalidations(MemEvent *event, DirEntry *entry,
                                             Command cmd) {
  std::string rqstr = (event->getSrc());

  for (set<std::string>::iterator it = entry->getSharers()->begin();
       it != entry->getSharers()->end(); it++) {
    if (*it == rqstr)
      continue;
...
void DirectoryController::issueInvalidation(std::string dst, MemEvent *event,
                                            DirEntry *entry, Command cmd) {
  ...
  if (responses.find(addr) == responses.end()) {
    ...
    responses.find(addr)->second.insert(
        std::make_pair(entry->getOwner(), inv->getID()));
  }
...
void DirectoryController::sendDataResponse(MemEvent *event, DirEntry *entry,
                                           std::vector<uint8_t> &data,
                                           Command cmd, uint32_t flags) {
...
void DirectoryController::sendResponse(MemEvent *event, uint32_t flags,
                                       uint32_t memflags) {
...
void DirectoryController::writebackData(MemEvent *event) {
  MemEvent *wb = new MemEvent(getName(), event->getBaseAddr(),
                              event->getBaseAddr(), Command::PutM, lineSize);
...
void DirectoryController::sendFetchResponse(MemEvent *event) {
  ...
  memMsgQueue.insert(
      std::make_pair(timestamp + accessLatency, MemMsg(ack, false)));
}
...
void DirectoryController::sendOutgoingEvents() {
  ...
  while (!cpuMsgQueue.empty() && cpuMsgQueue.begin()->first <= timestamp) {
    ...
    if (is_debug_event(ev)) {
      dbg.debug(_L4_,
                "E: %-20" PRIu64 " %-20" PRIu64 " %-20s Event:Send    (%s)\n",
                Simulation::getSimulation()->getCurrentSimCycle(), timestamp,
    ...
    if (startTimes.find(ev->getResponseToID()) != startTimes.end()) {
      if (CommandClassArr[(int)ev->getCmd()] == CommandClass::Data)
        stat_getRequestLatency->addData(timestamp -
                                        startTimes.find(ev->getResponseToID())
                                            ->second); // GetS, GetX, GetSX
      else
        stat_replacementRequestLatency->addData(
            timestamp -
            startTimes.find(ev->getResponseToID())->second); // Put*, FlushLine*
...
void DirectoryController::printDebugInfo() {
  ...
  dbg.debug(_L5_,
            "C: %-20" PRIu64 " %-20" PRIu64 " %-20s %-13s 0x%-16" PRIx64
            " %-15s %-6s %-6s %-10s %-15s",
            Simulation::getSimulation()->getCurrentSimCycle(), timestamp,
            getName().c_str(), cmd.c_str(), eventDI.addr, id.str().c_str(),
            StateString[eventDI.oldst], StateString[eventDI.newst],
            eventDI.action.c_str(), reas.str().c_str());
