...
public:
  ...
  SST_ELI_REGISTER_COMPONENT(MultiThreadL1, "memHierarchy", "multithreadL1",
                             SST_ELI_ELEMENT_VERSION(1, 0, 0),
                             "Layer to connect multiple CPUs to a single L1 as "
                             "if multiple hardware threads",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      {"clock",
       ...
       NULL},
      {"requests_per_cycle",
       "(uint) Number of requests to forward to L1 each cycle (for all threads "
       "combined). 0 indicates unlimited",
       "0"},
      {"responses_per_cycle",
       "(uint) Number of responses to forward to threads each cycle (for all "
       "threads combined). 0 indicates unlimited",
       "0"},
      {"debug",
  ...
  SST_ELI_DOCUMENT_PORTS({"cache",
                          "Link to L1 cache",
                          {"memHierarchy.MemEventBase"}},
                         {"thread%(port)d",
                          "Links to threads/cores",
                          {"memHierarchy.MemEventBase"}})
