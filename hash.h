...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      NoHashFunction, "memHierarchy", "hash.none",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Default hash function - none, returns unmodified value",
      SST::MemHierarchy::HashFunction)

  NoHashFunction(ComponentId_t id, Params &params) : HashFunction(id, params) {}

  inline uint64_t hash(uint32_t ID, uint64_t value) { return value; }
};
...
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(
      LinearHashFunction, "memHierarchy", "hash.linear",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Linear hash from C99 standard's RNG function",
      SST::MemHierarchy::HashFunction)

  LinearHashFunction(ComponentId_t id, Params &params)
      : HashFunction(id, params) {}

  uint64_t hash(uint32_t ID, uint64_t x) { return 1103515245 * x + 12345; }
};

/* Just a simple xor-based hash. */
class XorHashFunction : public HashFunction {
public:
  SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(XorHashFunction, "memHierarchy",
                                        "hash.xor",
                                        SST_ELI_ELEMENT_VERSION(1, 0, 0),
                                        "Simple XOR hash",
                                        SST::MemHierarchy::HashFunction)

  XorHashFunction(ComponentId_t id, Params &params)
      : HashFunction(id, params) {}
