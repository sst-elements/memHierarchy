...
MemNICFour::MemNICFour(ComponentId_t id, Params &params)
    : MemNICBase(id, params) {
  build(params);
}
...
void MemNICFour::build(Params &params) {
  ...
  for (int i = 0; i < 4; i++) {
    link_control[i] = loadUserSubComponent<SST::Interfaces::SimpleNetwork>(
        pref[i], ComponentInfo::SHARE_NONE, 1);
    if (link_control[i] == nullptr) {

      std::string lctype = params.find<std::string>(pref[i] + ".linkcontrol",
                                                    "kingsley.linkcontrol");
      Params lcparams;
      lcparams.insert("link_bw", params.find<std::string>(
                                     pref[i] + ".network_bw", "80GiB/s"));
      lcparams.insert("in_buf_size",
                      params.find<std::string>(
                          pref[i] + ".network_input_buffer_size", "1KiB"));
      lcparams.insert("out_buf_size",
                      params.find<std::string>(
                          pref[i] + ".network_output_buffer_size", "1KiB"));
      lcparams.insert("port_name",
                      params.find<std::string>(pref[i] + ".port", ""));

      link_control[i] = loadAnonymousSubComponent<SimpleNetwork>(
          lctype, pref[i], 0,
          ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, lcparams,
          1);
    }

    packetHeaderBytes[i] =
        extractPacketHeaderSize(params, pref[i] + ".min_packet_size");
  }
  ...
  link_control[DATA]->setNotifyOnReceive(new SimpleNetwork::Handler<MemNICFour>(
      this, &MemNICFour::recvNotifyData));
  ...
  stat_oooDepth =
      registerStatistic<uint64_t>("outoforder_depth_at_event_receive");
  stat_oooDepthSrc =
      registerStatistic<uint64_t>("outoforder_depth_at_event_receive_src");
...
bool MemNICFour::clock() {
  if (sendQueue[REQ].empty() && sendQueue[ACK].empty() &&
      sendQueue[FWD].empty() && sendQueue[DATA].empty() && recvQueue.empty())
    return true;
...
void MemNICFour::send(MemEventBase *ev) {
  ...
  if (ev->getPayloadSize() == 0) {
    if (CommandClassArr[(int)ev->getCmd()] == CommandClass::Request)
      net = REQ;
    else if (CommandClassArr[(int)ev->getCmd()] == CommandClass::Ack)
      net = ACK;
    else
      net = FWD;
  }
  ...
  if (is_debug_event(ev)) {
    ...
    if (net == REQ)
      ...
      netstr = "fwd";
    dbg.debug(_L9_,
              "%s, memNIC adding to %s send queue: dst: %" PRIu64
              ", bits: %zu, cmd: %s\n",
              getName().c_str(), netstr.c_str(), req->dest, req->size_in_bits,
...
void MemNICFour::doRecv(SimpleNetwork::Request *req, NetType net) {
  ...
  OrderedMemRtrEvent *mre =
      processRecv(req); // Return the splitmemrtrevent if we have one
  ...
  if (mre != nullptr) {
    ...
    if (mre->tag == recvTags[src]) {  // Got the tag we were expecting
      ...
      while (orderBuffer[src].find(recvTags[src]) != orderBuffer[src].end()) {
        ...
        stat_orderLatency->addData(getCurrentSimTime() -
                                   orderBuffer[src][recvTags[src]].second);
        orderBuffer[src].erase(recvTags[src]);
        recvTags[src]++;
      }
    } else {
      totalOOO++;
      stat_oooEvent[net]->addData(
          1); // Count number of out of order events received
...
MemNICFour::processRecv(SimpleNetwork::Request *req) {
  if (req != nullptr) {
    ...
    if (mre->hasClientData()) {
      ...
      if (networkAddressMap.find(imre->info.name) == networkAddressMap.end()) {
        dbg.fatal(CALL_INFO, -1,
                  "%s (MemNIC), received information about previously unknown "
...
void MemNICFour::printStatus(Output &out) {
  ...
  if (link_control[DATA])
    link_control[DATA]->printStatus(out);
  out.output("      Req linkcontrol status:\n");
  if (link_control[REQ])
    link_control[REQ]->printStatus(out);
  out.output("      Ack linkcontrol status:\n");
  if (link_control[ACK])
    link_control[ACK]->printStatus(out);
  out.output("      Fwd linkcontrol status:\n");
  if (link_control[FWD])
    link_control[FWD]->printStatus(out);
