...
public:
  ...
  SST_ELI_REGISTER_COMPONENT(ScratchCPU, "memHierarchy", "ScratchCPU",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"maxAddr", "(uint) Maximum address to generate (i.e., scratchSize + "
                  "size of memory)"},
      {"rngseed",
       "(int) Set a seed for the random generator used to create requests",
       "7"},
      {"scratchLineSize",
       "(uint) Line size for scratch, max request size for scratch", "64"},
      {"memLineSize",
       "(uint) Line size for memory, max request size for memory", "64"},
      {"clock", "(string) Clock frequency in Hz or period in s", "1GHz"},
      {"maxOutstandingRequests",
       "(uint) Maximum number of requests outstanding at a time", "8"},
      {"maxRequestsPerCycle",
       "(uint) Maximum number of requests to issue per cycle", "2"},
      {"reqsToIssue",
       "(uint) Number of requests to issue before ending simulation", "1000"})
...
private:
  ...
  uint64_t scratchLineSize; // Line size for scratchpad -> controls maximum
                            // request size
  ...
  std::unordered_map<uint64_t, SimTime_t>
      requests;                     // Request queue (outstanding requests)
  ...
  uint64_t num_events_issued;   // number of events that have been issued at a
                                // given time
