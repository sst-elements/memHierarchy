...
streamCPU::streamCPU(ComponentId_t id, Params &params)
  ...
  memory = loadUserSubComponent<Interfaces::SimpleMem>(
      ...
      new Interfaces::SimpleMem::Handler<streamCPU>(this,
...
streamCPU::streamCPU() : Component(-1) {
  // for serialization only
}

void streamCPU::init(unsigned int phase) { memory->init(phase); }
...
void streamCPU::handleEvent(Interfaces::SimpleMem::Request *req) {
  ...
  if (i == requests.end()) {
    ...
    out.verbose(CALL_INFO, 1, 0,
                "Received MemEvent (response to: %10" PRIu64 ", Addr=%15" PRIu64
                ", Took: %7" PRIu64 "ns, %6zu pending requests).\n",
  ...
  if ((numLS != 0) && ((rng.generateNextUInt32() % commFreq) == 0) &&
    ...
    for (int i = 0; i < reqsToSend; i++) {
      ...
      Interfaces::SimpleMem::Request::Command cmd =
          doWrite ? Interfaces::SimpleMem::Request::Write
                  : Interfaces::SimpleMem::Request::Read;

      Interfaces::SimpleMem::Request *req =
          new Interfaces::SimpleMem::Request(cmd, nextAddr, 4 /* 4 bytes*/);
      ...
      out.verbose(CALL_INFO, 1, 0,
                  "Issued request %10d: %5s for address %20d.\n", numLS,
