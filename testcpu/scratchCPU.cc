...
ScratchCPU::ScratchCPU(ComponentId_t id, Params &params)
  ...
  if (!scratchSize)
    out.fatal(CALL_INFO, -1,
              "Error (%s): invalid param 'scratchSize' - must be at least 1",
              getName().c_str());
  if (maxAddr <= scratchSize)
    out.fatal(CALL_INFO, -1,
              "Error (%s): invalid param 'maxAddr' - must be larger than "
              "'scratchSize'",
              getName().c_str());
  if (scratchLineSize < 1)
    out.fatal(CALL_INFO, -1,
              "Error (%s): invalid param 'scratchLineSize' - must be greater "
              "than 0\n",
              getName().c_str());
  if (memLineSize < scratchLineSize)
    out.fatal(CALL_INFO, -1,
              "Error (%s): invalid param 'memLineSize' - must be greater than "
              "or equal to 'scratchLineSize'\n",
              getName().c_str());
  ...
  if (!isPowerOfTwo(memLineSize))
    out.fatal(
        CALL_INFO, -1,
        "Error (%s): invalid param 'memLineSize' - must be a power of 2\n",
        getName().c_str());
  ...
  memory = loadUserSubComponent<Interfaces::SimpleMem>(
      ...
      new Interfaces::SimpleMem::Handler<ScratchCPU>(this,
                                                     &ScratchCPU::handleEvent));
...
void ScratchCPU::init(unsigned int phase) { memory->init(phase); }

void ScratchCPU::finish() {
  out.output(
      "ScratchCPU %s Finished after %" PRIu64 " issued memory events, %" PRIu64
      " returned, %" PRIu64 " cycles\n",
...
bool ScratchCPU::tick(Cycle_t time) {
  ...
  if (num_events_issued == reqsToIssue) {
    if (requests.empty()) {
      primaryComponentOKToEndSim(); // Have issued all our requests and all have
                                    // returned -> DONE!
      return true;
    }
  } else {
    ...
    if (requests.size() < reqQueueSize) {
      ...
      for (int i = 0; i < reqCount; i++) {
        ...
        if (instType == 0) { // Scratch read
          ...
          uint32_t log2Size =
              rng.generateNextUInt32() % (log2ScratchLineSize + 1);
          uint32_t size = 1 << log2Size;
          // Generate address aligned to request size
          Interfaces::SimpleMem::Addr addr = (Interfaces::SimpleMem::Addr)(
              ((rng.generateNextUInt64() % scratchSize) >> log2Size)
              << log2Size);

          req = new Interfaces::SimpleMem::Request(
              Interfaces::SimpleMem::Request::Read, addr, size);
          out.debug(_L3_,
                    "ScratchCPU (%s) sending Read. Addr: %" PRIu64
                    ", Size: %u\n\n",
          ...
          data.resize(size, 0);
          req = new Interfaces::SimpleMem::Request(
              Interfaces::SimpleMem::Request::Write, addr, size, data);
          out.debug(_L3_,
                    "ScratchCPU (%s) sending Write. Addr: %" PRIu64
                    ", Size: %u\n\n",
                    getName().c_str(), addr, size);
          ...
          req = new Interfaces::SimpleMem::Request(
              Interfaces::SimpleMem::Request::Read, dstAddr, size);
          req->addAddress(srcAddr);
          out.debug(_L3_,
                    "ScratchCPU (%s) sending ScratchGet. Dst Addr: %" PRIu64
                    ", Src Addr: %" PRIu64 ", Size: %u\n\n",
          ...
          req = new Interfaces::SimpleMem::Request(
              Interfaces::SimpleMem::Request::Write, dstAddr, size);
          req->addAddress(srcAddr);
          out.debug(_L3_,
                    "ScratchCPU (%s) sending ScratchPut. Dst Addr: %" PRIu64
                    ", Src Addr: %" PRIu64 ", Size: %u\n\n",
                    getName().c_str(), dstAddr, srcAddr, size);
          ...
          req = new Interfaces::SimpleMem::Request(
              Interfaces::SimpleMem::Request::Read, addr, size);
          out.debug(_L3_,
                    "ScratchCPU (%s) sending mem Read. Addr: %" PRIu64
                    ", Size: %u\n\n",
                    getName().c_str(), addr, size);
        } else { // Memory write
          uint32_t log2Size = rng.generateNextUInt32() % (log2MemLineSize + 1);
          ...
          req = new Interfaces::SimpleMem::Request(
              Interfaces::SimpleMem::Request::Write, addr, size, data);
          out.debug(_L3_,
                    "ScratchCPU (%s) sending mem Write. Addr: %" PRIu64
                    ", Size: %u\n\n",
                    getName().c_str(), addr, size);
        }
...
void ScratchCPU::handleEvent(Interfaces::SimpleMem::Request *response) {
  std::unordered_map<uint64_t, SimTime_t>::iterator i =
      requests.find(response->id);
  if (i == requests.end())
    out.fatal(CALL_INFO, -1,
              "Received response but request not found! ID = %" PRIu64 "\n",
              response->id);
  requests.erase(i);
  num_events_returned++;
  delete response;
