...
public:
  ...
  SST_ELI_REGISTER_COMPONENT(trivialCPU, "memHierarchy", "trivialCPU",
  ...
  SST_ELI_DOCUMENT_PARAMS(
      ...
      {"rngseed", "(int) Set a seed for the random generation of addresses",
       "7"},
      {"lineSize", "(uint) Size of a cache line - used for flushes", "64"},
      {"maxOutstanding",
       "(uint) Maximum Number of Outstanding memory requests.", "10"},
      {"num_loadstore", "(int) Stop after this many reads and writes.", "-1"},
      {"reqsPerIssue", "(uint) Maximum number of requests to issue at a time",
       "1"},
      {"do_write", "(bool) Enable writes to memory (versus just reads).", "1"},
      {"do_flush", "(bool) Enable flushes", "0"},
      {"noncacheableRangeStart",
       "(uint) Beginning of range of addresses that are noncacheable.", "0x0"},
      {"noncacheableRangeEnd",
       "(uint) End of range of addresses that are noncacheable.", "0x0"},
      {"addressoffset",
       "(uint) Apply an offset to a calculated address to check for "
  ...
  SST_ELI_DOCUMENT_STATISTICS({"pendCycle",
                               "Number of pending requests per cycle", "count",
                               1})
  ...
  void finish() {
    out.verbose(CALL_INFO, 1, 0,
                "TrivialCPU %s Finished after %" PRIu64
                " issued reads, %" PRIu64 " returned (%" PRIu64 " clocks)\n",
                getName().c_str(), num_reads_issued, num_reads_returned,
                clock_ticks);
    if (noncacheableReads || noncacheableWrites)
      out.verbose(CALL_INFO, 1, 0,
                  "\t%zu Noncacheable Reads\n\t%zu Noncacheable Writes\n",
                  noncacheableReads, noncacheableWrites);

    // out.output("Number of Pending Requests per Cycle (Binned by 2
    // Requests)\n"); for(uint64_t i = requestsPendingCycle->getBinStart(); i <
    // requestsPendingCycle->getBinEnd(); i +=
    // requestsPendingCycle->getBinWidth()) {
    //    out.output("  [%" PRIu64 ", %" PRIu64 "]  %" PRIu64 "\n",
    //	i, i + requestsPendingCycle->getBinWidth(),
    //requestsPendingCycle->getBinCountByBinStart(i));
    //}
  }
