...
trivialCPU::trivialCPU(ComponentId_t id, Params &params)
  ...
  if (maxReqsPerIssue < 1) {
    out.fatal(CALL_INFO, -1,
              "TrivialCPU cannot issue less than one request at a time...fix "
              "your input deck\n");
  }
  ...
  memory = loadUserSubComponent<Interfaces::SimpleMem>(
      ...
      new Interfaces::SimpleMem::Handler<trivialCPU>(this,
  ...
  if (!memory) {
    ...
    // out.fatal(CALL_INFO, -1, "Unable to load memHierarchy.memInterface
    // subcomponent\n");
  }
...
trivialCPU::trivialCPU() : Component(-1) {
  // for serialization only
}

void trivialCPU::init(unsigned int phase) { memory->init(phase); }
...
void trivialCPU::handleEvent(Interfaces::SimpleMem::Request *req) {
  ...
  if (requests.end() == i) {
    ...
    out.verbose(CALL_INFO, 2, 0,
                "%s: Received Request with command %d (addr 0x%" PRIx64
                ") [Time: %" PRIu64 "] [%zu outstanding requests]\n",
...
bool trivialCPU::clockTic(Cycle_t) {
  ...
  if ((0 != numLS) && (0 == (rng.generateNextUInt32() % commFreq))) {
    if (requests.size() < maxOutstanding) {
      ...
      if (maxReqsPerIssue > 1)
        reqsToSend += rng.generateNextUInt32() % maxReqsPerIssue;
      ...
      for (int i = 0; i < reqsToSend; i++) {
        ...
        Interfaces::SimpleMem::Request::Command cmd =
        ...
        Interfaces::SimpleMem::Request *req =
            new Interfaces::SimpleMem::Request(cmd, addr, 4 /*4 bytes*/);
        ...
        bool noncacheable =
            (addr >= noncacheableRangeStart && addr < noncacheableRangeEnd);
        if (noncacheable) {
          req->flags |= Interfaces::SimpleMem::Request::F_NONCACHEABLE;
          if (cmd == Interfaces::SimpleMem::Request::Write) {
            ++noncacheableWrites;
          } else if (cmd == Interfaces::SimpleMem::Request::Read) {
            ++noncacheableReads;
          }
        }
        ...
        out.verbose(
            CALL_INFO, 2, 0, "%s: %d Issued %s%s for address 0x%" PRIx64 "\n",
