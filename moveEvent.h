...
public:
  ...
  MoveEvent(std::string src, Addr srcAddr, Addr srcBaseAddr, Addr dstAddr,
            Addr dstBaseAddr, Command cmd)
  ...
  virtual MoveEvent *clone(void) override { return new MoveEvent(*this); }
  ...
  virtual std::string getVerboseString() override {
    ...
    str << " SrcVA: 0x" << srcVAddr_ << " DstVA: 0x" << dstVAddr_ << " IP: 0x"
        << iPtr_;
    str << std::dec;
    str << " Size: " << size_;
  ...
  virtual bool doDebug(std::set<Addr> &addr) override {
    ...
    if (it != addr.end()) {
      if (*it < dstAddr_ + size_)
        return true;
    }
    it = addr.lower_bound(srcBaseAddr_);
    if (it != addr.end()) {
      if (*it < srcAddr_ + size_)
        return true;
    }
    return false;
  }
  ...
  virtual size_t getPayloadSize() override { return 0; }
