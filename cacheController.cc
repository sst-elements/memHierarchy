...
bool Cache::clockTick(Cycle_t time) {
  ...
  // 1. Retry buffer      -> Events that need to be retried, e.g., were stalled
  // due to a pending action that is now resolved
  ...
  // Deadlock will not occur because an event cannot indefinitely block another
  // one
  // 1. An event can be accepted, in which case a later response moves up the
  // queue
  // 2. An event can be rejected, in which case we check the next one with no
  // penalty (doesn't block a later response)
  ...
  while (!prefetchBuffer_.empty()) {
    if (is_debug_event(prefetchBuffer_.front())) {
      dbg_->debug(_L3_,
                  "E: %-20" PRIu64 " %-20" PRIu64 " %-20s Event:Pref    (%s)\n",
                  Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
                  getName().c_str(),
                  prefetchBuffer_.front()->getVerboseString().c_str());
      fflush(stdout);
    }
    if (accepted != maxRequestsPerCycle_ &&
        processEvent(prefetchBuffer_.front(), false)) {
      accepted++;
...
void Cache::turnClockOn() {
  ...
  for (int64_t i = 0; i < cyclesOff;
       i++) { // TODO more efficient way to do this? Don't want to add in
              // one-shot or we get weird averages/sum sq.
    statMSHROccupancy->addData(mshr_->getSize());
  }
  // d_->debug(_L3_, "%s turning clock ON at cycle %" PRIu64 ", timestamp %"
  // PRIu64 ", ns %" PRIu64 "\n", this->getName().c_str(), time, timestamp_,
...
bool Cache::processEvent(MemEventBase *ev, bool inMSHR) {
  ...
  if (MemEventTypeArr[(int)ev->getCmd()] != MemEventType::Cache ||
      ev->queryFlag(MemEventBase::F_NONCACHEABLE)) {
  ...
  if (!arbitrateAccess(addr)) { // Disallow multiple requests to same line
                                // and/or bank in a single cycle
    if (is_debug_addr(addr)) {
      std::stringstream id;
      id << "<" << event->getID().first << "," << event->getID().second << ">";
      dbg_->debug(_L5_,
                  "A: %-20" PRIu64 " %-20" PRIu64 " %-20s %-13s 0x%-16" PRIx64
                  " %-15s %-6s %-6s %-10s %-15s\n",
                  Simulation::getSimulation()->getCurrentSimCycle(), timestamp_,
                  getName().c_str(), CommandString[(int)event->getCmd()], addr,
                  id.str().c_str(), "", "", "Stall", "(bank busy)");
    }
    return false;
  }
  ...
  default:
    out_->fatal(
        CALL_INFO, -1,
        "%s, Error: Received an unsupported command. Event: %s. Time = %" PRIu64
        "ns.\n",
...
void Cache::processNoncacheable(MemEventBase *event) {
  ...
  if (CommandCPUSide[(int)event->getCmd()]) {
    if (!(event->queryFlag(MemEvent::F_NORESPONSE))) {
      noncacheableResponseDst_.insert(
          std::make_pair(event->getID(), event->getSrc()));
    }
    coherenceMgr_->forwardTowardsMem(event);
  } else {
    std::map<SST::Event::id_type, std::string>::iterator it =
        noncacheableResponseDst_.find(event->getResponseToID());
    if (it == noncacheableResponseDst_.end()) {
      out_->fatal(CALL_INFO, 01,
                  "%s, Error: noncacheable response received does not match a "
                  "request. Event: (%s). Time: %" PRIu64 "\n",
                  getName().c_str(), event->getVerboseString().c_str(),
                  getCurrentSimTimeNano());
    }
...
void Cache::checkTimeout() {
  ...
  if (entry) {
    ...
    SimTime_t startTime = (getSimulation()->getTimeLord()->getNano())
                              ->convertFromCoreTime(entry->getStartTime());
    SimTime_t waitTime = curTime - startTime;
    if (waitTime > timeout_) {
      out_->fatal(CALL_INFO, -1,
                  "%s, Error: Maximum cache timeout reached - potential "
                  "deadlock or other error. Event: %s. Current time: %" PRIu64
                  "ns. Event start time: %" PRIu64 "ns.\n",
                  getName().c_str(),
                  entry->getEvent()->getVerboseString().c_str(), curTime,
                  startTime);
    }
  }
}
...
void Cache::init(unsigned int phase) {
  ...
  if (linkUp_ == linkDown_) {
    ...
    while (MemEventInit *event = linkDown_->recvInitData()) {
      ...
      /* If event is from one of our destinations, update parameters - link only
       * returns events from destinations */
      if (event->getInitCmd() == MemEventInit::InitCommand::Coherence) {
        MemEventInitCoherence *eventC =
            static_cast<MemEventInitCoherence *>(event);
        processInitCoherenceEvent(eventC,
                                  linkDown_->isSource(eventC->getSrc()));
      }
      delete event;
    }
    return;
  ...
  while (MemEventInit *memEvent = linkUp_->recvInitData()) {
    if (memEvent->getCmd() == Command::NULLCMD) {
      ...
      if (memEvent->getInitCmd() == MemEventInit::InitCommand::Coherence) {
        MemEventInitCoherence *eventC =
            static_cast<MemEventInitCoherence *>(memEvent);
