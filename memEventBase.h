...
public:
  MemEventInitRegion(std::string src, MemRegion region, bool setRegion)
      : MemEventInit(src, InitCommand::Region), region_(region),
  ...
  virtual std::string getVerboseString() override {
    return MemEventInit::getVerboseString() + region_.toString() +
           " SetRegion: " + (setRegion_ ? "T" : "F");
  }

private:
  MemRegion region_; // MemRegion for source
  bool setRegion_; // Whether this is a push to set the destination's region or
                   // not
