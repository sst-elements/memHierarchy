...
public:
  ...
  SST_ELI_REGISTER_COMPONENT(
      CoherentMemController, "memHierarchy", "CoherentMemController",
      SST_ELI_ELEMENT_VERSION(1, 0, 0),
      "Coherent memory controller, supports cache shootdowns and interfaces to "
      "a main memory model for timing",
...
private:
  ...
  // Caching information
  bool directory_; /* Whether directory is above us, i.e., whether a PutM
                      indicates block is no longer cached or not */
  ...
  public:
    ...
    std::set<SST::Event::id_type>
        writebacks; // Writebacks this event is waiting for, due to shootdown

    MSHREntry(SST::Event::id_type id, Command cmd, bool sdown = false)
        : id(id), cmd(cmd), shootdown(sdown) {}
  };

  std::map<Addr, std::list<MSHREntry>>
      mshr_; // Keeps outstanding events coherent

  // Event tracking
  class OutstandingEvent {
  public:
    MemEventBase *request; // Event
    std::set<Addr> addrs;  // Set of (base) addresses this event touches, should
                           // be one MSHR entry for each
    uint32_t count; // If customcmd, number of MSHR entries that are not yet
                    // ready to issue

    OutstandingEvent(MemEventBase *request, Addr addr)
        : request(request), count(0) {
      addrs.insert(addr);
    }
    OutstandingEvent(MemEventBase *request, std::set<Addr> addrs)
        : request(request), addrs(addrs) {
      count = addrs.size();
    }
