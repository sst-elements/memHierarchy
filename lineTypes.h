 ...
 * - getReplacementInfo() for returning the information that a replacement
 * policy might need
 */
...
public:
  DirectoryLine(uint32_t size, unsigned int index)
      : index_(index), addr_(0), state_(I), lastSendTimestamp_(0),
        wasPrefetch_(false) {
  ...
  bool isSharer(std::string shr) {
    return sharers_.find(shr) != sharers_.end();
  }
  size_t numSharers() { return sharers_.size(); }
  bool hasSharers() { return !sharers_.empty(); }
  bool hasOtherSharers(std::string shr) {
    return !(sharers_.empty() ||
             (sharers_.size() == 1 && sharers_.find(shr) != sharers_.end()));
  }
...
public:
  DataLine(uint8_t size, unsigned int index)
      : index_(index), addr_(0), tag_(nullptr) {
    data_.resize(size);
    info_ = new CoherenceReplacementInfo(index, I, false, false);
  }
  virtual ~DataLine() {}
  ...
  void setTag(DirectoryLine *tag) { tag_ = tag; }
  ...
  ReplacementInfo *getReplacementInfo() {
    return tag_ ? tag_->getReplacementInfo() : info_;
  }

  // String-ify for debugging
  std::string getString() { return (tag_ ? "Valid" : "Invalid"); }
};
...
public:
  CacheLine(uint32_t size, unsigned int index)
      : index_(index), addr_(0), state_(I), lastSendTimestamp_(0),
        wasPrefetch_(false) {
    data_.resize(size);
  }
...
public:
  L1CacheLine(uint32_t size, unsigned int index)
      : userLock_(0), LLSCAtomic_(false), eventsWaitingForLock_(false),
        CacheLine(size, index) {
  ...
  void setEventsWaitingForLock(bool eventsWaiting) {
    eventsWaitingForLock_ = eventsWaiting;
  }
...
public:
  SharedCacheLine(uint32_t size, unsigned int index)
      : owner_(""), CacheLine(size, index) {
...
public:
  PrivateCacheLine(uint32_t size, unsigned int index)
      : shared(false), owned(false), CacheLine(size, index) {
    info = new CoherenceReplacementInfo(index, I, false, false);
  }
