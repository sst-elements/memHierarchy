...
respPattern = re.compile(
    '\A\d+:Cache::(?:makeCPUResponse\(\):[0-9]+ ([^:]+): Creating \d+ byte Response to CPU: \([^)]+\) in Response To (\([0-9+, [0-9]+\))|handleWriteResp\(\):[0-9]+: ([^ ]+) Matched WriteResp to orig event WriteReq 0x[0-9a-f]+ (\([0-9]+, [0-9]+\)))')
