...
/*************************** Memory Controller ********************/
MemCacheController::MemCacheController(ComponentId_t id, Params &params)
   ...
   *  -> Fill backend slot with backend and memcontroller loads the compatible
   * convertor */
  MemBackend *memory = loadUserSubComponent<MemBackend>("backend");
  if (!memory) { /* Try to load from our parameters (legacy mode 1) */
    out.fatal(CALL_INFO, -1,
              "%s, ERROR: This component does not support loading "
              "backend/backendConvertor from parameters. Load as user "
              "(python-defined) subcomponents instead.",
              getName().c_str());
  }
  ...
  memBackendConvertor_->setCallbackHandlers(
      std::bind(&MemCacheController::handleLocalMemResponse, this, _1, _2),
      std::bind(&MemCacheController::turnClockOn, this));
  ...
  region_.interleaveStep =
      lineSize_ * interleaveStep; //(lineSize_ * interleaveStep) >> lineOffset_;
  ...
  if (!link_) {
    out.fatal(
        CALL_INFO, -1,
        "%s, Error: No link handler loaded into 'cpulink' subcomponent slot.\n",
        getName().c_str());
  }

  clockLink_ = link_->isClocked();
  link_->setRecvHandler(new Event::Handler<MemCacheController>(
      this, &MemCacheController::handleEvent));
  ...
  clockHandler_ =
      new Clock::Handler<MemCacheController>(this, &MemCacheController::clock);
  ...
  if (memSize_ % lineSize_ != 0)
    out.fatal(CALL_INFO, -1,
              "%s, Error - memory size must be a multiple of line size. Memory "
              "size is %zu bytes and line size is %" PRIu64 " bytes\n",
...
void MemCacheController::handleEvent(SST::Event *event) {
  ...
  default:
    out.fatal(CALL_INFO, -1,
              "Memory cache controller received unrecognized command: %s",
...
void MemCacheController::handleRead(MemEvent *event, bool replay) {
  ...
  if (cacheIndex >= cache_.size())
    out.fatal(CALL_INFO, -1,
              "%s, Error: cache index exceeds cache size, try again!\n",
              getName().c_str());
  ...
  if (is_debug_event(event)) {
    Debug(_L3_,
          "%" PRIu64 " (%s) handleRead, Line: %" PRIu64 ", 0x%" PRIx64 ", %s\n",
          getCurrentSimTimeNano(), getName().c_str(), cacheIndex, blockAddr,
  ...
  std::map<SST::Event::id_type, MemAccessRecord>::iterator it =
      outstandingEvents_.find(event->getID());
  ...
  if (mshr_.find(cacheIndex)->second.front() != event->getID()) { // Transition
    ...
    it->second.status =
        (blockState == M) ? AccessStatus::MISS_WB : AccessStatus::MISS;
    statReadMiss->addData(1);
    if (is_debug_event(event))
      Debug(_L3_, "%" PRIu64 " (%s) StateTransition %" PRIu64 ", %s\n",
            getCurrentSimTimeNano(), getName().c_str(), event->getID().first,
            (blockState == M) ? "MISS_WB" : "MISS");
  } else { // HIT
  ...
  it->second.reqev->setAddr(event->getAddr() - event->getBaseAddr() +
                            cacheIndex);
...
void MemCacheController::handleWrite(MemEvent *event, bool replay) {
  ...
  if (is_debug_event(event))
    Debug(_L3_,
          "\n%" PRIu64 " (%s) handleWrite, Line: %" PRIu64 ", 0x%" PRIx64
          ", %s\n",
          getCurrentSimTimeNano(), getName().c_str(), cacheIndex, blockAddr,
          StateString[blockState]);
...
void MemCacheController::handleDataResponse(MemEvent *event) {
  ...
  if (is_debug_event(event))
    Debug(_L3_,
          "\n%" PRIu64 " (%s) handleDataResponse, Line: %" PRIu64 ", 0x%" PRIx64
          ", %s\n",
          getCurrentSimTimeNano(), getName().c_str(), cacheIndex, blockAddr,
          StateString[blockState]);
  ...
  if (is_debug_event(event))
    Debug(_L3_, "\n%" PRIu64 " (%s) StateTransition %" PRIu64 ", FIN\n",
          ...
          it->second.event->getID().first);
  memBackendConvertor_->handleMemEvent(it->second.reqev);

  // Update backing store from the request that missed if it was a write
  if (it->second.event->getCmd() == Command::PutM ||
      (it->second.event->getCmd() == Command::GetX &&
       it->second.event->queryFlag(MemEvent::F_NONCACHEABLE))) {
...
void MemCacheController::handleLocalMemResponse(Event::id_type id,
                                                uint32_t flags) {
  std::map<SST::Event::id_type, MemAccessRecord>::iterator it =
      outstandingEvents_.find(id);
  if (it == outstandingEvents_.end())
    out.fatal(CALL_INFO, -1,
              "%s, MemoryCache received unrecognized response ID: %" PRIu64
              ", %" PRIu32 "",
  ...
  if (is_debug_event(ev))
    Debug(_L3_,
          "\n%" PRIu64 " (%s) handleLocalResponse, Line: %" PRIu64
          ", 0x%" PRIx64 ", %s\n",
          getCurrentSimTimeNano(), getName().c_str(), cacheIndex, blockAddr,
          StateString[blockState]);
  ...
  case AccessStatus::MISS_WB:
    ...
    remoteWr =
        new MemEvent(getName(), blockAddr, blockAddr, Command::PutM, lineSize_);
    readData(remoteWr);
  ...
  case AccessStatus::MISS:
    ...
    link_->send(remoteRd);
    it->second.status =
        AccessStatus::DATA; // We've request data, waiting for response
  ...
  case AccessStatus::HIT:
    /* Write data. Here instead of receive to try to match backing access order
     * to backend execute order */
  ...
  case AccessStatus::FIN: // Just finished updating the cache, ready for new
                          // requests now
    if (is_debug_event(it->second.event))
  ...
  default:
    out.fatal(
        CALL_INFO, -1,
        "%s, MemoryCache encountered unhandled record status. Event is %s\n",
...
void MemCacheController::retry(uint64_t cacheIndex) {
  MemEvent *ev =
      outstandingEvents_.find(mshr_[cacheIndex].front())->second.event;
...
void MemCacheController::readData(Addr addr, size_t bytes,
...
Addr MemCacheController::toLocalAddr(Addr addr) {
  ...
  Addr rAddr =
      shift / (region_.interleaveSize >> lineOffset_); // Divide by num caches
  ...
  if (is_debug_addr(addr)) {
    Debug(_L10_,
          "\tConverting global address 0x%" PRIx64 " to local index %" PRIu64
          "\n",
          addr, rAddr);
  }
  return rAddr;
...
void MemCacheController::processInitEvent(MemEventInit *me) {
  ...
  if (Command::NULLCMD == me->getCmd()) {
    ...
    if (is_debug_event(me)) {
      Debug(_L9_, "Memory init %s - Received GetX for %" PRIx64 " size %zu\n",
...
void MemCacheController::printStatus(Output &statusOut) {
  ...
  /*    for (std::map<SST::Event::id_type, MemEventBase*>::iterator it =
     outstandingEvents_.begin(); it != outstandingEvents_.end(); it++) {
