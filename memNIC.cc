...
void MemNIC::build(Params &params) {
  ...
  link_control = loadUserSubComponent<SimpleNetwork>(
      "linkcontrol", ComponentInfo::SHARE_NONE,
      1); // 1 is the num virtual networks
  if (!link_control) {
    Params netparams;
    netparams.insert("port_name", params.find<std::string>("port", "port"));
    netparams.insert("in_buf_size", params.find<std::string>(
                                        "network_input_buffer_size", "1KiB"));
    netparams.insert("out_buf_size", params.find<std::string>(
                                         "network_output_buffer_size", "1KiB"));
    netparams.insert("link_bw",
                     params.find<std::string>("network_bw", "80GiB/s"));
    std::string link_control_class =
        params.find<std::string>("network_link_control", "merlin.linkcontrol");

    if (link_control_class != "merlin.linkcontrol")
      dbg.output("%s, Warning: use of the 'network_link_control' parameter is "
                 "deprecated in favor of specifying a named 'linkcontrol' "
                 "subcomponent in the input configuration.\n",
                 getName().c_str());

    link_control = loadAnonymousSubComponent<SimpleNetwork>(
        link_control_class, "linkcontrol", 0,
        ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, netparams, 1);
  }
  link_control->setNotifyOnReceive(
      new SimpleNetwork::Handler<MemNIC>(this, &MemNIC::recvNotify));
...
void MemNIC::send(MemEventBase *ev) {
  ...
  if (is_debug_event(ev)) {
    dbg.debug(_L9_,
              "%s, memNIC adding to send queue: dst: %s, bits: %zu, cmd: %s\n",
...
void MemNIC::printStatus(Output &out) {
  ...
  // Since this is just debug/fatal we're just going to read out the queue &
  // re-populate it
  std::queue<SST::Interfaces::SimpleNetwork::Request *> tmpQ;
  while (!sendQueue.empty()) {
    MemEventBase *ev =
        static_cast<MemRtrEvent *>(sendQueue.front()->inspectPayload())->event;
