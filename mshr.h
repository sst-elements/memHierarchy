...
public:
  ...
  std::list<Addr> *getPointers() { return evictPtrs; }

  MemEventBase *getEvent() { return event; }

  bool getDowngrade() { return downgrade; }

  /* Remove event at head of queue and swap with a new one (leaving entry in
   * place) */
  ...
  std::string getString() {
    ...
    if (type == MSHREntryType::Event) {
      ...
      str << " Type: Evict (";
      for (std::list<Addr>::iterator it = evictPtrs->begin();
           it != evictPtrs->end(); it++) {
        str << " 0x" << std::hex << *it;
      }
      str << ")";
...
private:
  ...
  SimTime_t time;
  bool needEvict;
  bool inProgress; // Whether event is currently being handled; prevents early
                   // retries
  bool profiled;
...
public:
  ...
  int insertEvent(Addr addr, MemEventBase *event, int position, bool fwdRequest,
                  bool stallEvict);
...
private:
  void printDebug(uint32_t level, std::string action, Addr addr,
                  std::string reason);
